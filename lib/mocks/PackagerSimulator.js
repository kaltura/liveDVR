...
PackagerSim.prototype.alignTime = function (time){
    ...
    var idx = indexOfTime(time + that.playlist.inner.clipTimes[j] - (that.playlist.inner.firstClipTime + seq.inner.firstKeyFrameOffset),
    ...
    var srcIdx = indexOfTime(time - src.offset,
        src.durations);

    playlistUtils.dbgAssert(srcIdx>=0);

    return { offset: seq.keyFramesMan.firstDTS[idx],
...
var beforePlaylistChanged = function () {
    ...
    if(totalDuration > 0){
        ...
        var lowerLimit = that.playlist.inner.firstClipTime - that.playlist.inner.segmentBaseTime;
        var upperLimit = totalDuration + lowerLimit;

        var maxSegIndex = that.getSegmentIndex(upperLimit);
        if(maxSegIndex > 0) {
