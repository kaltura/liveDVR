...
WowzaAdapter.prototype.getLiveEntries = function() {
    ...
    return networkClient.read({
        ...
        .then(function(res) {
            ...
            var instanceList = _.filter(res.instanceList, function(instanceList) {
                return instanceList.incomingStreams.length > 0;
            });
            if (instanceList.length === 0) {
                logger.debug("No streams are currently playing");
                return [];
            }
            var uniqueList = _.uniq(instanceList[0].incomingStreams, function(item) {
