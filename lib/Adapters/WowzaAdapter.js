...
function getIncomingStreamsInfo(incomingStreams, uniqueList) {
...
function mapNewStream(stream) {
    let that = this;
    var entryId = stream.entryId;
    let flavorParamsIds = stream.flavors.join(',');

    if (checkFlavorsMatch.call(that, that.entries[entryId], flavorParamsIds)) {
...
WowzaAdapter.prototype.getLiveEntries = function() {
    ...
    return networkClient.read({
        ...
        .then(function(res) {
            ...
            let uniqueList = {};
            getIncomingStreamsInfo.call(that, incomingStreams, uniqueList);
            ...
            var promises = _.map(uniqueList, (stream) => {
                return mapNewStream.call(that, stream);
            });
