...
WowzaAdapter.prototype.getLiveEntries = function() {
    ...
    return networkClient.read({
        ...
        .then(function(res) {
            ...
            // First filter and leave only the objects containing incoming streams list
            var instanceList = _.filter(res.instanceList, function(list) {
                return (list.incomingStreams.length > 0);
            });
            var incomingStreams = [];
            // Concat all incoming streams into one big list
            _.each(instanceList, function(list) {
               _.each(list.incomingStreams, function(item) {
                   incomingStreams.push(item);
               })
            });
            // Erase duplicated stream names
