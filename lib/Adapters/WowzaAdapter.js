...
var prefix = ((user !== "") && (password !== "")) ? user + ":" + password + "@" + hostname : hostname;
var wowzaUrl = "http://" + prefix + ":8087/v2/servers/_defaultServer_/vhosts/_defaultVHost_/applications/" + applicationName + "/instances";
...
function deleteInactiveStreams(currentlyActiveStreams) {
    var that = this;
    logger.debug("Retrieve list of entries that are no longer active for deletion");
    var unactiveEntries = _.reject(that.entries, function(e1) {
        return _.findWhere(currentlyActiveStreams, {'entryId' : e1.entryId});
    });
    logger.debug("Deleting %d entries from list", unactiveEntries.length);
    _.each(unactiveEntries, function(e) {
        logger.debug("Entry %s deleted", e.entryId);
...
WowzaAdapter.prototype.getLiveEntries = function() {
    ...
    return networkClient.read({
        ...
        .then(function(res) {
            ...
            logger.debug("Check for streams that no longer play and update wowzaAdapter's entries list");
                ...
                return backendClient.getEntries([entryId])
                    .then(function(entryArr) {
                        if (entryArr.length) {
                            ...
                            logger.info("Received stream info for entry %s", entryId);
        ...
        .catch(function(err) {
            logger.error("Received the following warning: %s, %s", err, err.stack);
            return [];
        });
}
