...
function WowzaAdapter() {
    ...
    this.logger= loggerDecorator(logger, "[WowzaAdapter] ");
}
...
function deleteInactiveStreams(currentlyActiveStreams) {
    ...
    var inactiveEntries = _.reject(that.entries, function(e1) {
     ...
     _.each(inactiveEntries, function(e) {
        that.logger.debug("[%s] Detected that entry is not streaming anymore", e.entryId);
...
WowzaAdapter.prototype.getLiveEntries = function() {
    ...
    return networkClient.read({
        ...
        .then(function(res) {
            that.logger.debug("Received playing entries list from Wowza")
            if (res.instanceList.length === 0) {
                that.logger.debug("No wowza instances");
                return [];

            }
            if (res.instanceList[0].incomingStreams.length === 0) {
                that.logger.debug("No streams are currently playing");
                return [];
            }
            ...
            that.logger.debug("Check for streams that are no longer playing and update list");
                ...
                return backendClient.getEntries([entryId])
                    .then(function(entryArr) {
                        if (entryArr.length) {
                            ...
                            that.logger.info("Received stream info for entry %s %j", entryId,newEntry);
                            return Q.resolve(newEntry);
                        }
                        that.logger.error("Entry %s doesn't exist in server, reject promise", entryId);
        ...
        .catch(function(err) {
            that.logger.error("Received the following warning: %j, (%s)", err, err.stack ? err.stack : "");
            return [];
        });
}
