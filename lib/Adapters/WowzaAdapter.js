...
function WowzaAdapter(mediaServerConfig,hostname, metadataHostName) {
    this.entries = {};
    ...
    this._wowzaUrl = "http://" + authentication + metadataHostName + ":8087/v2/servers/_defaultServer_/vhosts/_defaultVHost_/applications/" + applicationName + "/instances";
    this._wowzaDiagUrl = "http://" + authentication + metadataHostName + ":8086/diagnostics/entries";
    ...
    if (index > 0) {
        pathPrefix = hostname.substr(index);
    ...
    this._baseUrl= url.format({
        ...
        hostname : hostname,
        port : port,
        pathname : pathPrefix + applicationName
    });
    ...
    return networkClient.read({
        ...
        .catch(function(err) {
            ...
            if ((Date.now() - that.getLiveEntriesTimestamp) < (1000 * that.disconnectTimeoutInSec) ) {
                ...
                logger.warn(`Wowza access problem (${that._wowzaUrl}) for more than ${that.disconnectTimeoutInSec} sec, all live entries will be stopped!!!`);                that.entries = {};
