...
function WowzaAdapter(mediaServerConfig,hostname) {
    ...
    this.disconnectDurationMillisecToResetLiveEntries = mediaServerConfig.disconnectDurationMillisecToResetLiveEntries;
    this.getLiveEntriesTimestamp = 0;
    this.liveEntriesCash = [];
        ...
        .then(function(res) {
            ...
            return Q.allSettled(promises)
                .then(function(res) {
                    ...
                    that.liveEntriesCash = _.map(passed, (p) => {
                        return p.value;
                    });
                    return Q.resolve(that.liveEntriesCash)
                });
        })
        .catch(function(err) {
            logger.error("Failed to get live entries from wowza. Error: %s", errorUtils.error2string(err));
            if (Date.now() - that.getLiveEntriesTimestamp < that.disconnectDurationMillisecToResetLiveEntries ) {
                return Q.resolve(that.liveEntriesCash);
            } else {
                logger.warn(`Wowza access problem for more than ${that.disconnectDurationMillisecToResetLiveEntries} milliseconds, all live entries will be stopped!!!`);
                that.entries = {};
                return [];
            }
        });
