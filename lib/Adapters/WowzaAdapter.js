...
WowzaAdapter.prototype.getLiveEntries = function() {
    ...
    return networkClient.read({
        ...
        .then(function(res) {
            ...
            var instanceList = _.map(res.instanceList, function(list) {
               if (list.incomingStreams.length > 0) {
                   return _.each(list.incomingStreams, function(s) { return s });
               }
            });
            ...
            var uniqueList = _.uniq(instanceList[0], function(item) {
