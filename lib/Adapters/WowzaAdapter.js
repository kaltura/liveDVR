    ...
    return networkClient.read({
        ...
        .then(function(res) {
            ...
            var promises = _.map(uniqueList, (stream) => {
                ...
                if (cachedEntryInfo) {
                    if (!cachedEntryInfo.canPlay ) {
                        if (cachedEntryInfo.startRetryCount > that.flavorMismatchRetryAttempts) {
                            cachedEntryInfo.canPlay = true;
                            logger.info("flavorParamsIds mismatch timeout (%d retries), override configured (%j) with (%j)", cachedEntryInfo.startRetryCount, cachedEntryInfo.flavorParamsIds, flavorParamsIds);
                            ///TODO should we override or just go with the configured ones?!?!?
                            cachedEntryInfo.flavorParamsIds = flavorParamsIds;
                        } else if (cachedEntryInfo.flavorParamsIds === flavorParamsIds) {
                            cachedEntryInfo.canPlay = true;
                            logger.info("configured and actual flavorParamsIds is now the same  (%j) after %d tries", cachedEntryInfo.flavorParamsIds, cachedEntryInfo.startRetryCount);
                        }

                        if (cachedEntryInfo.canPlay) {
                            delete cachedEntryInfo.startRetryCount;
                        } else {
                            logger.info("flavorParamsIds mismatch between configured (%j) and actual (%j)", cachedEntryInfo.flavorParamsIds, flavorParamsIds);
                            cachedEntryInfo.startRetryCount++;
                        }
                    }
                ...
                return backendClient.getEntryInfo(entryId)
                    .then((entryInfo) => {
                        if (entryInfo) {
                            ...
                            if (!entryInfo.canPlay) {
                                logger.info("flavorParamsIds mismatch between configured (%j) and actual (%j)",entryInfo.flavorParamsIds,flavorParamsIds);
                            }
                            entryInfo.sourceIp = stream.sourceIp;
                            entryInfo.baseUrl = that._baseUrl;
                            entryInfo.startRetryCount=1;
