    ...
    return NetworkClient.read( {
        ...
        .then(function(res) {
            ...
            var promises= _.map(res.instanceList[0].incomingStreams,function(stream) {
                ...
                return backendClient.getEntries([entryId]).then(function (entryArr) {

                    var newEntry=entryArr[0];
                    self.entries[entryId] = newEntry;

                    newEntry.getStreamInfo=function() {
                        return  new WowzaStreamInfo(entryId, newEntry.flavorParamsIds, stream.sourceIp);
                    };
