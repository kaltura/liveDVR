...
TestHelper.prototype.validateFlavor = function (entryId, flavorId) {
    ...
    return networkClient.read({ url: url, timeout: 10000 }).then(function(content) {
        ...
        that.logger.warn('[%s-%s] **** SUCCESSFULLY VALIDATED ****  obj=[%s], url=[%s]', entryId, flavorId, url, m3u8);
    }).catch (function(e) {
        that.logger.error('[%s-%s] **** VALIDATION FAILED!!! **** error=[%s], url=[%s]', entryId, flavorId, url, e.message);
    });
}
