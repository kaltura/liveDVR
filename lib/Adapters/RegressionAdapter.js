...
/* Todo
    ...
    3. what is the data I am checking? playlist.json and mp4 files????
 */
...
/*
function FlavorInfo(flavorId) {

    this.id = flavorId;
    this.chunklist_counter = 0;
    this.playlist = {};

    this.increaseCounter = function() {
        this.chunklist_counter++;
    }

    this.updateFlavorPlaylist(playlist) {
...
function EntryInfo(entryId) {
    ...
    this.id = entryId;
    // dictionary of chunclist counters per entry flavor
...
// Todo: verify how correctly export this function from WowzaStreamInfo
...
TestHelper.prototype.initEntries = function() {
    ...
    _.each(entriesInfo, function(entryConfig) {
        ...
        _.each(flavors, function (id) {
            that.entries[entryConfig.entryId].chunklist_index[id] = 0;
            that.entries[entryConfig.entryId].initialized[id] = false;
        });
    });
}
...
TestHelper.prototype.initPlaylist = function() {
    ...
    //Todo
    // 1) initialize the entries separate from the playlist
    // 2) separate to more method calls and path that
    // 3) retest
                ...
                return m3u8Parser.parseM3U8(content, {'verbatim': true})
                    .then(function (m3u8) {
                        ...
                        _.each(m3u8.items.StreamItem, function (item) {
                            ...
                            var playlist_per_flavor = JSON.stringify(m3u);
                            that.entries[entryConfig.entryId].playlist[id] = playlist_per_flavor;
                            that.entries[entryConfig.entryId].initialized[id] = true;

                        });
                    })
                    .then ( function() {

                        that.entries[entryConfig.entryId].playlist_initialized = true;
                    });
                    */
            })
...
TestHelper.prototype.initPlaylistPerFlavor = function (entryId, all_playlist) {
    ...
    return m3u8Parser.parseM3U8(all_playlist, {'verbatim': true})
        .then(function (m3u8) {
            // build playlist per flavor
            _.each(m3u8.items.StreamItem, function (item) {
                ...
                */
                ...
                that.entries[entryId].initialized[id] = true;
            });
        })
        .then( function() {
...
TestHelper.prototype.getPlaylist = function(entryId, flavorId) {
     return this.entries[entryId].playlist[flavorId];
}
...
TestHelper.prototype.prevChunklist = function (entryId, flavorId) {
    ...
    return ("00000" + (_.max([this.entries[entryId].chunklist_index[flavorId]-1,0])) ).slice(-6) + '_chunklist.m3u8';
}

TestHelper.prototype.validateFlavor = function (entryId, flavorId) {

    var url=util.format("http://localhost:8080/kLive/smil:%s_all.smil/%s/%s", entryId, flavorId, this.prevChunklist(entryId, flavorId));
...
TestHelper.prototype.resolveUrl = function(url, params) {
  ...
  /*      else if (params.filename.indexOf('playlist.m3u8') === -1) {
         var extended_filename = this.nextChunklist(params.entryId, params.flavorId);
         params.fullpath += '/' + params.flavorId + '/' + extended_filename;
         }
         else if (params.flavorId.indexOf('all') > -1) {
        ...
        /*
         var min = _.min(flavorChunklistIndex);
         if (flavorChunklistIndex[flavorId]===min) {
         flavorChunklistIndex[flavorId]++;
         validate=true;
         }

         filename='/' + flavorId + '/' + flavorChunklistIndex[flavorId].pad('0',5)_chunklist.m3i8;
         */
        ...
        if (testHelper.resolveUrl(req.url, params)) {
            if (!params.playlist && testHelper.entries[params.entryId].playlist_initialized
                ...
                p.finally(function () {
                    ...
                    /*
                    if (params.playlist) {
                            testHelper.initPlaylistPerFlavor(params.entryId, res)
                                .then( function() {
                                    res.end(testHelper.entries[params.entryId].playlist[params.flavorId]);
                                })
                                .catch( function() {
            ...
            } else if (testHelper.entries[params.entryId].initialized[params.flavorId]) {
                res.writeHead(200);
