...
function readFile(res, full_path_file, entryId) {
    fs.readFile(full_path_file, function (err, data) {
        if (err) {
            ...
            testHelper.logger.error('[%s] failed to read path=%s. Error: %s Stack: %s', entryId, full_path_file, err.message, err.stack);
/*            return testHelper.validators[entryId].saveResultsToFile('Failed to download %s.')
                .then(function () {
                    // todo: move the process.exit() to better place...
                    process.exit(-5);
                })
                .catch(function (error) {
                    testHelper.logger.error('[%s] failed to validate and maybe save regression test results. Error: %s Stack: %s', entryId, error.message, error.stack);
                    process.exit(-5);
                });*/
        }
        res.writeHead(200);
        ...
        return fs_utils.cleanFolder(fullpath, 'obsolete').then(function () {
            ...
            return that.getLastChunklistFileIndex(entryConfig.entryId, that.entries[entryConfig.entryId].data_warehouse_path)
                .then(function (last_chunklist_index) {
                    that.entries[entryConfig.entryId].last_chunklist_index = last_chunklist_index - entryConfig.firstChunklistIndex;
                    that.validators[entryConfig.entryId] = new validator(entryConfig.entryId, flavors, that.entries[entryConfig.entryId].last_chunklist_index);
                })
                .then(function() {
                    return that.validators[entryConfig.entryId].init(entryConfig.validator);
                })
                .then(function () {
            ...
            that.logger.error('[%s] initialization failed. Error: %s, stack: %s', entryConfig.entryId, error.message, error.stack)
        });
    });
    return Q.all(p);
}
    ...
    fs.readdir(fullPath, function (err, items) {
        if (err) {
           deferred.reject(err);
            ...
            last_chunklist_index = _.reduce(items, function (index, next_file) {
                return (next_file.endsWith('m3u8') && parseInt(next_file.split('_')[0]) > index ? parseInt(next_file.split('_')[0]): index);
            }, 0);
            that.logger.debug('[%s] last checuklist index is %s', entryId, last_chunklist_index);
            deferred.resolve(last_chunklist_index);
        }
    });
    ...
    return this.readyToValidateChunklist(entryId, index)
        ...
        .then( function() {
            if (finished_regression_test) {
                return testHelper.validators[entryId].saveResultsToFile('[%s] Finished regression test on %s chunklists.', entryId, )
                    .then(function () {
                        process.exit(0);
                    })
                    .catch(function (error) {
...
function RegressionAdapter() {
    ...
    this.initPromise=testHelper.initEntries()
        ...
        .catch( function(error) {
           logger.error('RegressionAdapter failed to initialize. Error=%s', error);
            throw err;
        });
}
