...
class RegressionConfig {
    ...
    properties() {
        ...
        this.regressionConfig['command'] = {run_regression: this.regressionConfig.enable, record: config.get('enable') };
...
RegressionConfig.prototype.initChunklistEdgeIndexes = function (entry_index) {
    ...
    let chunklist_indexes_not_initialized = ((!entryConfig.start_chunklist_index || entryConfig.start_chunklist_index === -1) ||
                                             (!entryConfig.last_chunklist_index || entryConfig.last_chunklist_index === -1));
    ...
    try {
        ...
        if (chunklist_indexes_not_initialized) {
            ...
            entryConfig.last_chunklist_index = entryConfig.last_chunklist_index === -1 ? last_chunklist_index : entryConfig.last_chunklist_index;
            ...
            entryConfig.start_chunklist_index = entryConfig.start_chunklist_index === -1 ? first_chunklist_index : entryConfig.start_chunklist_index;

            if (entryConfig.last_chunklist_index === entryConfig.start_chunklist_index) {
                let err = util.format('data warehouse is empty, verify correctness of path (%s). Cannot run regression tests', fullPath);
                this.logger.info(err);
                return false;
            } else if (entryConfig.last_chunklist_index < entryConfig.start_chunklist_index) {
                let err = util.format(`chunklist indexes error, [end_index:${entryConfig.last_chunklist_index} < start_index:${entryConfig.start_chunklist_index}] , last must be greater than start, need to fix configuration. Cannot run regression tests`);
                this.logger.info(err);
                return false;
            }
        }
