...
// todo: add support in entryId & flavors list commandline args as a replacement for url (in regression)
...
/*------------------------------------------------------------------------------
 ...
 | try to avoid using url and use only entryId and flavors.
 | if url is used with entryId and or flavors the url will be ignored.
 | Important: if entryId doesn't exit in default config or configMapping and
    ...
    try {
        for (var i = this.first + 1; i < process.argv.length; i++) {
            ...
            if (cl_config && cl_config.hasOwnProperty('url') &&
                (!this.regressionConfig.run_regression || (this.regressionConfig.run_regression && !cl_config.hasOwnProperty('entryId')))) {

                    let re = /http:\/\/.*\/smil:([01]_[^_]+)_all\.smil[\/]?(\d+)?\/(playlist.m3u8|chunklist.m3u8)/g.exec(cl_config.url);

                    cl_config['entryId'] = re[1];
...
RegressionConfig.prototype.updateRegressionConfig = function(cl_config, entry_index) {
    ...
    try {
        ...
        if (entry_index === this.regressionConfig.entries.length) {
            ...
            this.regressionConfig.entries[entry_index].entryId = cl_config['entryId'];
