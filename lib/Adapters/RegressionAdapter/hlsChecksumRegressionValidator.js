...
class HlsChecksumRegressionValidator extends RegressionValidatorBase {
    ...
    validateAllFlavors(index) {
        let p = _.map(this.flavors, (next_flavor) => {
            return this.validateSingleFlavor(this.entryId, next_flavor, index);
        });
...
HlsChecksumRegressionValidator.prototype.getSegmentFromPackager = function(item, data) {
    ...
    var flavor_header = logheader1 `${this.entryId}${data.flavor}${data.index}`;

    //let url = util.format("http://localhost:8080/kLive/smil:%s_all.smil/%s/%s", this.entryId, data.flavor, item.properties.uri);
    var options = {
        method: 'HEAD',
        url: `http://localhost:8080/kLive/smil:${this.entryId}_all.smil/${data.flavor}/${item.properties.uri}`,

        timeout: 10000};
    networkClient.read(options)
        .then(function (content) {
            // add to results new segment
            //let parsed_content = JSON.stringify(content);
            //that.logger.debug(parsed_content);
            var ts_id = item.properties.uri.split('.')[0];
            if (!_.has(that.this_run_results[data.flavor].ts[ts_id])) {
                that.this_run_results[data.flavor].ts[ts_id] = parseInt(content['content-length']);
                deferred.resolve();
...
HlsChecksumRegressionValidator.prototype.downloadFlavorSegments = function(data) {
    ...
    let that = this;
    let flavor_header = logheader1 `${this.entryId}${data.flavor}${data.index}`;
    ...
    let p = _.map(data.m3u8.items.PlaylistItem, function(item) {
        that.logger.debug('%s next playlist item to validate %s', flavor_header, data.index, item.properties.uri);
...
HlsChecksumRegressionValidator.prototype.validateSingleFlavor = function (entryId, flavorId, index) {

    let that = this;
    let deferred = Q.defer();
    var flavor_header = logheader1 `${entryId}${flavorId}${index}`;
    ...
    try {
        ...
        return networkClient.read({url: url, timeout: 10000})
            .then(function (content) {
                return that.addChunklist(content, flavorId, index);
            })
            .then(function (m3u8) {
            ...
            .catch(function (errors) {
                return that.exitRegressionWithError(entryId, flavorId, index, errors, url);
            })
            .then(function() {
                deferred.reject();
            })
...
HlsChecksumRegressionValidator.prototype.saveResultsToFile = function (reason) {
