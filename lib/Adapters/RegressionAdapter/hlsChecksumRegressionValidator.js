...
HlsChecksumRegressionValidator.prototype.setControllerWrapper = function(controllerWrapper) {
    this.controller = controllerWrapper;
};

HlsChecksumRegressionValidator.prototype.gracefullyExit = function(error) {
    if (this.controller) {
...
HlsChecksumRegressionValidator.prototype.getSegmentFromPackager = function(item, data) {
    ...
    var options = {
        method: 'HEAD',
        url: `http://localhost:8080/live/hls/p/0/entryId/${this.entryId}/${item.properties.uri}`,
        timeout: 10000};
...
HlsChecksumRegressionValidator.prototype.validateSingleFlavor = function (flavorId, index) {
    ...
    let url = util.format(`http://localhost:8080/live/hls/p/0/entryId/${this.entryId}/index-s${flavorId}.m3u8`);
    ...
    try {
        ...
        return networkClient.read({url: url, timeout: 10000})
            ...
            .catch(function (errors) {
                return that.exitRegressionWithError(this.entryId, flavorId, index, errors, url)
                    .done((errors) => {
                       deferred.reject(errors);
                    });
            });
