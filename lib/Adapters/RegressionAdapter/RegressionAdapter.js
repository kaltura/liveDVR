...
let regression_status = { running: 'running', ending_failed: 'failed', ending_succeeded: 'succeeded', stopped: 'stopped' };
...
function TestHelper(controller) {
...
TestHelper.prototype.gracefullyExit = function(error) {
...
function RegressionAdapter(contoller) {
    ...
    testHelper = new TestHelper(contoller);
    this.initPromise=testHelper.initEntries()
        .then( function() {
            testHelper.logger.info('RegressionAdapter successfully initialized.');
            createServer();
            deferred.resolve();
        })
        .catch( function(err) {
            let err_msg = err ? ErrorUtils.error2string(err) : 'Check log for detailed reason.';
            testHelper.logger.error('RegressionAdapter failed to initialize. %s', err_msg);

            testHelper.gracefullyExit(-3);
        });
