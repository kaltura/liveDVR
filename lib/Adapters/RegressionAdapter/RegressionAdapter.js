            ...
            return that.getLastChunklistFileIndex(entryConfig.entryId, that.entries[entryConfig.entryId].data_warehouse_path)
                ...
                .then(function () {
                    _.each(flavors, function (flavor) {
                        that.entries[entryConfig.entryId].chunklist_index[flavor] = entryConfig.firstChunklistIndex;
                    });
                });
        }).catch(function (err) {
        ...
        .then( function() {
            var p = _.map(that.entries[entryId].flavors, function(flavor) {
                ...
                return networkClient.read({url: url, timeout: 10000})
                    ...
                    .then(function (m3u8) {
                        that.logger.info('[%s-%s] **** SUCCESSFULLY VALIDATED ****  obj=[%s], url=[%s]', entryId, flavor, m3u8, url);
                    })
                    .catch(function (e) {
                        that.logger.error('[%s-%s] **** VALIDATION FAILED!!! **** error=[%s], url=[%s]', entryId, flavor, e.message, url);
                    });
            });
            ...
            return Q.all(p)
                .then(function () {
                    ...
                    if (that.entries[entryId].chunklist_count === that.entries[entryId].last_chunklist_index - that.entries[entryId].initial_chunklist_index + 1) {
                        finished_regression_test = true;
                    }
                })
                .then( function() {
                    if (finished_regression_test) {
                        that.logger.debug('[%s]<< iteration %s >> successfully finished aggregating results .', entryId, that.entries[entryId].chunklist_count);
                        return testHelper.validators[entryId].saveResultsToFile('finished regression test. last chunklist index ' + index)
                            ...
                            .catch(function (err) {
                                that.logger.error('[%s]<< iteration %s >> error aggregating results. Error: %s', entryId, that.entries[entryId].chunklist_count, entryId,ErrorUtils.error2string(err));
                                return testHelper.validators[entryId].saveResultsToFile('validation failed')
                                    .then(function() {
                                        process.exit(-5);
                                    })
                                    .catch( function(err) {
                                        that.logger.error('[%s] error saving regression results of failed test. Error: %s', entryId, entryId,ErrorUtils.error2string(err));
                                        process.exit(-5);
                                    });
                            });
                    }
