...
class HlsAnalysisRegressionValidator extends HlsChecksumRegressionValidator {
    ...
    let url = util.format(`http://localhost:8080/live/hls/p/0/entryId/${this.entryId}/master.m3u8`);
    ...
    try {
        ...
        // python [live-monitor root path]/analyzer/main.py -i http://localhost:8080/live/hls/p/0/1_oorxcge2/playlist.m3u8
...
''
    ...
    saveResultsToFile(reason) {
       ...
       super.saveResultsToFile(reason);
        ...
        var report_fullpath = `${this.analyzer_fullPath}\/reports/hlsAnalyze.json`;
        ...
        .then((content) => {
            ...
            if (json_report.entryReports.abc.hasOwnProperty('entryAlerts')
                ...
                return qio.write(hlsAnalize_copy, content)
                    .then(() => {
                      return qio.write(`${this.latest_regression_path}/hlsAnalyze.json`, content);
                    })
                    .then(() => {
