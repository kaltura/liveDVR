...
HlsStreamRegressionValidator.prototype.addChunklist = function (chunklist, flavorId, index, url) {
    ...
    return m3u8Handler.parseM3U8(chunklist, {'verbatim': true})
            ...
            var p = _.map(m3u8.items, function (playlist) {
                ...
                var tsp = _.map(new_ts_list, function(item) {
                    ...
                    return networkClient.read({url: url, timeout: 10000})
                        ...
                        .catch(function (err) {
                            that.logger.debug('[%s] <<< iteration %s >>> failed to get playlist item %s from packager. Error = %s', that.entryId, index, item, ErrorUtils.error2string(err));
                            deffered.reject(err);
                        });
        ...
        .then(function (value) {
            ...
            that.logger.info('[%s-%s] <<< iteration %s >>> **** SUCCESSFULLY VALIDATED ****  obj=[%s], url=[%s]', that.entryId, index, flavorId, m3u, url);
            deffered.resolve(m3u);
        })
        .catch(function (err) {
            that.logger.error('[%s-%s] <<< iteration %s >>> **** VALIDATION FAILED!!! **** error=[%s], url=[%s]', that.entryId, index, flavorId, ErrorUtils.error2string(err), url);
            deffered.reject();
        });
...
HlsStreamRegressionValidator.prototype.saveResultsToFile = function (reason) {
    ...
    if (this.override || this.regression_results_db === undefined || !test_passed) {
        return qio.write(fullpath, JSON.stringify(this.this_run_results, null, 2))
            .then(function() {
                that.logger.debug('[%s] successfully saved regression test results to %s. Reason: %s', that.entryId, fullpath, reason);
            })
            .catch(function(err) {
                that.logger.error('[%s] error writing regression test results to %s (save reason: %s). Error: %s', that.entryId, fullpath, reason, ErrorUtils.error2string(err));
            });
    }
    else {
        this.logger.debug('[%s] \'override\' is false, not saving regression test results to %s.', that.entryId, fullpath);
        return Q.resolve();
    }
