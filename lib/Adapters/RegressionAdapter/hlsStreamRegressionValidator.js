...
HlsStreamRegressionValidator.prototype.getSegmentFromPackager = function(item, data) {
    ...
    // let url = util.format("http://localhost:8080/kLive/smil:%s_all.smil/%s/%s", 'dfahjl', data.flavor, item.properties.uri);
    ...
    return networkClient.read({url: url, timeout: 10000})
        .then(function (content) {
            ...
            if (!_.has(that.this_run_results[data.flavor].ts[ts_id])) {
                ...
                data.errors[data.flavor].errors[ts_id] = 'playlist item validation failed';
...
HlsStreamRegressionValidator.prototype.downloadFlavorSegments = function(data) {
    ...
    return Q.all(p)
        ...
        .catch(function() {
            return that.validateResults(false, data.errors);
        })
        .catch(function (err) {
            that.logger.debug('%s save regression results failed. Check if it\'s a bug. Error=[%s], url=[%s]', flavor_header, ErrorUtils.error2string(err));
        })
