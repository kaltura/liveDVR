...
/*
  ...
  This class is responsibilities:
   ...
   period.
 */
function HlsStreamRegressionValidator(config, flavors, last_chunklist_index) {
    ...
    // add root to path
    this.db_filename = this.entryId + '_regression_test_db.json';
...
HlsStreamRegressionValidator.prototype.saveResultsToFile = function (reason) {
    ...
    var fullpath = test_passed ? that.fullpath + this.db_filename : that.fullpath + that.entryId + '_all_regression_run_results.json';
...
HlsStreamRegressionValidator.prototype.loadResultsDatabase = function () {
    ...
    var that = this;
    var dbFullPath =  this.fullpath + this.db_filename;
    ...
    return this.createDestinationDir()
        .then( function() {
            ...
            if (that.override === true) {
                ...
                that.logger.debug('Going to open %s. Override=%s', dbFullPath, that.override);

                return qio.read(dbFullPath)
                    .then(function (content) {
                        that.logger.debug('[%s] successfully read file %s.', that.entryId, dbFullPath);
                        that.regression_results_db = JSON.parse(content);
                    })
                    .catch(function (err) {
                        that.logger.info('[%s] failed to open regression tests db file, %s, @@@@ creating new regression db. Error: %s', that.entryId, dbFullPath, ErrorUtils.error2string(err));
                        that.override = true;
                    });
            }
        });
