...
HlsStreamRegressionValidator.prototype.createDestinationDir = function() {
    ...
    return qio.makeTree(getDirName(this.fullpath))
        .then( function() {
...
HlsStreamRegressionValidator.prototype.addChunklist = function (chunklist, flavorId, index, url) {
    ...
    return m3u8Handler.parseM3U8(chunklist, {'verbatim': true})
                    ...
                    return networkClient.read({url: url, timeout: 10000})
                        .then(function (content) {
                            ...
                            if (!_.has(that.this_run_results[flavorId].ts[ts_id])) {
                                ...
                                if (!errors.hasOwnProperty(flavorId)) {
                                    errors[flavorId] ={'errors': {}};
                                }
                                errors[flavorId]['errors'][ts_id] = 'playlist item validation failed';
                                return Q.reject('playlist item validation failed');
                           }
                        })
                        .catch(function (err) {
                            if (!errors.hasOwnProperty(flavorId)) {
                            ...
                            that.logger.debug('[%s-%s] <<< iteration %s >>> failed to get playlist item %s from packager. Error = %s', that.entryId, flavorId, index, item, err_msg);
                            errors[flavorId]['errors'][counter.toString()] = err_msg;
                            counter++;
        ...
        .catch(function (err) {
            let err_msg = ErrorUtils.error2string(err);
            that.logger.error('[%s-%s] <<< iteration %s >>> **** VALIDATION FAILED!!! **** error=[%s], url=[%s]', that.entryId, flavorId, index, err_msg, url);
            deffered.reject(err);
        });
      ...
      /*      .then(function() {
            ...
            .catch(function(err){
                return that.validateResults(false, errors);
            })
            .then(function() {
                return Q.reject();
            });*/
...
HlsStreamRegressionValidator.prototype.saveResultsToFile = function (reason) {
    ...
    if (this.override || this.regression_results_db === undefined || !test_passed) {
        return qio.write(fullpath, JSON.stringify(this.this_run_results, null, 2))
            ...
            .catch(function(err) {
                that.logger.error('[%s] error writing %s to %s (save reason: %s). Path: %s, Error: %s', that.entryId, what, reason, fullpath, hErrorUtils.error2string(err));
            });
    }
    else {
        ...
        .then( function() {
            ...
            if (that.override === true) {
                ...
                return qio.read(that.fullpath)
                    ...
                    .catch(function (err) {
                        that.logger.info('[%s] failed to open regression tests db file, %s, @@@@ creating new regression db. Error: %s', that.entryId, that.fullpath, ErrorUtils.error2string(err));
                        that.override = true;
                        //process.exit(-3);
                    });
            }
