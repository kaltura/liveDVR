...
HlsStreamRegressionValidator.prototype.createDestinationDir = function() {
    ...
    return qio.makeTree(getDirName(this.fullpath))
        ...
        .catch (function(err) {
            that.logger.error('[%s] error creating dir to save regression test results to %s. Error: %s', that.entryId, that.fullpath, ErrorUtils.error2string(err));
        });
}
...
HlsStreamRegressionValidator.prototype.saveResultsToFile = function (reason) {
    ...
    if (this.override || this.regression_results_db === undefined) {
        qio.write(this.fullpath, JSON.stringify(this.this_run_results, null, 2))
            .then(function() {
