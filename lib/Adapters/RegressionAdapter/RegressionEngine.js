...
/*
        this.regression_status = {
            running: 'running',
            ending_failed: 'failed',
            ending_succeeded: 'succeeded',
            stopped: 'stopped'
        };
...
RegressionEngine.prototype.initEntries = function() {
    ...
    var p = _.map(entriesInfo, function (entryConfig) {
        ...
        qio.removeTree(fullpath)
            ...
            .then(function () {
                that.validators[entryConfig.entryId] = new validator(entryConfig, that.entries[entryConfig.entryId].flavors, that.entries[entryConfig.entryId].last_chunklist_index);
            })
            .then(function() {
...
RegressionEngine.prototype.initChunklistEdgeIndexes = function (entryId, fullPath) {
    ...
    pGlob(fullPath + '/*')
        ...
        .then(function() {
            if (that.entries[entryId].last_chunklist_index === that.entries[entryId].initial_chunklist_index) {
                let err = util.format('data warehouse is empty, verify correctness of path (%s). Cannot run regression tests', fullPath);
                that.logger.info(err);
                deferred.reject(err);
            }
...
RegressionEngine.prototype.validateSingleFlavor = function (entryId, flavorId, iteration) {
...
RegressionEngine.prototype.resolveUrl = function(url, params) {
    ...
    try {
        ...
        if (params.filename.indexOf('chunklist.m3u8') > -1 ||
            ...
            weblogger.info('[%s-%s] <<<< iteration %s >>>> next chunklist to get from DATA WAREHOUSE, fullpath=%s', params.entryId, params.flavorId, this.entries[params.entryId].chunklist_index[params.flavorId],  params.fullpath);
