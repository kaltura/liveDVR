...
RegressionEngine.prototype.initEntries = function() {
    ...
    var p = _.map(entriesInfo, function (entryConfig) {
        ...
        qio.removeTree(fullpath)
            ...
            .then(function () {
                that.validators[entryConfig.entryId] = validatorFactory.getValidator(entryConfig, that.entries[entryConfig.entryId].flavors, {start_index: that.entries[entryConfig.entryId].initial_chunklist_index, end_index: that.entries[entryConfig.entryId].last_chunklist_index});
            })
            .then(function() {
...
RegressionEngine.prototype.gracefullyExit = function(error, entryId) {
    ...
    var code = isNaN(error) && error.hasOwnProperty(code) ? error.code : error;
    code = !isNaN(code) ? code : -5;

    if (entryId && error != 0) {
        this.logger.error(`regression ended with error: [${ErrorUtils.error2string(error)}]`);
