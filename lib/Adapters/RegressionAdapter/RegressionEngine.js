...
RegressionEngine.prototype.exitRegressionWithError = function(entryId, flavorId, index, errors, url) {
  ...
  if (!_.has(err_obj, flavorId)) {
    ...
    return this.validators[entryId].validateResults(false, err_obj)
        ...
        .then(function(err) {
            if (!isNaN(err)) {
                that.gracefullyExit(parseInt(err), entryId)
            } else {
                that.gracefullyExit(-5, entryId);
            }
        })
        .then(function() {
            deferred.reject();
    ...
    return Q.all(p_finished_iteration)
        .then( function() {
            that.logger.debug('%s starting validation iteration, getting stream data from \'packager\'', log_header);
                that.validators[entryId].validateAllFlavors(index)
                .then(function () {
                        ...
                        return that.validators[entryId].saveResultsToFile('finished regression test. last chunklist index ' + index)
                            ...
                            .catch(function (err) {
                                ...
                                return that.validators[entryId].saveResultsToFile('validation failed')
                                    ...
                                    .then(function() {
                                        that.gracefullyExit(code, entryId);
                                    })
                                    .catch( function(err) {
                ...
                .catch( function(err) {
                    that.entries[entryId].chunklist_count++;
                    that.logger.error('%s aggregating results returned error, %s.', log_header, ErrorUtils.error2string(err));
                    that.validators[entryId].saveResultsToFile('validation failed');
                    that.gracefullyExit(-9, entryId);
                })
                .then( function() {
...
RegressionEngine.prototype.gracefullyExit = function(error, entryId) {
    if (entryId && error != 0) {
        this.validators[entryId].saveResultsToFile('validation failed');
    }
    _utility.logCommandLineArgs();
    let total_run_time = process.hrtime(this.start_time);
    this.logger.info(`================================================================================`);
    this.logger.info(`@@@ exiting with code [${error}], total run time <<< ${(total_run_time[0] + total_run_time[1]/1e9).toFixed(3)} seconds >>>`);
    this.logger.info(`================================================================================`);
   // this.regressionAdapter.gracefullyExit(error);
    this.regressionAdapter.emit( 'exit', error );
}
