...
const regression_util = require('./utility');
const fs = require('fs');


class hlsAnalysisStreamRegressionValidator extends HlsStreamRegressionValidator {
    ...
    let log_header = regression_util.logHeader `${this.entryId}${this.flavors}${index}`;
    ...
    return analyzerp.promise
        .then(() => {
            // let hlsStreamRegressionValidator do its validation (and prepare new results db)
...
hlsAnalysisStreamRegressionValidator.prototype.verifyNoAlarms = function(index) {
