...
function TestStreamInfo(entryId,flavorParamsIds,wowzaUrl,streamItem) {
    ...
    if (simulateStreams.useNginx) {
        this.streamUrl = "http://localhost:2000/entry/" + this.entryId + "/origin/" + wowzaUrl.slice(7);
    } else {
        this.streamUrl =  wowzaUrl;
            ...
            return m3u8Parser.parseM3U8(content, {'verbatim': true}).then(function (m3u8) {
                ...
                for (var i = 0; i <simulateStreams.count; i++) {
                    result.push({
                        ...
                        "dvrWindow": simulateStreams.dvrWindow,
