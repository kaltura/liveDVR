...
function checkChunklistReturnCode() {
    var baseUrl = getFullLiveUrl(this._baseUrl, this.entryId, this.flavorParamsIds[0]);
...
function mapFlavors() {
    ...
    return _.map(that.flavorParamsIds, function(flavor) {
        var mediaServerBaseUrl = getFullLiveUrl(that._baseUrl,that.entryId, flavor);
        var mediaServerPlaylistUrl = url.resolve(mediaServerBaseUrl, 'playlist_w111.m3u8');
...
function insertFlavorToPlaylist(flavorsObj, flavor, index) {
    var mediaServerBaseUrl = getFullLiveUrl(this._baseUrl, this.entryId, this.flavorParamsIds[index]);
    flavorsObj.push({
...
WowzaStreamInfo.prototype.getAllFlavors = function() {
    ...
    }, 5000, config.get('numOfManifestRetries'), "Could not get manifest");
};
