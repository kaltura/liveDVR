...
const originMode = config.get('mediaServer.originMode');
...
function addFlavor(flavorId, flavorsArray) {
    ...
    //originMode this is a mode of wowza (license), use originMode=false for developer mode.
    let mediaServerChunklistUrl = url.resolve(mediaServerBaseUrl, originMode ? 'chunklist.m3u8' : 'master.m3u8');
    ...
    return networkClient.read(mediaServerChunklistUrl)
        ...
        .then((m3u8) => {
            ...
            if (m3u8 && m3u8.items && m3u8.items.PlaylistItem.length > 0) {
                ...
                if (!originMode && m3u8 && m3u8.items && m3u8.items.StreamItem.length > 0) {
                    let flavorObj = {
                        name: flavorId,
                        liveURL: url.resolve(mediaServerBaseUrl,m3u8.items.StreamItem[0].properties.uri),
                        entryId: this.entryId
                    };
                    flavorsArray.push(flavorObj);
                    this.logger.info("Add flavor %j", flavorObj);
