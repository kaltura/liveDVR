...
function addFlavor(flavorId, flavorsArray) {
    ...
    return networkClient.read(mediaServerChunklistUrl)
        ...
        .catch(err => {
            this.logger.warn(`URL response: ${err}`);
            return Q.reject();
        });
}
...
WowzaStreamInfo.prototype.getAllFlavors = function() {
    ...
    return promUtils.retryPromise(() => {
        ...
        return Q.allSettled(promiseArr)
            .then((results)=> {
                // Check if any of the promises is rejected
                let anyRejected = _.find(results, p => {
                    return p.state === 'rejected';
                });
                return (anyRejected  === undefined) ? Q.resolve() : Q.reject();
            });
