...
function readRequest(requestData, retries = 0)
{
    ...
    return requestPromisify(requestData).then((res)=> {
        ...
        return Q.reject(new Error(util.format("Response for %j returned with status code %d", requestData, res.response.statusCode)));

    }).catch(function(err){
        if (err.hasOwnProperty('retryable') && err.retryable && retries > 0) {
