...
module.exports = function(outputFolderPath, filename, dvrWindowSize, logger){
    ...
    var init = function(){
        return Q.all([intermediateManifest.init(), externalizedManifest.init()]);
    }
    ...
    var update = function updateChunklist(newItems) {
        return intermediateManifest.update(newItems)
            .then(function(){
                ...
                logger.debug("consecutive existing chunks in intermediate manifest: " + JSON.stringify(_.map(consecutiveExistingChunks, function(item){ return item.get('uri');})));
                ...
                var chunksToUpdate = getNewChunksToUpdate(currentExternalizedManifest, consecutiveExistingChunks)
