...
var MP4WriteStream = (function(){
    function MP4WriteStream(chunkPath, tsPath, loggerInfo) {

        var that = this;
        // Todo: Lilach do not commit
        this.startTests = Date.now();

        that.logger = loggerModule.getLogger("MP4WriteStream", loggerInfo);
        that.tsChunkPath = tsPath || persistenceFormat.getTSChunknameFromMP4FileName(chunkPath);
...
var onFinish = function() {
    ...
    try {
        ...
        /* TODO: Lilach remove and do not commit */
        let now = Date.now();
        if (now - that.startTests >= 1800000) {
       tempIndex++;
        let res3 = tempIndex % 3;
        let res4 = tempIndex % 4;
        //let res5 = tempIndex % 5;
        if (res3 === 0  || res4 === 0/*|| res5 === 0*/) {
            throw new Error('TEMP CODE!!! ->> test limit max clips');
        }
        tempIndex = tempIndex % 100;
        } else if (now - that.startTests >= 3600000) {
            that.startTests = Date.now();
        }
        /* End of Lilach do not commit */
    } catch(err){
        onError.call(that,err);
        return;
    }
