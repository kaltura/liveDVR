...
var onError = function (errStr) {
    ...
    Q.allSettled([that.chunkPath ? qio.remove(that.chunkPath) : Q.resolve(),
        that.chunkDownloaded ? qio.write( that.tsChunkPath + ".dump.ts", Buffer.concat(that.chunks)) : Q.resolve()])
        .then((promises) => {
...
var onFinish = function() {
    ...
    try {
        that.chunkPath = path.join(path.dirname(that.tsChunkPath),persistenceFormat.getMP4FileNamefromInfo(that.tsChunkPath));
