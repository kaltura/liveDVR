...
var logger = require('../common/logger/loggerNew').getLogger('BackendClient');
var BackendClient = (function(){
    ...
    function getEntries(list) {
        ...
        client.liveStream.listAction(function(results, err, headers) {
            ...
            if (results.objects) {
                ...
                var objs = _.map(unRejectedEntries,  convertKalturaEntry);
                logger.info("Got live entries: %j (took %d)", objs,operationTime);
                deferred.resolve(objs);
            }
            else {
                printAPIResponse(moduleLogger, results, headers);
                logger.error("Failed to retrieve live entries. Code: %s; Error: %s; took %d", results.code, err,operationTime);
                deferred.reject(new Error("Failed to retrieve live entries from server"));
            }
        }, filter);
    ...
    BackendClient.registerEntryInDatabase = function(entryObject, state, event, logger) {
        ...
        return createSession()
            .then(function() {
                // TODO: Change hard coded 0 value to serverIndex parameter -> Gad
                return registerEntryInDatabase(entryObject.entryId, 0, state, event, logger);
            });
    };
