...
var BackendClient = (function(){
    ...
    function registerEntryInDatabase(entryId, serverIndex, state) {
        var deferred = Q.defer();
        var startTime = new Date();
        ...
        client.liveStream.registerMediaServer(function(result) {
            if (result.objectType === "KalturaLiveStreamEntry") {  //Use to indicate success operation
                ...
                logger.info("Register media server for entry " + result.id + " finished successfully");
            }
            else {
                logger.error("Failed to register media server for entry: " + result.id + ", Error: " + result.message);
    ...
    BackendClient.registerEntryInDatabase = function(entryObject, state) {
        ...
        return createSession()
            .then(function() {
                // TODO: Change hard coded 1 value to serverIndex parameter
                return registerEntryInDatabase(entryObject.entryId, 1, state);
            });
    };
