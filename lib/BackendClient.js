...
var BackendClient = (function(){
    ...
    // todo: revert and add new method createRecordedEntry
    ...
    function updateEntryDuration(entryId, cumulativeDurationMs, recordedEntryId, entryServerNodeId) {
        ...
        entryServerNode.entryId = entryId;
        ...
        client.doMultiRequest((results, err, headers)=> {
            ...
            if (iskMultiRequestSuccessful(results, err, 2)) {
                logger.info(`[${entryId}-${recordedEntryId}] Successfully updated duration to [${durationString}]=[${cumulativeDurationMs} msec], entryServerNodeId:[${entryServerNodeId}]`);
                return deferred.resolve();
            }
            else {
                let errMsg = util.format(`[${entryId}-${recordedEntryId}] Failed to update entry duration to [${durationString}]=[${cumulativeDurationMs} msec], entryServerNodeId:[${entryServerNodeId}] ${ErrorUtils.error2string(err)}. ${results}`);
                logger.error(errMsg);
    ...
    function resetRecordingEntry(entryObj) {
        ...
        client.doMultiRequest((results, err, headers)=> {
            ...
            if (iskMultiRequestSuccessful(results, err, 4)) {
                let newRecordedEntryId = results[3].recordedEntryId;
                logger.info(`[${entryObj.entryId}] Successfully retrieved new recordedEntryId:[${newRecordedEntryId}]`);
    ...
    BackendClient.getSingleEntryServerNode = function(entryId, filter) {
        return getLiveEntryServerNodes(entryId, filter)
            .then((liveEntryServerNodes)=> {
                if (liveEntryServerNodes.length > 0) {
                    ...
                    return Q.resolve(liveEntryServerNodes[0]);
                }
                else {
                    logger.error(`[${entryId}] failed to get entry server node`);
                    Q.reject(`Couldn't find entryServerNode Id, of DC provisioning live entry:[${entryId}]`);
                }
            });
    };
