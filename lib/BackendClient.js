...
var BackendClient = (function(){
    ...
    var createSession = function () {
        ...
        client.session.start(function(ks, err){
            ...
            logger.debug("Request for createSession (started at " + startTime + ") took " + operationTime);
            if (err){
            ...
            if (results) {
                var objs = _.map(results.objects, function (item) {
                    ...
                    if (item.dvrStatus === kalturaTypes.KalturaDVRStatus.ENABLED) {
                        var dvrWindowPartner=item.dvrWindow*60;     //   assuming dvrWindow from backend is in minutes!
                        if (dvrWindowPartner && dvrWindowPartner> config.get("dvrTimeWindowThreshold"))
                        {
                           res.dvrWindow = dvrWindowPartner;
                        }
                        else
                        {
                            res.dvrWindow = config.get("dvrTimeWindow");
                        }
