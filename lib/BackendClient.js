...
var BackendClient = (function(){
    ...
    function extractFlavorsExtraParams(flavorsObjectList, entryFlavors) {
        let liveFlavors = _.map(entryFlavors, (f)=> {
           let flavorParams = _.find(flavorsObjectList, (obj)=> {
               return (obj.id.toString() == f && obj.objectType == "KalturaLiveParams")
           });
           return {
               id : flavorParams.id,
               extraParams : flavorParams.conversionEnginesExtraParams
           }
        });
    ...
    function getFlavorsExtraParams(entryObject, flavors, filter = new kalturaVO.KalturaEntryServerNodeFilter()) {
        ...
        let flavorParamsFilter = filter;
        flavorParamsFilter.entryIdEqual = entryObject.entryId;
        client.setPartnerId(partnerId);
        client.flavorParams.listAction((result, error, headers)=> {
            printAPIResponse(result, headers);
            if (error || result.objectType === 'KalturaAPIException') {
                logger.debug("[%s] Failed to retrieve extra params for flavor [%s]", entryObject.entryId, flavor);
                deferred.reject();
            }
            let eParamsObject = extractFlavorsExtraParams(result.objects, flavors);
            deferred.resolve(eParamsObject);
        }, flavorParamsFilter);
    ...
    BackendClient.getFlavorsExtraParams = function(entryObject, flavors) {
        logger.debug("[%s] Entered getFlavorsExtraParams. Creating session and retrieving live flavors extra params", entryObject.entryId);
        return createSession()
            .then(()=> {
                return getFlavorsExtraParams(entryObject, flavors);
            })
    };
