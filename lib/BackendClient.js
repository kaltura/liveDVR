...
var BackendClient = (function(){
    ...
    function convertKalturaEntry(item){
        var res = {
            ...
            "explicitLive" : item.explicitLive,
            "viewMode" : item.viewMode,
            "reocrindgStatus" : item.recordingStatus
        };
    ...
    function registerToPush(templateSystemName, partnerIdToRegister, entryId)
    {
        const deferred = Q.defer();
        const pushNotificationsParams = {objectType: 'KalturaPushNotificationParams'};

        client.setPartnerId(partnerIdToRegister);
        const userParams = {'userParams':[{key:'entryId',value: {objectType:'KalturaStringValue', 'value': entryId}}]};
        client.eventNotificationTemplate.register((result, err, headers)=> {
            printAPIResponse(result, headers);
            if(err || result.objectType === 'KalturaAPIException') {
                logger.debug(`operation permission.get failed. Error: ${ErrorUtils.error2string(err)}`);
                deferred.reject();
            }
            else {
    ...
    BackendClient.registerToPush = (templateSystemName, partnerIdToRegister, entryId) => {
        return createSession().
            then(() => {
               return registerToPush(templateSystemName, partnerIdToRegister, entryId);
            });
    };
