...
var BackendClient = (function(){
    ...
    function getLiveEntry(entryId) {
        let deferred = Q.defer();

        client.liveStream.get((result, err, headers)=> {
            printAPIResponse(result, headers);
            if (err || result.objectType === 'KalturaAPIException') {
                logger.debug(`[${entryId}] Failed to retrieve entryId from server. Error: ${ErrorUtils.error2string(err)}`);
                deferred.reject();
            }
    ...
    BackendClient.getLiveEntry = function(entryId) {
        logger.debug(`[${entryId}] Entered getLiveEntry. Creating session and calling liveEntry.get`);
        return createSession()
            .then(()=> {
                return getLiveEntry(entryId);
            });
    };
