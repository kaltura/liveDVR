...
var BackendClient = (function(){
    ...
    function getEntryInfo(entryId, mediaServerIndex) {
        ...
        //get entryServerNode information to identify if current server is primary or backup
        var entryServerNodeFilter = new kalturaVO.KalturaLiveEntryServerNodeFilter();
        entryServerNodeFilter.entryIdIn = entryId;
        entryServerNodeFilter.serverNodeIdEqual = mediaServerIndex;
        client.entryServerNode.listAction(null, entryServerNodeFilter);

        var startTime = new Date();
        client.doMultiRequest(function(results, err, header) {
            var endTime = new Date();
            var operationTime = endTime - startTime;

            if(!err && results.length > 0 && results[0].objectType !== 'KalturaAPIException' && results[1].objectType !== 'KalturaAPIException' ) {
                moduleLogger.info("Got server node info for entry %s response: %j (took %d)", entryId, results, operationTime);
                var liveEntry = convertKalturaEntry(results[0]);
                liveEntry.serverType = results[1].objects[0].serverType;
                deferred.resolve(liveEntry);
            }
            else {
                printAPIResponse(moduleLogger, results, header);
                moduleLogger.error("Failed to retrieve server node info for from server: %s", results[0].message);
                deferred.reject(new Error("%s: %s", results[0].objectType, results[0].code));
            }
        });
    ...
    BackendClient.getEntries = function(entryId) {
        return createSession()
            .then(function () {
                return getMediaServerIndex().then(function(index) {
