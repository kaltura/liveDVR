...
var BackendClient = (function(){
    ...
    function printAPIResponse(results, headers, msgId = 'xxx') {
        logger.info("[msgId %s] API call headers: %j", msgId, headers);
        if(results)
            logger.info("[msgId %s] API call result: %j", msgId, results);
    }
    ...
    function registerEntryInDatabase(entryObject, state, event, msgId) {
        var deferred = Q.defer();
        logger.debug("[%s][msgId %d] Calling registerMediaServer. Hostname: [%s], serverIndex: [%s], Application: [%s], state: [%s]", entryObject.entryId, msgId, mediaServerHostname, entryObject.serverType, applicationName, state);
        ...
        client.liveStream.registerMediaServer(function(result, err, headers) {
            printAPIResponse(result, headers, msgId);
            if (result && result.objectType === "KalturaLiveStreamEntry") {  //Use to indicate success operation
                logger.info("[%s][msgId %d] Register media server finished successfully - '%s'", entryObject.entryId, msgId, event.toUpperCase());
                deferred.resolve(result);   //check which object, if any, need to pass.
            }
            else {
                var msg=util.format("[%s][msgId %d] Failed to register media from server: %s %j", entryObject.entryId, msgId, ErrorUtils.error2string(err), result);
                logger.error(msg);
                deferred.reject(new Error(msg));
            }
    ...
    function unregisterEntryInDatabase(entryId, serverIndex, msgId) {
        var deferred = Q.defer();
        logger.debug("[%s][msgId %s] Calling unregisterMediaServer. Hostname: [%s], serverIndex: [%s], Application: [%s]", entryId, msgId, mediaServerHostname, serverIndex, applicationName);
        client.liveStream.unregisterMediaServer(function(result, err, headers) {
            printAPIResponse(result, headers, msgId);
            if (result && result.objectType === "KalturaLiveStreamEntry") {  //Use to indicate success operation
                logger.info("[%s][msgId %s] Unregister media server finished successfully", entryId, msgId);
                deferred.resolve(result);   //check which object, if any, need to pass.
            }
            else {
                var msg=util.format("[%s][msgId %s] Failed to unregister media from server: %s %j", entryId, msgId, ErrorUtils.error2string(err), result);
                logger.error(msg);
                deferred.reject(new Error(msg));
            }
    ...
    function updateStreamInfo(entryId, entryServerNodeId, liveEntryServerNode, msgId)
    {
        var deferred = Q.defer();
        logger.debug("[%s][msgId %s] Calling updateStreamInfo for entryServerNode id [%s], with stream info array [%j]", entryId, msgId, entryServerNodeId, liveEntryServerNode.streams);
        client.entryServerNode.update(function(result, err, headers) {
            printAPIResponse(result, headers, msgId);
            if(err || result.objectType === 'KalturaAPIException') {
                logger.debug("[%s][msgId %s] Failed to updateStreamInfo for entryServerNode id [%s] with error [%s]", entryId, msgId, entryServerNodeId,  ErrorUtils.error2string(err));
                deferred.reject();
            }
            deferred.resolve();
    ...
    function getLiveEntryServerNodes(entryId, filter, msgId) {
        var deferred = Q.defer();
        logger.debug("[%s][msgId %s] Retrieve entry server node list", entryId, msgId);
        ...
        createSession()
            .then(function () {
                client.entryServerNode.listAction(function (result, err, headers) {
                    ...
                    printAPIResponse(result, headers, msgId);
                    if (result && result.objectType !== "KalturaAPIException" && result.objects) {  //Use to indicate success operation
                        logger.info("[%s][msgId %s] entry server node list successfully retrieved [%j]", entryId, msgId, result.objects);

                        deferred.resolve(result.objects);
                    }
                    else {
                        var msg = util.format("[%s][msgId %s] Failed to retrieve entry server node list: %s %j", entryId, msgId, ErrorUtils.error2string(err), result);
                        logger.error(msg);
                        deferred.reject(new Error(msg));
                    }
    ...
    BackendClient.isEntryLive = function(entryId) {
        ...
        filter.isLive = kalturaTypes.KalturaNullableBoolean.TRUE_VALUE;
        filter.statusIn = '1,2';
        return getLiveEntryServerNodes(defaultMsgId, entryId, filter)
            .then((serverObjs) => {
    ...
    BackendClient.getLiveEntryServerNodes = function(msgId, entryId, filter) {
        return createSession()
            .then(function() {
                return getLiveEntryServerNodes(msgId, entryId, filter);
            });
    };
    ...
    BackendClient.registerEntryInDatabase = function(entryObject, state, event, msgId) {
        logger.debug("[%s][msgId %s] Entered registerEntryInDatabase. Creating session and calling register", entryObject.entryId, msgId);
        return createSession()
            .then(function() {
                return registerEntryInDatabase(entryObject, state, event, msgId);
            });
    };

    BackendClient.unregisterEntryInDatabase = function(entryObject, msgId) {
        logger.debug("[%s][msgId %s] Entered unregisterEntryInDatabase. Creating session and calling unregister", entryObject.entryId, msgId);
        return createSession()
            .then(function() {
                return unregisterEntryInDatabase(entryObject.entryId, entryObject.serverType, msgId);
            });
    };
    ...
    BackendClient.updateStreamInfo = function(entryId, entryServerNodeId, flavorsObjArray, msgId) {
        logger.debug("[msgId %s] Calling updateStreamInfo, with new flavors object array", msgId);
        ...
        _.each(flavorsObjArray, function(flavorInfo) {
            ...
            KalturaLiveStreamParams.bitrate = flavorInfo.mediaInfo.bitrate_kbps * 1000; // Convert to bit/second
            ...
            KalturaLiveStreamParams.frameRate = flavorInfo.mediaInfo.framesPerSecond;
        ...
        return createSession()
            .then(function() {
                return updateStreamInfo(entryId, entryServerNodeId, liveEntryServerNode, msgId);
            });
    };
