...
var BackendClient = (function(){
    ...
    function updateIsLiveUser(entryId, isLiveUser)
    {
        var KalturaLiveStreamEntry = new kalturaVO.KalturaLiveStreamEntry();
        var deferred = Q.defer();
        KalturaLiveStreamEntry.isLiveUser = isLiveUser;
        client.liveStream.update( function(result, err, headers) {
            printAPIResponse(result, headers);
            if(err || result.objectType === 'KalturaAPIException') {
                logger.debug("[%s] Failed to update is-live-user for [%s] with error [%s]", entryId,  ErrorUtils.error2string(err));
                deferred.reject();
            }
            else {
                deferred.resolve();
        ...
        }, entryId, KalturaLiveStreamEntry);
        return deferred.promise;
    }
    ...
    BackendClient.updateIsLiveUser = (entryId, isLiveUser) => {
        return createSession().
            then(() => {
               return updateIsLiveUser(entryId, isLiveUser);
            });
    }
