...
var BackendClient = (function(){
    ...
    function registerEntryInDatabase(entryObj, state, event) {
        var deferred = Q.defer();
        client.startMultiRequest();

        logger.debug(`[${entryObj.entryId}] Calling registerMediaServer. Hostname: [${mediaServerHostname}], serverIndex: [${entryObj.serverType}], Application: [${applicationName}], state: [${state}]`);
        ...
        client.liveStream.registerMediaServer(null, entryObj.entryId, mediaServerHostname, entryObj.serverType, applicationName, state);
        client.liveStream.createRecordedEntry(null, entryObj.entryId, entryObj.serverType, state);

        client.doMultiRequest((results, err, headers)=> {
            printAPIResponse(results, headers);

            if (iskMultiRequestSuccessful(results, err, 2)) {
                logger.info(`[${entryObj.entryId}] Register media server finished successfully - '${event.toUpperCase()}'`);
                deferred.resolve(results[1]);
            }
            else {
                var errMsg=util.format(`[${entryObj.entryId}] Failed to register media from server: ${ErrorUtils.error2string(err)} ${results}`);
                logger.error(errMsg);
    ...
    function updateStreamInfo(entryId, serverNodeId, liveEntryServerNode) {
        var deferred = Q.defer();
        logger.debug("[%s] Calling updateStreamInfo for entryServerNode id [%s], with stream info array [%j]", entryId, serverNodeId, liveEntryServerNode.streams);
        client.entryServerNode.update(function(result, err, headers) {
            printAPIResponse(result, headers);
            if(err || result.objectType === 'KalturaAPIException') {
                logger.debug("[%s] Failed to updateStreamInfo for entryServerNode id [%s] with error [%s]", entryId, serverNodeId,  ErrorUtils.error2string(err));
                deferred.reject();
            }
            deferred.resolve();
            
        }, serverNodeId, liveEntryServerNode);
    ...
    function updateEntryDuration(entryId, cumulativeDurationMs, recordedEntryId, serverNodeId) {
        let deferred = Q.defer();
        var mediaEntry = new kalturaVO.KalturaMediaEntry();
        mediaEntry.msDuration = cumulativeDurationMs;
        var serverEntryNode = new kalturaVO.KalturaLiveEntryServerNode();
        serverEntryNode.entryId = entryId;
        ...
        let recordedInfoArray = [];
        recordedInfoArray.push(recordingInfo);
        serverEntryNode.recordedInfo = recordedInfoArray;
        let durationMinutes = (cumulativeDurationMs/60000).toFixed(2);

        client.startMultiRequest();
        client.entryServerNode.update(null, serverNodeId, serverEntryNode);
        client.liveStream.update(null, entryId, mediaEntry);
        client.doMultiRequest((results, err, headers)=> {
            printAPIResponse(results, headers);

            if (iskMultiRequestSuccessful(results, err, 2)) {
                logger.info(`[${entryId}] Successfully updated duration to [${durationMinutes}] minutes, liveEntry:[${entryId}], recordedEntry:[${recordedEntryId}] serverNodeId:[${serverNodeId}]`);
                return deferred.resolve();
            }
            else {
                let errMsg = util.format(`[${entryId}] Failed to update entry duration to [${durationMinutes}] minutes, liveEntry:[${entryId}], recordedEntry:[${recordedEntryId}] serverNodeId:[${serverNodeId}] ${ErrorUtils.error2string(err)}. ${results}`);
                logger.error(errMsg);
    ...
    function resetRecordingEntry(entryObj) {
        ...
        client.doMultiRequest((results, err, headers)=> {
            ...
            if (iskMultiRequestSuccessful(results, err, 4)) {
                logger.info(`[${entryObj.entryId}] Successfully retrieved new recordedEntryId`);
                deferred.resolve(results[3].recordedEntryId);
            }
            else {
    ...
    BackendClient.updateStreamInfo = function(entryId, serverNodeId, flavorsDownloaders) {
        ...
        return createSession()
            .then(function() {
                return updateStreamInfo(entryId, serverNodeId, liveEntryServerNode);
            });
    };
