...
var BackendClient = (function(){
    ...
    function getLiveEntryServerNodeId(entryId, serverType) {
        var deferred = Q.defer();
        ...
        client.entryServerNode.listAction(function(result, err, headers) {
            ...
            if (result.objectType !== "KalturaAPIException") {  //Use to indicate success operation
                ...
                _.each(result.objects, function(liveEntryServerNode) {
                    if(liveEntryServerNode.serverType == serverType) {
                        deferred.resolve(liveEntryServerNode.id);
                    }
                });

                logger.error("[%s] Failed to locate entry server node id for server type [%s]", entryId, serverType);
    ...
    BackendClient.getLiveEntryServerNodeId = function(entryId, serverType) {
        return createSession()
            .then(function() {
                return getLiveEntryServerNodeId(entryId, serverType);
            });
    };
