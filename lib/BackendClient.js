...
var BackendClient = (function(){
    ...
    function executeMultiRequest( entryId ) {
        var deferred = Q.defer();
        var startTime = new Date();
    ...
    BackendClient.resetRecordingEntry = function(entryObj, state) {

        client.startMultiRequest();

        var kalturaLiveEntry = new kalturaVO.KalturaLiveEntry();
        kalturaLiveEntry.recordedEntryId = null;
        kalturaLiveEntry.redirectEntryId = null;
        // (1) reset current recording entry
        client.liveStream.update(null, entryObj.entryId, kalturaLiveEntry, state);
        // (2) register entry and get new recorded entry
        ...
        return executeMultiRequest(entryObj.entryId)
            .then((newEntryObj) => {
                entryObj.recordedEntryId = newEntryObj.recordedEntryId;
                // ??? what is it used for???
                entryObj.recordedEntryId = newEntryObj.redirectEntryId;
                return Q.resolve();
            });
