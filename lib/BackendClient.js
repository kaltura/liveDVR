...
var BackendClient = (function(){
    ...
    function getFlavorExtraParams(entryObject, flavor) {
        let deferred = Q.defer();
        logger.debug("[%s] Calling flavorParamsGetAction. Hostname: [%s], serverIndex: [%s], Application: [%s]", entryObject.entryId, mediaServerHostname, entryObject.serverType, applicationName);

        //Impersonate call to the entry's partnerId to execute the call in the partner's context
        let partnerId = entryObject.partnerId;
        client.setPartnerId(partnerId);
        client.flavorParams.get(function(result, error, headers) {
            printAPIResponse(result, headers);
            ...
            let eParams = result.conversionEnginesExtraParams;
            deferred.resolve(eParams);
        }, flavor);
    ...
    BackendClient.getFlavorExtraParams = function(entryObject, flavor) {
        return createSession()
            .then(() => {
                return getFlavorExtraParams(entryObject, flavor);
            });
    };
