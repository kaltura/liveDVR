...
var BackendClient = (function(){
    ...
    function iskMultiRequestSuccessful(results, err) {
        return (!err && results && _.isArray(results) && results.length===2 && results[0] && results[1] &&
        (results[0].objectType !== "KalturaAPIException" || results[1].objectType !== "KalturaAPIException"));
    }
    ...
    function getEntryInfo(entryId, serverNodeId) {
        ...
        client.doMultiRequest(function(results, err, headers) {
            ...
            if (iskMultiRequestSuccessful(results, err)) {  //Use to indicate success operation
    ...
    function updateEntryDuration(entryId, cumulativeDurationMs) {
        ...
        KalturaMediaEntry.recordedEntryId = null;
    ...
    function resetRecordingEntry(entryObj, duration) {
        ...
        client.liveStream.update(null, KalturaMediaEntry);
        client.entryServerNode.listAction(null, EntryServerNodeFilter);
        // TODO: Check with Yossi again how to retrieve that parameter
        ...
        client.doMultiRequest((results, err, headers)=>{
            ...
            if (iskMultiRequestSuccessful(results, err)) {
    ...
    BackendClient.resetRecordingEntry = function(entryObj, duration) {
        logger.debug(`[${entryObj.entryId}] Recording reached max duration, reset recordedEntryId`);
        return createSession()
            .then(()=> {
                return resetRecordingEntry(entryObj, duration);
            });
    };
