...
var BackendClient = (function(){
    ...
    function printResponseHeaders(logger, headers) {
        logger.info("API call headers: %j", headers);
    }
    ...
    function getEntryInfo(entryId, mediaServerIndex) {
        ...
        client.liveStream.get(function(result, err, headers) {
            printResponseHeaders(moduleLogger, headers);
            var endTime = new Date();
            var operationTime = endTime - startTime;

            if (result.objectType !== "KalturaAPIException") {  //Use to indicate success operation
                moduleLogger.info("Got server node info for entry %s response: %j (took %d)", entryId, result, operationTime);
                printResponseHeaders(moduleLogger, headers);
                var liveEntry = convertKalturaEntry(result);

                if(result.primaryServerNodeId === mediaServerIndex)
