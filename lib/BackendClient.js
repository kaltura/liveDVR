...
var BackendClient = (function(){
    ...
    function convertKalturaEntry(item){
        ...
        if (item.recordStatus !==kalturaTypes.KalturaRecordStatus.DISABLED){ //todo API shuld addd enable for recoridng
            res.liveSessionExpirationDuration = 180000;
            //res.liveSessionExpirationDuration = item.liveSessionExpirationDuration;
        }
    ...
    function registerEntryInDatabase(entryId, serverIndex, state, event) {
        ...
        client.liveStream.registerMediaServer(function(result, err, headers) {
            ...
            if (result && result.objectType === "KalturaLiveStreamEntry") {  //Use to indicate success operation
                ...
                var msg=util.format("[%s] Failed to register media from server: %s %j", entryId,  ErrorUtils.error2string(err), result);
                logger.error(msg);
                deferred.reject(new Error(msg));
            }
