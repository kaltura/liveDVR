...
var BackendClient = (function(){
    ...
    function parseServerEntryInfo(serverNodeId, apiCallResults) {
        ...
        liveEntryObject.serverNodeId = serverNodeId;
    ...
    function registerEntryInDatabase(entryObj, state, event) {
        ...
        client.doMultiRequest((results, err, headers)=> {
            ...
            if (iskMultiRequestSuccessful(results, err, 2)) {
                ...
                var errMsg=util.format(`[${entryObj.entryId}] Failed to register media from server: ${ErrorUtils.error2string(err)} ${result}`);
                logger.error(errMsg);
    ...
    function updateEntryDuration(entryObj, cumulativeDurationMs) {
        var deferred = Q.defer();
        var KalturaMediaEntry = new kalturaVO.KalturaMediaEntry();
        KalturaMediaEntry.msDuration = cumulativeDurationMs;
        var kalturaLiveEntryServerNode = new kalturaVO.KalturaLiveEntryServerNode();
        let recordingInfo = new KalturaLiveEntryServerNodeRecordingInfo();
        recordingInfo.recordedEntryId = entryObj.recordedEntryId;
        recordingInfo.duration = cumulativeDurationMs;
        let recordedInfoArray = [recordingInfo];
        kalturaLiveEntryServerNode.recordedInfo = recordedInfoArray;
        let durationMinutes = (cumulativeDurationMs/60000).toFixed(2);

        client.startMultiRequest();
        client.entryServerNode.update(null, entryObj.entryServerNodeId, kalturaLiveEntryServerNode);
        client.liveStream.update(null, entryObj.entryId, KalturaMediaEntry);
        client.doMultiRequest((results, err, headers)=> {
            printAPIResponse(results, headers);

            if (iskMultiRequestSuccessful(results, err, 2)) {
                logger.info(`[${entryObj.entryId}] Successfully updated duration to [${durationMinutes}] minutes, liveEntry:[${entryObj.entryId}], recordedEntry:[${entryObj.recordedEntryId}] serverNodeId:[${entryObj.entryServerNodeId}]`);
                deferred.resolve();
            }
            else {
                let errMsg = util.format(`[${entryObj.entryId}] Failed to update entry duration to [${durationMinutes}] minutes, liveEntry:[${entryObj.entryId}], recordedEntry:[${entryObj.recordedEntryId}] serverNodeId:[${entryObj.entryServerNodeId}] ${ErrorUtils.error2string(err)}. ${results}`);
                logger.error(errMsg);
                deferred.reject(new Error(errMsg));
            }
        });
