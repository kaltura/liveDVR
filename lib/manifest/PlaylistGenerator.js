...
function PlaylistGenerator (entryObj,logger){
    ...
    that.logger = logger;
    that.playListLimits =  {
        manifestTimeWindow: entryObj.dvrWindow,
        maxClipCountPerPlaylist:Math.max(10,entryObj.maxChunkCount),
    ...
    that.playlistPath = path.join(persistenceFormat.getEntryFullPath(entryObj.entryId),persistenceFormat.getMasterManifestName());

    // according to https://github.com/kaltura/nginx-vod-module
    that.playlistImp = new PlaylistUtils.Playlist();

    that.addNewClip = that.addNewClip.bind(that);

    that.handleEvents = that.handleEvents.bind(that);
...
PlaylistGenerator.prototype.checkIsStopped = function checkIsStopped(){
    return this.stopPromise && this.stopPromise.promise.state === 'fulfilled';
};

PlaylistGenerator.prototype.getClipFromFileInfo = function getClipFromFileInfo(fileInfo) {
    var that = this;
    ...
    if( !lastClip/*
        ...
        newClip.addListener(that.handleEvents);
        clips.push(newClip);
    }
    return clips[clips.length - 1];
};

PlaylistGenerator.prototype.addNewClip = function addNewClip (fileInfo){
    var that = this;

    if(that.checkIsStopped()) {
        fs.unlink(fileInfo.path,empty);
        throw new Error('Playlist is stopped');
    }

    var lastClip = that.getClipFromFileInfo(fileInfo);
    ...
    lastClip.insert(fileInfo,expires,that.logger);
...
PlaylistGenerator.prototype.checkRemoveChunks = function(flavor){
    var that = this;
...
PlaylistGenerator.prototype.update = function update (playlist) {
    ...
    var removedChunks = [];
    playlist.forEach(that.addNewClip);

    if(playlist.length) {
        removedChunks.concat(that.checkRemoveChunks(playlist[0].flavor));
    }
    return [that.persistPlayList(), Q.resolve({removedChunks: removedChunks})];
};
...
PlaylistGenerator.prototype.persistPlayList = function persistPlayList(force){
    var that = this;
    ...
    if(that.isDirty || force) {
        ...
        if(that.persistDef){
            return that.persistDef.promise
                .then( function(){
                    return that.persistPlayList(force);
                });
        }
...
PlaylistGenerator.prototype.finaizeStart = function (){
    var that = this;
    ...
    that.playlistImp.addListener(that.handleEvents);
    ...
    try {
        ...
        fs.exists(that.playlistPath,function(ok) {
            ...
            if(!ok || config.get('dontInitializePlaylist')){
                that.persistPlayList()
                    .then(that.finaizeStart.bind(that))
            ...
            that.playlistImp.serializeFrom(rs,function(pl){
                that.playlistImp = pl;
                that.persistPlayList().finally(that.finaizeStart.bind(that));
            });
        });
...
PlaylistGenerator.prototype.stop = function stop (){
    ...
    if(!that.checkIsStopped()){
        that.startPromise = null;
        return that.persistPlayList(true).then( that.stopPromise.resolve );
    }
    return that.stopPromise.promise;
};

PlaylistGenerator.prototype.handleEvents = function (type,arg) {
    var that = this;
...
PlaylistGenerator.prototype.that.getChunkCount = function(flavorId){
    var that = this;
    ...
    return that.playlistImp.sequences[seqN].clips.reduce( function(c,val){
