...
PlaylistGenerator.prototype.remove = function(){
    var that = this;
    return (that.stopPromise.promise ? that.stopPromise.promise : qResolved)
        .then( function () {
            return (that.persistPromise ? that.persistPromise : qResolved)
        })
        .then(function () {
...
var persistPlayList = function persistPlayList(force){
    ...
    if(that.isDirty || force) {
        ...
        that.isDirty = false;
        try {
            var str = JSON.stringify(that.playlistImp);
            return that.persistPromise = fsUtils.writeFileAtomically(that.playlistPath,JSON.stringify(that.playlistImp));
        } catch(err){
