        ...
        var singleFileAlignmentPromise =
            qio.read(path.join(that.folderPath, filenameToProcess), "b") // Read binary data into a buffer
                .then(function(dataBuffer) {
                    tha
                    t.logger.debug("Old PTS alignment context: %j" , that.context);
                    var chunkDuration = nativePtsAligner.rebaseTs(that.context, dataBuffer, that.universalTimestamp, this.correctionThreshold);
