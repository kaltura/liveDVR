...
module.exports = function(outputFolderPath, filename, dvrWindowSize){
    ...
    var update = function updateChunklist(newItems) {
        ...
        return Q.fcall(function()
        {
            ...
            var manifestLongestChunk = _.max(currentManifest.items.PlaylistItem, function(item) {
                return item.get('duration');
            });

            if (manifestLongestChunk) {
                currentManifest.set('targetDuration', Math.ceil(manifestLongestChunk.get('duration')));
            }
