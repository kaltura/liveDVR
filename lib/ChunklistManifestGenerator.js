...
module.exports = function(outputFolderPath, filename, dvrWindowSize){
    ...
    var update = function updateChunklist(newItems) {
        ...
        return Q.fcall(function()
        {
            ...
            var manifestLongestChunkDuration = _.max(_.map(currentManifest.items.PlaylistItem, function(item) {
                return item.get('duration');
            }));

            if (manifestLongestChunkDuration > 0) { // Some chunk exists
                currentManifest.set('targetDuration', Math.ceil(manifestLongestChunkDuration));
            }
        }).then(function(){
