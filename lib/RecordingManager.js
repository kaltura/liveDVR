    ...
    constructor() {
        ...
        const startRecording =  (entryObject, flavorId) => {
            ...
            if (!recordingList[entryId]) { // if this flavor is the first todo check that no race between flavor
                ...
                /*
                let lastTimeUpdateChunk = recordingList[entryId].lastTimeUpdateChunk
                let options = {
                    'isNewSession' : true,
                    'isRecording' : true,
                    'recordingPath': recordingPath
                ...
                createManifestPromise= playlistGenerator.createManifest();
                */
            }
            let recordingFlavorPath = path.join(recordingPath, flavorId);
            return qio.makeTree(recordingFlavorPath)
                .then( ()=> {
