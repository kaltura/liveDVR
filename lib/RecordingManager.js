...
class RecordingManager {
    ...
    constructor() {
        ...
        const LinkAndUpdateJson = (playlistGenerator, Mp4Files, recordingSessionPath, recordingSessionId) => {
            var promises = _.map(Mp4Files, function (Mp4File, index) {
                ...
                return qio.link(SourcefilePath, DestFilePath)
                    .then(()=> { //todo should verify that if we links a then b, then the callback is also be at the same order, so it will add to json with the same order
                        ...
                        logger.debug("[%s] Linked %s to %s", recordingSessionId, chunkName, DestFilePath)
                        return Q.resolve(Mp4File)
                    }, (err)=> {
                        if (err.code == 'EEXIST') { // if the error is the file exist...
                            logger.warn("[%s] %s", recordingSessionId, err.message)
                            return Q.resolve()
                        }
                        else { //Otherwise return reject promise!
                            logger.error("[%s] %s ", recordingSessionId, ErrorUtils.error2string(err))
                            return Q.reject(ErrorUtils.error2string(err))
                        }
                    })
            });

            return Q.allSettled(promises).then(function (results) { //todo check why no error trow when try to acess non existing elemnt in promise
            ...
            .then(function (Mp4FilesUpdated) {
                ...
                logger.debug("[%s] About to update chunks: %s", recordingSessionId, chunksNameChained)
                return playlistGenerator.update(Mp4FilesUpdated);
            })
            .catch(function (err) {
                logger.error("[%s] Error occurred while try to add new chunks: %s", recordingSessionId, ErrorUtils.error2string(err));
                return Q.reject(err);
            })
        ...
        const startRecording = (entryObj, flavorId) => {
            ...
            if (!recordingList[entryId]) { // if this flavor is the first todo check that no race between flavor
                ...
                //recordingList[entryId] = new Recording();
                recordingList[entryId] = {
                    ...
                    "recordingSessionPath": recordingSessionPath,
                    "duration": 0, //todo consider remove this fields, since it in playlistGenerator
                    "flavors" : []
                }
            ...
            //              logger.info("[%s] delay ron ",flavorId);
        ...
        const addNewChunks = (Mp4Files, entryId, flavorId, duration) => {
            ...
            //recordingList[entryId].addChunk();
            ...
            return LinkAndUpdateJson(recordingList[entryId].playlistGenerator, Mp4Files, recordingList[entryId].recordingSessionPath, entryId + '-' + recordedEntryId)
        }
            ...
            try {
                ...
                _.each(recordingList, function (element, entryId) {
                    ...
                    if (now - element.lastTimeUpdateChunk > element.recordingSessionDuration) {
                        ...
                        if (source == -1) {
                            logger.error("Error while trying to find source stream of %s from flavor list %s" ,entryId,element.flavors)
                            return
                        }
        ...
        const onStartUp = ()=> { // onsttartup  finish before open new entry downloaders!
            ...
            const checkFilesFromLiveFolder = (liveEntryPath, recordingSessionPath, recordingSessionId, playlistGenerator) => {
                ...
                return pGlob(path.join(liveEntryPath, '*/*/*.mp4')).then((downloadedChunks)=> { //todo should verify that pglobe is giving the chunks in right order
                    ...
                    if (missingChunks.length == 0) {
                        ...
                        return getMetaData(missingChunks, recordingSessionId)
                            .then((Mp4Files)=> {
                                return LinkAndUpdateJson(playlistGenerator, Mp4Files, recordingSessionPath, recordingSessionId)
                            })
                    }
                })
            }
            ...
            const getMetaData = (missingChunks, recordingSessionId)=> {
                ...
                return Q.allSettled(updateChunksPromises).then((results)=> {
                    ...
                    if (errs.numErrors > 0) {
                        logger.error("[%s] checkAndAddFileToJson: Failed to extract matadata for the chunks %s: %s", recordingSessionId, missingChunks, errs.err.message)
