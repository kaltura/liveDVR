...
class RecordingManager {
    ...
    constructor() {
        ...
        const onStartUp = ()=> { // onsttartup  finish before open new entry downloaders!
            ...
            const checkAndGetMissionChunks = (liveEntryPath, recordingSessionPath, recordingSessionId, playlistGenerator) => {//todo should be for all flavors
                return checkAndAddFileToJson(recordingSessionPath, playlistGenerator, recordingSessionId).then(()=>{
                        return pGlob(path.join(liveEntryPath, '*/*/*.mp4')).then((downloadedChunks)=> { //todo should verify that pglobe is giving the chunks in right order
                                ...
                                _.each(downloadedChunks,  (filePath)=> {
                                    ...
                                    if (! playlistGenerator.checkFileExists(fileName)) {
                                        logger.info("checkAndGetMissionChunks: Found chunk %s exist on live directory but missing on recording directory: path %s", fileName, filePath)
                                        missingChunks.push(filePath)
                                    }
                                })
                                if (missingChunks.length == 0){
                                    logger.debug("checkAndGetMissionChunks: No missing chunks found (in Json) for entry %s", recordingSessionId)
                                    return Q.resolve()
                                }
                                else{
                                    //should create hard link and then add to json
                                    logger.debug("checkAndGetMissionChunks: No missing chunks found (in Json) for recorded session id %s", recordingSessionId)
                                    ...
                                    return Q.allSettled(linkChunksPromises).then((results)=>{
                                        _.each(results, (element, index)=> {
                                            if(element.state !== 'fulfilled'){
                                                logger.error("Failed to link %s- : %s", missingChunks[index], element.reason  ? element.reason : "Unknown reason")
                                                delete missingChunks[index] //remove failed chunks from array
                                            }
                                        })
                                        return  getMetaData(missingChunks, recordingSessionId).then((chunksToUpdate)=>{
                                            ...
                                            .then(()=>{
                                                logger.info("checkAndGetMissionChunks: Successfully add  chunks for recorded entry %s", recordingSessionId)
             ...
             */
            const checkAndAddFileToJson = (recordingSessionPath, playlistGenerator, recordingSessionId)=>{ //todo should verify that json is update on the main object!!!
                return pGlob(path.join(recordingSessionPath, '*/*.mp4'))
                    .then(function(downloadedChunks) {
                        ...
                        _.each(downloadedChunks, function (filePath) {
                            ...
                            if (! playlistGenerator.checkFileExists(fileName)) {
                                logger.info("checkAndAddFileToJson: Found chunk %s exist but missing on json for %s",fileName, recordingSessionId)
                                missingChunks.push(filePath)
                            }
                        })
                        if (missingChunks.length == 0){
                            logger.debug("checkAndAddFileToJson: No missing chunks found (compare to Live directory) for recorded session id  %s", recordingSessionId)
                            return Q.resolve()
                        }
                        else{
                            return getMetaData(missingChunks, recordingSessionId).then((chunksToUpdate)=>{
                                logger.debug("checkAndAddFileToJson: About to update chunks %s in json for recorded session id  %s", chunksToUpdate, recordingSessionId)
                                return  playlistGenerator.update(chunksToUpdate)
                            })
                                .then(()=>{
                                    logger.info("checkAndAddFileToJson: Successfully add chunks %s for recorded session id %s", missingChunks, recordingSessionId)
                                    return Q.resolve() // return resolve anyway!
                                })
            ...
            const getMetaData = (missingChunks, recordingSessionId)=>{
                logger.debug("About to parse meta data for the file %s, recordingSessionId: %s", missingChunks, recordingSessionId) //todo verify that all chunks are sorted
                ...
                return Q.allSettled(updateChunksPromises).then((results)=> {
                    ...
                    if (errs.numErrors > 0) {
                        logger.error("checkAndAddFileToJson: Failed to extract matadata for recordingSessionId: %s - %s", recordingSessionId, errs.err.message)
                                ...
                                return pGlob(path.join(recordingEntryPath, recordedEntryId, '*/'))
                                    .then((flavorListPath)=>{
                                        ...
                                        let recordingSessionSourcePath =  path.join(recordingSessionPath, source)
                                            return Q.all([getLastUpdateChunk(recordingSessionSourcePath), qio.exists(path.join(recordingSessionPath, 'done'))])
                                                .then((results)=> { //todo if faied return reject promice
                                                    ...
                                                    return accessibleRecording(playlistGenerator, recordedEntryId, recordingSessionPath).then(()=>{
                                                        ...
                                                        return checkAndGetMissionChunks(liveEntryPath, recordingSessionPath, entryId+recordedEntryId, playlistGenerator).then(()=>{
