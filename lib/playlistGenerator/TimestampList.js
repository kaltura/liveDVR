...
TimestampList.prototype.removeRange = function(from,to){
    ...
    if(that.durations.length) {
        ...
        if(startIndex >= lastIndex){
            that.setDurationAt(startIndex, to - from);
            return;
        }
        ...
        if(from <= playlistUtils.playlistConfig.timestampToleranceMs) {
            from = 0;
        }
        ...
        if( that.editPolicy === TimestampList.prototype.editPolicy.cutaway ) {
            ...
            if(from > 0){
                that.totalDuration -= that.durations[startIndex] - from;
                that.setDurationAt(startIndex++, from);
            }
