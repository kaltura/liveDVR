...
TimestampList.prototype.validate = function(){
    var that = this;
...
TimestampList.prototype.setDurationAt = function(index,val){
    ...
    if(index === 0){
        that.offset = that.firstDTS[0] - that.playlist.inner.firstClipTime;
    }
};
...
TimestampList.prototype.removeRange = function(from,to){
    ...
    that.logger.warn("removeKeyFrames. %j-%j firstDTS=%d keyFrameDurations=%d",from,to,that.firstDTS.length , that.durations.length);

    if(that.durations.length) {

        assert(that.firstDTS.length == that.inner.durations.length + 1);
        ...
        if( this.editPolicy === TimestampList.prototype.editPolicy.cutaway ) {
            ...
            from -= that.firstDTS[startIndex];
            to -= that.firstDTS[lastIndex];
            ...
            if (leftOver > 0) {
                that.durations[startIndex] = leftOver;
                startIndex++;
            }
            ...
            if (lastIndex > startIndex) {
                ...
                if (lastIndex > 0) {
                    var a = _.rest(that.firstDTS, lastIndex);
            ...
            that.setDurationAt(startIndex++, from - that.firstDTS[startIndex]);
            that.setDurationAt(lastIndex--, that.firstDTS[lastIndex] + that.durations[lastIndex] - to);
            for(;startIndex < lastIndex;startIndex++) {
                that.setDurationAt(startIndex, 0);
            }
        }

        that.totalDuration -= timeRange;

        assert(that.firstDTS.length == that.durations.length + 1);
...
TimestampList.prototype.insertAt = function(index,firstDTS,duration) {
    ...
    assert(that.firstDTS.length === that.durations.length);
    ...
    if(index > 0 && duration == 0){
        duration = firstDTS - that.firstDTS[index-1];
    }
...
TimestampList.prototype.onUnserialize = function(){
    ...
    that.firstDTS = _.map(that.durations,function(d) {
        return d + that.offset + that.playlist.inner.firstClipTime;
    });
