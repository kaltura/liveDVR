...
GapPatcher.prototype.update = function () {
    ...
    if(false === that.playlist.inner.discontinuity) {
        ...
        while (that.itemCount) {
            var gap = that.at(0);
            // ensure all flavours are ready for a new clip
            if (_.every(that.playlist.inner.sequences, function (seq) {
                ...
                if(_.any(that.playlist.inner.sequences, function (seq) {
                    return seq.inner.clips.length && seq.inner.clips.last.validateGap(gap)
                }))  {
