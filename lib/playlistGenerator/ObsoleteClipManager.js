...
ObsoleteClipManager.prototype.add = function(path) {
    ...
    /*
        grace period during which packager still can access files removed by playlist generator.
        without this removing a chunk would have resulted in 404 in case packager is processing request
        while using older playlist snapshot.
    */
    var removeTimeout = 10000;
    var expires = that.playlist.getCurrentTime() + removeTimeout;

    that.logger.trace("Add new item %j expires at %j timeout %j",path, expires, removeTimeout);
...
ObsoleteClipManager.prototype.checkExpires = function (expires) {
    ...
    while( that.inner.paths.length && that.firstDTS[0] <= expires) {
        ...
        that.logger.trace("checkExpires. removing obsolete item %j",rpath);
        paths.push(rpath);
