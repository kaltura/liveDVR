...
Playlist.prototype.updateGap = function () {
    ...
    if (that.playlist.inner.clipTimes.length > 1) {
        ...
        //TODO make sure that.playlist.gap isFinite & >= 0!!!!
        if(!_.isFinite(that.playlist.gap)){
            throw new Error('sss');
        }
    }
};
...
Playlist.prototype.insertChunk = function (fileInfo) {
    ...
    this.throwChunkIndex =  this.throwChunkIndex ? this.throwChunkIndex + 1 : 1;

    if(this.throwChunkIndex % 2 == 0){
        throw new InvalidClipError(new diagnosticsAlerts.ValueOutOfRangeAlert(fileInfo.flavor,fileInfo.chunkName,
            `fileInfo.video.duration`,track.duration) )
    }
...
Playlist.prototype.isModified = function(){
    ...
    if (playlist.durations.length > 0) {
        ...
        _.each(_.first(this.inner.clipTimes,this.inner.clipTimes.length - 1),(c) => {
            if( !_.isFinite(c.value)){
