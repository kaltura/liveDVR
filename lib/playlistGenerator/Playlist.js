...
* */
function Playlist(playlistType, loggerInfo, serializationCtx) {
...
Playlist.prototype.serializeFrom = function (playlistType, playlistJSON,loggerInfo,cbDone,reportError){
    var playlist = undefined;
    try {
        playlist = JSON.parse(playlistJSON);
        playlist = new Playlist(playlistType, loggerInfo,playlist);
    } catch (err) {
        loggerModule.getLogger("Playlist.serializeFrom", loggerInfo + " ").warn('Unable to un-serialize playlist. Data loss is inevitable!');
        if(reportError) {
           playlist = err;
        } else {
           playlist = new Playlist(playlistType, loggerInfo);
        }
    }
    if(cbDone){
        cbDone(playlist);
