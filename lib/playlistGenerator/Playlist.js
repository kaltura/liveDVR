...
Playlist.prototype.toJSON = function() {
    ...
    if (obj.clipTimes.length > (Number(maxClipsPerFlavor) + 1)) {
        ...
        if (nonEmptyClips > totalAllowedClips) {
            ...
            that.logger.de(`total clips (with empty ones) > maxClipsPerFlavor [total clips: ${obj.sequences[0].clips.length},  empty clips: ${emptyClips}, non-empty clips: ${nonEmptyClips}, max allowed clips: ${totalAllowedClips}]`);
        }
    }
