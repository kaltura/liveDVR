...
Playlist.prototype.validate = function validate(opts) {
    ...
    if( !_.every(that.inner.sequences,function(seq){
        ...
        that.logger.warn("Playlist.validate duration is greater than the sum of the durations array ");
        return false;
    }
...
var rangeCompare = function(dtsRange2){
...
Playlist.prototype.recalculateOffsetsAndDuration = function recalculateOffsetsAndDuration (index){
   ...
   var ranges = _.map(that.inner.sequences,function(seq,index){
    ...
    ranges = _.filter(ranges, function(dtsRange){
       return _.every(ranges,rangeCompare,dtsRange);
    });

    _.each(ranges,function(dtsRange){
...
Playlist.prototype.serializeFrom = function (logger,stream,cbDone){
    ...
    stream.on('end', function () {
        ...
        plalylist = new Playlist(logger,plalylist);
        cbDone(plalylist);
    });
    stream.resume();
};
...
Playlist.prototype.collapseGap = function(from,to) {
    ...
    if(from < to) {
        ...
        that.logger.warn('Playlist.collapseGap. that.inner.firstClipTime=%s',new Date(that.inner.firstClipTime));
    }
};
...
Playlist.prototype.toJSON = function(){
    ...
    obj.sequences = _.filter(obj.sequences,function(seq){
        if(seq.clips.length) {
            ...
            return rangeCompare.call(that.minMax,dtsRange);
        }
    });
