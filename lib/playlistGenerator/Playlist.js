...
const maxAllowedClips = require('./../../common/Configuration').get('maxClipsPerPlaylistJson');
...
Playlist.prototype.collectObsoleteClips = function () {
    ...
    while (that.inner.durations.length) {
        ...
        if (_.every(seqs, function (seq) {
                return seq.inner.clips.first.isEmpty() || (seq.inner.clips.length > maxAllowedClips);
            })) {
            _.each(seqs, function (seq) {

                if (seq.inner.clips.length > maxAllowedClips) {
                    that.logger.info("collectObsoleteClips remove oldest clip seq=(%j) clips_count=%j (reduce clips to max allowed, %d)", seq.inner.id, seq.clips.length, maxAllowedClips);
                } else {
