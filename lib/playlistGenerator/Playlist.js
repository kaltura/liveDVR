...
Playlist.prototype.doValidate = function playlist_doValidate(opts) {
    ...
    if(that.inner.firstClipStartOffset >= that.inner.durations[0]){
        that.logger.warn("that.inner.firstClipStartOffset >= that.inner.durations[0] %j >= %j", that.inner.firstClipStartOffset,that.inner.durations[0]);
        return false;
    }
    ...
    if(!_.every(that.inner.clipTimes, clipTime => { return _.isFinite(clipTime); } ) )  {
    ...
    if(_.some(that.inner.clipTimes, (clipTime,index) => {
            if( index > 0 ){
                let prevClipTime =  that.inner.clipTimes[index-1] ;
                if (index > 1) {
                    prevClipTime += that.inner.firstClipTime;
                }
                return that.inner.clipTimes[index] > 0 && prevClipTime + that.inner.durations[index-1] > that.inner.clipTimes[index];
            }
            return false;
        })) {
    ...
    if( that.inner.clipTimes.length && that.inner.firstClipTime !== that.inner.clipTimes.first + that.inner.firstClipStartOffset - that.cumulatedGaps ){
...
var calcClipStartTimeAndDuration = function(index){
    ...
    if(retVal.valid) {
        ...
        that.inner.durations[index].value = retVal.max - retVal.min;

        if( !_.isFinite(that.inner.clipTimes[index].value) ) {
        ...
        if (index === 0) {
            ...
            if(firstClipTime != that.inner.firstClipTime) {
                ...
                that.emit(playlistUtils.ClipEvents.base_time_changed, retVal.min);
            }
            that.inner.firstClipStartOffset =  retVal.min - that.inner.clipTimes[index];
        }
    }
...
Playlist.prototype.onUnserialize = function () {
    ...
    if(that.inner.clipTimes.length) {
        that.cumulatedGaps = that.inner.clipTimes.first + that.inner.firstClipStartOffset -  that.inner.firstClipTime;
    }
...
Playlist.prototype.toJSON = function() {
    ...
    that.logger.trace("toJSON: %d valid sequences", obj.sequences.length);
