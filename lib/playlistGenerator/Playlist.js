...
Playlist.prototype.toJSON = function() {
    ...
    if (obj.clipTimes.length > (Number(maxClipsPerFlavor) + 1)) {
        ...
        let emptyClips = obj.sequences[0].clips.length - nonEmptyClips;
        ...
        if (nonEmptyClips > totalAllowedClips) {
            ...
            that.logger.debug(`total clips (with empty ones) > maxClipsPerFlavor [total clips: ${obj.sequences[0].clips.length},  empty clips: ${emptyClips}, non-empty clips: ${nonEmptyClips}, max allowed clips: ${totalAllowedClips}]`);
        }
    }
