...
* */
function Playlist(loggerInfo, serializationCtx, playlistType, maxClipsPerFlavor) {
    ...
    this.maxClipsPerFlavor = maxClipsPerFlavor;
...
Playlist.prototype.serializeFrom = function (playlistJSON,loggerInfo,cbDone,reportError, maxClipsPerFlavor){
    var playlist = undefined;
    try {
        playlist = JSON.parse(playlistJSON);
        playlist = new Playlist(loggerInfo, playlist, undefined, maxClipsPerFlavor);
    } catch (err) {
        loggerModule.getLogger("Playlist.serializeFrom", loggerInfo + " ").warn('Unable to un-serialize playlist. Data loss is inevitable!');
        if(reportError) {
           playlist = err;
        } else {
           playlist = new Playlist(loggerInfo, undefined, undefined, maxClipsPerFlavor);
        }
    }
    if(cbDone){
        cbDone(playlist);
