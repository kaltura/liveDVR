...
function Playlist(loggerInfo, serializationCtx) {
    ...
    PlaylistItem.prototype.constructor.call(this, this.logger, serializationCtx);
...
Playlist.prototype.getSequenceForFlavor = function (flavor) {
    ...
    if(sequenceIndex === undefined){
        that.logger.info("Add new sequence for flavor [%s]", flavor);
...
Playlist.prototype.validate = function validate(opts) {
    ...
    if( !_.every(that.inner.sequences,function(seq){
            for( var i in seq.clips ) {
                ...
                if( overallD - clipD > playlistUtils.playlistConfig.timestampToleranceMs ){
                    that.logger.warn("Clip [%d] duration = %d > sum of durations = %d", i, overallD, clipD);
                    return false;
                }
            }
            return true;
    ...
    if(_.some(that.inner.clipTime, function(clipTime,index){
        ...
        that.logger.warn("that.inner.clipTime[idx]-that.inner.durations[idx] != that.inner.clipTime[idx-1]");
        return false;
    }
...
var recalculateOffsetsAndDurationInner = function(index){
    ...
    if(retVal.min > Number.MIN_VALUE) {
        ...
        that.logger.info("Recalculate Offsets And Duration(%d). Set firstClipTime: %s (%d)",
            index,
...
Playlist.prototype.serializeFrom = function (playlistJSON,loggerInfo,cbDone){
    ...
    try {
        ...
        this.logger.warn('Unable to un-serialize playlist. Data loss is inevitable: %s', ErrorUtils.error2string(err));
    }
...
Playlist.prototype.getClipFromFileInfo = function (fileInfo) {
    ...
    if( !lastClip/*
     ...
     || lastClip.getDuration(that.logger) + fileInfo.videoDuration > that.playListLimits.maxClipDuration))*/
        ...
        var newClip = new MixFilterClip(loggerInfo, that, clips.length, seq);
        clips.push(newClip);
    }
