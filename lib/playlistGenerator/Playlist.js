...
Playlist.prototype.toJSON = function() {
    ...
    that.logger.debug("toJSON: total sequences: %d, total clips: %d", obj.sequences.length, obj.sequences[0].clips.length);

    if (obj.sequences.length > maxClipsPerFlavor) {
        that.logger.error(`A bug found!!! total clips > maxClipsPerFlavor [${obj.sequences[0].clips.length} > ${Number(maxClipsPerFlavor)+1}]`);
    }
