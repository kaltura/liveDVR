...
Playlist.prototype.doValidate = function playlist_doValidate(opts) {
    ...
    if(_.some(that.inner.clipTimes, (clipTime,index) => {
            if( index > 0 ){
                ...
                if (index === 1) {
                    prevClipTime += that.inner.firstClipStartOffset;
                }
                return _.isFinite(that.inner.clipTimes[index]) && prevClipTime + that.inner.durations[index-1] > that.inner.clipTimes[index];
            }
            return false;
        })) {
