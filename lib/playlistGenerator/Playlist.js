...
const maxClipsPerFlavor = require('./../../common/Configuration').get('maxClipsPerFlavor');
...
var calcClipStartTimeAndDuration = function(index){
    ...
    if(retVal.valid) {
        ...
        if( that.inner.clipTimes[index].value != retVal.min) {
            ...
            if (index === 0) {
               that.inner.firstClipTime = that.inner.clipTimes[index] - that.cumulatedGaps;
...
// collectObsoleteClips now handles both the playWindow roll over time
// and enforces max number of clips limitation
Playlist.prototype.collectObsoleteClips = function () {
    ...
    while (that.inner.durations.length) {
        ...
        if (_.every(seqs, function (seq) {
                return seq.inner.clips.first.isEmpty() || (seq.inner.clips.length > maxClipsPerFlavor);
            })) {
            _.each(seqs, function (seq) {

                if (seq.inner.clips.length > maxClipsPerFlavor) {
                    that.logger.info("collectObsoleteClips remove oldest clip seq=(%j) clips_count=%j (reduce clips to max allowed, %d)", seq.inner.id, seq.clips.length, maxClipsPerFlavor);
                } else {
