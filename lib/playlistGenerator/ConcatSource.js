...
function ConcatSource(loggerInfo, playlistObj, clipTime, track, chunkPath) {
    ...
    this.logger = loggerModule.getLogger("ConcatSource", loggerInfo + " ");
...
var removeItem = function(index,surpressEvent) {
    ...
    that.logger.info('Remove Item (%s). ', that.inner.tracks, util.inspect(that.getItem(index)));
...
ConcatSource.prototype.checkExpires = function () {
    ...
    if (that.playlist.playListLimits.manifestTimeWindowInMsec) {
        var tiotalDuration = arguments[1] || that.durationsMan.totalDuration;
        while (that.durationsMan.totalDuration > 0 &&
            tiotalDuration > that.playlist.playListLimits.manifestTimeWindowInMsec) {
            paths.push(that.inner.paths[0]);
            that.logger.info('Check Expires(%s). Found clip %s', that.inner.tracks, that.inner.paths[0]);
            tiotalDuration -= that.durationsMan.durations[0];
            popClip.call(that);
        }
    }
        ...
        while (that.durationsMan.durations.length > that.playlist.playListLimits.maxClipCountPerPlaylist) {
            paths.push(that.inner.paths[0]);
            that.logger.info('Check Expires(%s). Found clip %s, clip_count = %d', that.inner.tracks, that.inner.paths[0], that.durationsMan.durations.length);
            popClip.call(that);
        }
    }
    return paths;
...
ConcatSource.prototype.concatClip = function (fileInfo,track) {
    ...
    that.durationsMan.insertAt(idx,firstDTS,duration);
    ...
    that.logger.info('Concat Clip(%s): Append clip = [%s]; Dts = [%d]; Enc_dts = [%d]; Duration = [%d]; Sig = [%s]',
        that.inner.tracks,filePath, firstDTS,track.firstEncoderDTS.toFixed(0),duration,fileInfo.sig);
...
ConcatSource.prototype.split = function (newSrc,r) {
    ...
    // update duration of a last item before gap if needed
      if(lastThisIndex > 0){
        var updatedDuration = Math.min(that.durationsMan.firstDTS[lastThisIndex-1]
            + that.durationsMan.durations[lastThisIndex-1] - r.from,that.durationsMan.durations[lastThisIndex-1]);
    ...
    _.each( splitItems, function(duration,index) {
            ...
            that.logger.info('Split(%s): Append clip = [%s]; Dts = [%d]; Duration = [%d]',
                that.inner.tracks, item.paths, item.firstDTS, item.durations);
    ...
    newSrc.inner.offset = that.inner.offset + newSrc.durationsMan.firstDTS[0] - that.durationsMan.firstDTS[0];
...
ConcatSource.prototype.checkDiscontinuity = function (fileInfo,track) {
    ...
    if (len < 0) {
        ...
        if (fileInfo.startTime === 0) {
            that.logger.warn('CheckDiscontinuity(%s): Clip %s zero fileInfo.startTime.', that.inner.tracks, track.path);
        ...
        if (fileInfo.startTime === 0) {
            ...
            if (encTimeVariance < playlistConfig.maxAllowedPTSDrift) {
                ...
                if (timestampDiff < playlistConfig.maxChunkGapSize &&
                    ...
                    log.call(that.logger, 'CheckDiscontinuity(%s): Slight time diff. Previous clip [%s] -> new clip [%s, %s] Overlap = [%d msec] Enc delta = [%d]; Avg = [%d] Stddev = [%d] Prev clip duration = [%d]; Acc drift = [%d]',
                        that.inner.tracks,
                        ...
                        that.dtsJitter);
                    that.durationsMan.setDurationAt(len, that.durationsMan.durations[len] + timestampDiff);
                }

             } else {
                that.logger.warn('CheckDiscontinuity(%s): Encoder reset. Previous clip (%s,%d) -> New clip (%s,%d). Dts diff = [%d], Enc_diff = [%d]',
                    that.inner.tracks,
                    that.inner.paths[len],
                    lastDTS.toFixed(2),
                    fileInfo.path.substr(that.inner.basePath.length),
            ...
            if( timestampDiff > playlistConfig.maxChunkGapSize ) {
                that.logger.warn('CheckDiscontinuity(%s): Gap. Prev clip [%s] -> New clip [%s]. Gap = %d > maxChunkGapSize = %d',
                    that.inner.tracks,
                    that.inner.paths[len],
                    fileInfo.path.substr(that.inner.basePath.length),
                    timestampDiff,
        ...
        if(playlistUtils.playlistConfig.ignoreGaps) {
            that.durationsMan.setDurationAt(len,firstDTS - that.durationsMan.firstDTS[len]);
        }
    }
