...
var palylistConfig = playlistUtils.palylistConfig;
...
function ConcatSource(track,chunkPath) {
    ...
    PlaylistItem.prototype.constructor.call(that,chunkPath ? undefined : track);
...
var removeItem = function(index) {
    ...
    playlistUtils.Info('MixFilterClip.removeItem. ', util.inspect(that.getItem(index)));

    scheduleClipRemoval.call(that,that.inner.paths[index], Date.now() + palylistConfig.waitTimeBeforeRemoval);
...
ConcatSource.prototype.checkExpires = function (expires,maxChunkCount) {
    ...
    if (expires) {
        while (that.inner.expires.length > 0) {
            ...
            if (that.inner.expires[0] + that.inner.durations[0] < expires) {
                playlistUtils.Debug('MixFilterClip.checkExpires. found clip %s clip_expire=%d. expires=%d', that.inner.paths[0], that.inner.expires, expires);
                popClip.call(that);
            } else {
                break;
            }
    ...
    if (maxChunkCount) {
        while (that.inner.expires.length > maxChunkCount) {
            playlistUtils.Debug('MixFilterClip.checkExpires. found clip %s clip_count=%d. > maxChunkCount=%d', that.inner.paths[0], that.inner.expires.length, maxChunkCount);
            popClip.call(that);
        }
    }
...
ConcatSource.prototype.concatClip = function (fileInfo,track,expires) {
    ...
    if (that.inner.basePath) {
        ...
        if (offset < 0) {
            playlistUtils.Warn('clip=%s does not match basePath=%s', fileInfo.path, that.inner.basePath);
        } else {
    ...
    if( existingIdx >= 0 ){
        playlistUtils.Warn('ConcatSource.concatClip(%d,%s). signature already exists in the file list', fileInfo.flavor,fileInfo.path);
        if(Path.join(that.inner.basePath,that.inner.paths[existingIdx]) !== fileInfo.path) {
            playlistUtils.Warn('ConcatSource.concatClip(%d,%s). remove duplicate item', fileInfo.flavor,fileInfo.path);
    ...
    that.inner.firstDTS.push(Math.ceil(track.firstDTS) || 0);
    that.inner.firstEncoderDTS.push(Math.ceil(track.firstEncoderDTS) || 0);

    playlistUtils.Info('ConcatSource.concatClip(%d,%s): append clip=%s dts=%d enc_dts=%d dur=%d sig=%s',
        fileInfo.flavor,that.inner.tracks,filePath, track.firstDTS.toFixed(0),track.firstEncoderDTS.toFixed(0),duration,fileInfo.sig);
...
ConcatSource.prototype.checkDiscontinuity = function (fileInfo,track) {
    ...
    if (fileInfo.startTime === 0) {
        ...
        if (Math.abs(drift) < palylistConfig.maxAllowedPTSDrift) {
            ...
            playlistUtils.Warn('ConcatSource.checkDiscontinuity(%d,%s): previous clip (%s,%d) -> new clip (%s,%d) drift is too high=%d msec. dts diff=%d enc_diff=%d',
                fileInfo.flavor,
                that.inner.tracks,
        ...
        if (overlap != 0) {
            var log = Math.abs(overlap) > 1000 ? playlistUtils.Warn : playlistUtils.Info;
        ...
        if (overlap > palylistConfig.maxChunkGapSize) {
            playlistUtils.Warn('ConcatSource.checkDiscontinuity(%d,%s): gap: prev clip (%s) -> new clip (%s) gap=%d > maxChunkGapSize=%d',
                fileInfo.flavor,
                that.inner.tracks,
                that.inner.paths[len],
                fileInfo.path.substr(that.inner.basePath.length),
            ...
            that.inner.emit(ConcatSourceState.prototype.ConcatEvents.gap_limit_exceeded,
...
ConcatSource.prototype.validate = function (opts) {
    ...
    if( typeof that.inner.tracks !== 'string' || (that.inner.tracks[0] !== 'v' && that.inner.tracks[0] !== 'a' && that.inner.type !== 'concat') )  {
        playlistUtils.Warn("ConcatSource.validate tracks[0] === 'v' || tracks[0] === 'a') && type === concat");
        return false;
    }

    if( that.inner.offset === undefined ){
        playlistUtils.Warn("that.inner.offset === undefined");
        return false;
    }
    ...
    if( that.inner.paths.length !== that.inner.firstDTS.length && that.inner.durations.length !== that.inner.firstDTS.length && that.inner.firstDTS.length !== that.inner.expires.length ){
        playlistUtils.Warn("ConcatSource.validate durations.length !== firstDTS.length firstDTS.length && !== expires.length");
        return false;
    }
    ...
    _.each(that.inner.paths, function(path,index){
        ...
        if(!retVal){
            playlistUtils.Warn("ConcatSource.validate path %s does not exists!",filePath);
            nonexisting.push(index);
        }
    });

    if( nonexisting.length) {
        playlistUtils.Warn("!fs.existsSync(path);");
    ...
    if( !_.every(that.inner.durations, function(d){
        ...
        playlistUtils.Warn("ConcatSource.validate  duration > 0 failed");
        return false;
    }
    ...
    if( calcD  !== td ){
        playlistUtils.Warn("ConcatSource.validate  that.inner.durations.reduce( sum,0)  !== that.totalDuration");
        if(opts && opts.recover) {
