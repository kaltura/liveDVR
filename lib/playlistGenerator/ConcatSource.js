 ...
 */
function ConcatSource(logger,playlistObj,track,chunkPath) {
...
var createTimestampList = function(){
    this.durationsMan = new TimestampList(this.logger,
        this.playlist,
        this.inner,
        'offset',
        this.inner.durations,
    ...
    if (len < 0) {
        ...
        if (fileInfo.startTime === 0) {
            ...
            if (Math.abs(encDiff) < playlistConfig.maxAllowedPTSDrift) {
                ...
                that.inner.refEncoderDTS = that.inner.lastEncoderDTS = track.firstEncoderDTS;
                that.ptsStats.reset();
            }
        }
    }
