...
ConcatSource.prototype.concatChunk = function (fileInfo,track) {
    ...
    var result = that.getFirstDTS(fileInfo, track);
    ...
    result.firstDTS = Math.ceil(result.firstDTS || 0);
    let oldDuration = 0;
...
// todo: check with Igor why is it required here if checked in MixFilterClip
...
ConcatSource.prototype.getFirstDTS = function (fileInfo,track) {
    var that = this;
    ...
    if (that.durationsMan.durations.length === 0) {
        ...
        var encDiff = dtsDistance(track.firstEncoderDTS, that.inner.lastEncoderDTS, track.wrapEncoderDTS);
        ...
        let diagDrift = {
            deltaPts: track.firstDTS - that.playlist.ptsReference.absolute,
...
// todo Lilach: check with Igor what is the reason that ID3 continuity is not checked even though it is considered
// more reliable than encoder PTS/DTS
...
ConcatSource.prototype.checkDiscontinuity = function (fileInfo, track) {
    ...
    // if this file does not have absolute time information, skip all measuring - todo: check with igor what did he mean?
    var firstDTS = (fileInfo.startTime) ? Math.ceil(track.firstDTS) : lastDTS;
    ...
    // check difference between PTS of last track in stream source and the new track, taking in account PTS wrap
    var encDiff = dtsDistance(track.firstEncoderDTS, that.inner.lastEncoderDTS, track.wrapEncoderDTS);
    ...
    if (Math.abs(encDiff) >= playlistConfig.maxAllowedPTSDrift) {
        ...
        // make sure next chunk will not be thrown due to discontinuity
        // todo Lilach: check that gap is inserted with next chunk!!! -
        // resetPtsReference.call(that, firstDTS, track.firstEncoderDTS);
        ...
        // todo Lilach: check with Igor why is it needed? already found discontinuity problem!
        //checkClipStartTime.call(that, fileInfo);

    } else {
        checkOverlap.call(that,fileInfo,result.firstDTS,result.lastDTS);
        if (lastDTS - playlistUtils.playlistConfig.maxAllowedPTSDrift > firstDTS) {
           // that.logger.warn("checkOverlap. overlap not supported : %j",lastDTS-firstDTS);
           // throw new InvalidClipError(new diagnosticAlerts.OverlapPtsAlert (fileInfo.flavor, fileInfo.chunkName,firstDTS,lastDTS));
           result.overlap = true;
...
/*    if (checkGap.call(that, encDiff, fileInfo)) {
       // result.discontinuity = true;
        that.emit(playlistUtils.ClipEvents.gap_limit_exceeded, that, {from: lastDTS, to: firstDTS});
    }*/
