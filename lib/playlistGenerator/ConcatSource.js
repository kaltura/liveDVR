...
function ConcatSource(loggerInfo, playlistObj, parentObj, track, chunkPath) {
    ...
    this.searchIndex = new Set(this.inner.paths);
...
var removeItems = function(index, count=1,  surpressEvent=false) {
    ...
    if(!surpressEvent) {
        reportObsoleteChunks.call(this,that.inner.paths.slice(index,count));
    }
    ...
    _.each(allKeysButDurations,function (key) {
        let deleted = this[key].splice(index, count);
        if(key === 'paths'){
            for (var path of deleted) {
                that.searchIndex.delete(Path.basename(path));
            }
        }
    }, that.inner);
...
ConcatSource.prototype.split = function (newSrc,r) {
    ...
    _.each(that.inner.durations.slice(lastThisIndex,firstThatIndex),function(duration){
        ...
        removeItems.call(that,lastThisIndex, 1, true);
...
ConcatSource.prototype.doValidate = function (opts) {
    ...
    if(that.inner.paths.length !== that.searchIndex.size) {
       that.logger.warn("that.inner.paths.length !== that.searchIndex.size %d !== %d",
           that.inner.paths.length , that.searchIndex.size );
        return false;
    }
...
Object.defineProperty(ConcatSource.prototype , "KeyFrameDurations", {
    ...
    set: function set_keyFrameDurations(keyFrameDurations) {
        ...
        _.every(that.inner.durations, function (d, index) {
            ...
            if (that.inner.durations.length - 1 === index && idx === keyFrameDurations.length) {
                ...
                if(that.inner.lastKeyFrameDuration){
                    ...
                    if( that.inner.durations.length > that.keyFrameDurations.length ){
                        playlistUtils.dbgAssert(d === 0);
                        that.keyFrameDurations.push([that.inner.lastKeyFrameDuration]);
                    } else {
                        playlistUtils.dbgAssert(d === that.inner.lastKeyFrameDuration);
                ...
                playlistUtils.dbgAssert(keyFrameDurations.length === 0 || idx > idxlast);
            }
            return idx > idxlast;
        });
