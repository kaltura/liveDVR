...
var removeItems = function(index, count=1,  surpressEvent=false) {
    ...
    _.each (removedPaths , (path) => that.searchIndex.remove(Path.basename(path)))
...
ConcatSource.prototype.concatChunk = function (fileInfo,track) {
    ...
    this.searchIndex.add(Path.basename(filePath))
...
ConcatSource.prototype.split = function (newSrc,r) {
    ...
    _.each( splitItems, function(duration,index) {
        ...
        newSrc.searchIndex.add(Path.basename(item.paths));
...
Object.defineProperty(ConcatSource.prototype , "KeyFrameDurations", {
    ...
    set: function set_keyFrameDurations(keyFrameDurations) {
        ...
        _.every(that.inner.durations, function (curChunkDuration, curChunkIndex) {
            var idxKeyFrameBeforeIteration = idxKeyFrame;

            // find where current key frame fit
            for (;idxKeyFrame < keyFrameDurations.length && curChunkDuration > 0; idxKeyFrame++) {
                curChunkDuration -= keyFrameDurations[idxKeyFrame];
            }

            // current chunk duration has been exhausted, so append resulting range to keyFrameDurations
            if (idxKeyFrame > idxKeyFrameBeforeIteration) {
                that.keyFrameDurations.push(keyFrameDurations.slice(idxKeyFrameBeforeIteration, idxKeyFrame));
            }
            // NB: last gop is never passed along so make it up
            if (that.inner.durations.length - 1 === curChunkIndex && idxKeyFrame === keyFrameDurations.length) {
                ...
                playlistUtils.dbgAssert(keyFrameDurations.length === 0 || idxKeyFrame > idxKeyFrameBeforeIteration);
            }
            // proceede on condition that key frame index has advanced
            return idxKeyFrame > idxKeyFrameBeforeIteration;
        });

        playlistUtils.dbgAssert( idxKeyFrame === keyFrameDurations.length);
    }
});
