...
var removeItem = function(index,surpressEvent) {
    ...
    that.logger.debug('Remove Item(%j) %j', index, that.getItem(index));
...
ConcatSource.prototype.checkExpires = function (clock,totalDuration) {
    ...
    else if (that.playlist.playListLimits.maxClipCountPerPlaylist && that.playlist.playListLimits.maxClipCountPerPlaylist != 'INF') {
...
ConcatSource.prototype.concatClip = function (fileInfo,track) {
    ...
    if( track.keyFrameDTS && track.keyFrameDTS.length) {
        ...
        if(keyFrameDTS.length === 0 || _.any(keyFrameDTS,(dts,index) => { return index > 0 && dts < keyFrameDTS[index-1] } )){
            ...
            throw new playlistUtils.BadClipError(playlistUtils.ClipErrors.badKeyFrames);
        }
        ...
        if(oldDuration > 0 && that.keyFrameDurations.length ){
            ...
            var correction = that.durationsMan.durations.last - oldDuration;
