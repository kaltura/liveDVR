...
Object.defineProperty(ConcatSource.prototype , "KeyFrameDurations", {
    get: function get_keyFrameDurations() {
        ...
        let result = _.reduce(that.keyFrameDurations,(result,kfd) => {
            _.each(kfd,(d)=> {
                result.push(d);
            });
            return result
    ...
    set: function set_keyFrameDurations(keyFrameDurations) {
        ...
        let lastGopDuration = _.reduce(that.inner.durations,(reduced,chunkDuration) => {
            _.every(keyFrameDurations,(kfd,keyFrameIndex)=>{

                playlistUtils.dbgAssert(chunkDuration >= kfd);
                chunkDuration -= kfd

                if(chunkDuration <= 0){

                    playlistUtils.dbgAssert(chunkDuration === 0);
                    that.keyFrameDurations.push(keyFrameDurations.splice(0,keyFrameIndex+1))
                    return false
                }
                return true
            })
        ...
        playlistUtils.dbgAssert(lastGopDuration > 0);

        keyFrameDurations.push(lastGopDuration)
        that.keyFrameDurations.push(keyFrameDurations);
