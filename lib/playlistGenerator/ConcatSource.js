...
//      supressEvent: if 'true' fire event per removed clip
var removeItems = function(index, count=1,  surpressEvent=false) {
    var that = this;

    that.logger.debug('Remove Item(%j) %j count=%d', index, that.getItem(index), count);

    if(!surpressEvent) {
        that.emit(playlistUtils.ClipEvents.clip_removed, that, index, count);
    }

    that.durationsMan.splice(index, count);

    that.keyFrameDurations.splice(index, count);

    _.each(allKeysButDurations,function(key){
        this[key].splice(index, count);
    }, that.inner);
...
ConcatSource.prototype.clearAll = function() {
    ...
    var paths = this.inner.paths.slice(0);
...
ConcatSource.prototype.concatClip = function (fileInfo,track) {
    ...
    if( track.keyFrameDTS && track.keyFrameDTS.length) {
        ...
        if (keyFrameDTS.length === 0 || _.any(keyFrameDTS, (dts, index) => {
            ...
            that.logger.warn("concatClip. bad key frame times : %j", track.keyFrameDTS);
...
ConcatSource.prototype.split = function (newSrc,r) {
    ...
    _.each(that.inner.durations.slice(lastThisIndex,firstThatIndex),function(duration){
        ...
        removeItems.call(that,lastThisIndex, 1);
...
ConcatSource.prototype.checkFileExists = function(fileName) {
    if( _.findIndex(this.inner.paths,(val) => {
            return val.indexOf(fileName) >= 0;
        }) >= 0) {
        return true;
    }
    return false;
