 ...
 */
function ConcatSource(loggerInfo, playlistObj, clipIndex, track, chunkPath) {
...
ConcatSource.prototype.checkExpires = function () {
    ...
    if (that.playlist.playListLimits.manifestTimeWindowInMsec) {
        while (that.durationsMan.totalDuration > that.playlist.playListLimits.manifestTimeWindowInMsec) {
...
ConcatSource.prototype.concatClip = function (fileInfo,track) {
    ...
    that.logger.info('Concat Clip(%s): Append clip = [%s]; Dts = [%d]; Enc_dts = [%d]; Duration = [%d]; Sig = [%s]',
        that.inner.tracks,filePath, firstDTS.toFixed(0),track.firstEncoderDTS.toFixed(0),duration,fileInfo.sig);
...
ConcatSource.prototype.split = function (newSrc,r) {
    ...
    _.each( that.inner.durations.slice(firstThatIndex), function(duration,index) {
            ...
            that.logger.info('Split(%s): Append clip = [%s]; Dts = [%d]; Duration = [%d]',
                that.inner.tracks, filePath, firstDTS.toFixed(0), duration);
        ...
        if (fileInfo.startTime === 0) {
            ...
            if (Math.abs(encDiff) < playlistConfig.maxAllowedPTSDrift) {
                ...
                if (overlap < playlistConfig.maxChunkGapSize &&
                    ...
                    log.call(that.logger, 'CheckDiscontinuity(%s): Previous clip [%s] -> new clip [%s, %s]; Overlap = [%d msec]; Enc delta = [%d]; Avg = [%d]; Stddev = [%d]; Prev clip duration = %[d]; Acc drift = [%d]',
                        that.inner.tracks,
                    ...
                    that.logger.warn('CheckDiscontinuity(%s): Prev clip [%s] -> New clip [%s]. Gap = %d > maxChunkGapSize = %d',
                        that.inner.tracks,
                        that.inner.paths[len],
                        fileInfo.path.substr(that.inner.basePath.length),
                        overlap,
                ...
                that.logger.warn('CheckDiscontinuity(%s): Previous clip (%s,%d) -> New clip (%s,%d). Dts diff = [%d], Enc_diff = [%d]',
                    that.inner.tracks,
                    that.inner.paths[len],
                    lastDTS.toFixed(2),
                    fileInfo.path.substr(that.inner.basePath.length),
