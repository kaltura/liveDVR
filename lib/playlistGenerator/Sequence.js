            ...
            if (trackDurations[s.inner.tracks]) {
                ...
                if (before > 0) {
                    ...
                    if (expired.length) {
                        ...
                        c.appendFilesToDelete(expired, removedChunks);
                    }
...
Sequence.prototype.checkMaxAllowedClips = function (maxClipsPerFlavor) {
    ...
    try {
        ...
        if (nonEmptyclips.length > maxClipsPerFlavor) {
            this.logger.warn(`Max allowed clips exceeded. [total non-empty clips: ${nonEmptyclips.length}, max allowed: ${maxClipsPerFlavor}, clips to remove: ${nonEmptyclips.length - maxClipsPerFlavor}]`);

            return _.chain(nonEmptyclips).first(nonEmptyclips.length - maxClipsPerFlavor).reduce((filesToBeRemoved, c) => {

                return filesToBeRemoved.concat(c.clearClip()).unique();
