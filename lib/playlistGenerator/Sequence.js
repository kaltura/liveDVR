            ...
            if (trackDurations[s.inner.tracks]) {
                ...
                if (before > 0) {
                    ...
                    if (expired.length) {
                        ...
                        c.appendFilesToBeRemoved(expired, removedChunks);
                    }
...
Sequence.prototype.checkMaxAllowedClips = function (maxClipsPerFlavor) {
    ...
    let nonEmptyclips = _.filter(this.clips, (clip) => {
        return !clip.isEmpty();
    });
    ...
    try {
        ...
        if (nonEmptyclips.length > maxClipsPerFlavor) {
            this.logger.warn(`Max allowed clips exceeded. [total clips: ${this.clips.length}, max allowed: ${maxClipsPerFlavor}, clips to remove: ${nonEmptyclips.length - maxClipsPerFlavor}]`);

            return _.chain(nonEmptyclips).first(nonEmptyclips.length - maxClipsPerFlavor).reduce((filesToBeRemoved, c) => {

                return filesToBeRemoved.concat(c.clearClip());
        ...
        this.logger.error(`checkMaxAllowedClips: failed to clear clips. Max allowed: ${maxClipsPerFlavor}, total non-empty clips: ${nonEmptyclips.length}. Error ${ErrorUtils.error2string(err)}`)
    }
