...
var unserializeClip = function (c){
    ...
    switch(c.type){
        case 'mixFilter':
            return new MixFilterClip(that.logger,that.playlist,c);
        case 'source':
            return new SourceClip(that.logger,that.playlist,c);
        default:
...
var Sequence = function (logger,playlistObj,inner) {
    ...
    PlaylistItem.prototype.constructor.call(that,logger,playlistObj,inner);
    ...
    if(!_.isObject(inner) || !Object.keys(inner).length) {
        that.inner = {
            ...
            scheduledForRemoval: new ObsoleteClipManager(logger,this);
        };
...
var createTimestampList = function(){
    ...
    this.keyFramesMan = new TimestampList(this.logger,
        this.playlist,
        this.inner,
        'firstKeyFrameOffset',
        this.inner.keyFrameDurations,
...
Sequence.prototype.onUnserialize = function(){
    ...
    that.inner.scheduledForRemoval = new ObsoleteClipManager(that.logger,that,that.inner.scheduledForRemoval);
};
...
var mergeKeyFrames = function(src,keyFrames){
    ...
    that.logger.debug("mergeKeyFrames. keyFrames=%d firstDTS=%d keyFrameDurations=%d",keyFrames.length,
    ...
    keyFrames = _.map(keyFrames.slice(0,keyFrames.length-1),function(kf){
        return kf - 5;
    });

    that.keyFramesMan.insertRange(keyFrames.slice(0,keyFrames.length-1));
...
Sequence.prototype.handleEvent = function (type,arg) {
    ...
    switch(type){
        ...
        case playlistUtils.ClipEvents.clip_removed:
            var src = arg,
                idx = arguments[2],
                from = src.durationsMan.firstDTS[idx],
                to = from + src.durationsMan.durations[idx];
            that.keyFramesMan.remove(from,to);
            break;
        default:
            //forward
...
Sequence.prototype.collapseGap = function (from,to) {
    ...
    that.keyFramesMan.collapseGap(from,to);
};
...
Sequence.prototype.checkExpires = function (expires) {
    ...
    var clips = that.clips;

    while( clips && clips.length ) {
        var clip = clips[0];
        var expired = clip.checkExpires();
        if (expired.length) {
    ...
    return trashBin.checkExpires(expires);
};
