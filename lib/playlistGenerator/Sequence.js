...
Sequence.prototype.createIterator = function* (opts) {
    let criteria = null;
    if(_.isBoolean(opts)){
        criteria =  (s) => opts === s.isVideo
    } else {
        criteria =  (s) => _.isEqual(opts,s.inner.tracks)
    }
    for( let c of this.clips ){
        let found = _.filter(c.inner.sources,criteria );
        if(!_.isEmpty(found) ) {
            let s = found[0];
            for (let i = 0; i < s.inner.durations.length; i++) {
                yield s.getItem(i)
            }
        }
    }
