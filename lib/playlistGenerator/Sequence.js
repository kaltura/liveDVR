...
Sequence.prototype.checkMaxAllowedClips = function (maxClipsPerFlavor) {
    ...
    let numEmptyClips = _.reduce(this.clips, (count, clip) => {
        return clip.isEmpty() ? count + 1 : count;
    }, 0);

    let clips2Remove = Math.max(this.clips.length - numEmptyClips - maxClipsPerFlavor, 0);

    // check if number of non-emtpy clips passed maximum allowed
    if (clips2Remove > 0) {
        this.logger.warn(`Max allowed clips exceeded. [total clips: ${this.clips.length}, max allowed: (${maxClipsPerFlavor}), clips to remove: ${clips2Remove}]`);

        _.each(this.clips, (c) => {
            if (!c.isEmpty() && clips2Remove > 0) {
                clips2Remove--;

                // for each source in removed clip, remove all chunks until clip is empty
                _.each(c.inner.sources, (s) => {
                    // empty source from chunks
                    let paths = s.removeClip();
                    if (paths.length) {
                        filesToDeleteUpdate.call(this, paths, c, filesToDelete);
                    }
                });

                if (!c.isEmpty()) {
                  this.logger.error(`checkMaxAllowedClips: Bug found. Clip is not empty!!!`)
                }
            }
        });
...
function filesToDeleteUpdate( pathsOfRemovedChunks, clip, filesToDelete ) {

    // only remove file if no source uses it...
    let unusedFiles = _.filter(pathsOfRemovedChunks, function (file) {
        return _.every(clip.inner.sources, function (s1) {
            return s1.inner.paths.indexOf(file) < 0;
        });
    });
    if (unusedFiles.length) {
        _.each(unusedFiles, function (p) {
