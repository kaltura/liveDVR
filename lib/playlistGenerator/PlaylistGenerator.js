...
function PlaylistGenerator (entryObject,isNewSession) {
    ...
    if (entryObject.playWindow && entryObject.maxChunkCount) {
        that.playListLimits =  {
        ...
        //this.playListLimits = entryObject.playListLimits;
        this.logger.warn(`>>>> playlist limits not initialized. Check if set from persistence list?`);
    }
...
var setPlayList = function (playlist){
    ...
    that.playlistImp.playListLimits = that.playListLimits ? that.playListLimits : that.playlistImp.playListLimits;
...
var doPersist =  function(ended) {
    ...
    if(that.playlistImp.inner.liveWindowDuration) {
        that.playlistImp.inner.presentationEndTime = !ended ? that.playlistImp.getCurrentTime() + that.playlistImp.inner.liveWindowDuration : Date.now;
    } else if (ended) {
        that.playlistImp.inner.presentationEndTime = Date.now;
    }
    try {
...
// save playlist
// todo: if stream ended set presentationEndTime to now - playWindow
var persistPlayList = function persistPlayList(force, ended = false){
    var that = this;
    ...
    if(shouldPersist) {
        ...
        if(Q.isPending(that.persistPromise)){
            return that.persistPromise
                .then( function(){
                    return persistPlayList.call(that,force, ended);
                });
        }
        ...
        if(that.playlistImp.isModified() || force) {
            ...
            if( playlistUtils.playlistConfig.enablePlaylistHistory ) {
                ...
                return deferred.promise.then( function(){
                    return doPersist.call(that, ended);
                });
...
// todo: set property of stopped stream instead of presentationEndTime
PlaylistGenerator.prototype.stop = function stop (){
    ...
    if(isRunning.call(that)){
        that.startPromise = null;
        that.playlistImp.expirationTime = Data.now;
...
var checkTranscodingProfileChange = function () {
    var that = this;
    var entryPath = persistenceFormat.getEntryBasePath(that.entryId);
...
// todo: set ended entry PresentationEndTime
PlaylistGenerator.prototype.end = function() {

    return persistPlayList.call(this, true, true)
        .then(() => {
            this.logger.debug(`@@@ [${this.entryId}], stream ended.`);
        })
        .catch((err) => {
            this.logger.error(`Error ending stream of entry [${this.entryId}]: , error: ${ErrorUtils.error2string(err)}`);
        });
}

// update presentationEndTime and save playlist
PlaylistGenerator.prototype.persistStopped = function() {
    return persistPlayList.call(this, true);
}

// update presentationEndTime and expireTime and save to disk
PlaylistGenerator.prototype.restoreStopped = function() {
    this.start();
    if (this.playlistImp.presentationEndTime) {
        delete this.playlistImp.presentationEndTime;
    }
    this.playlistImp.expirationTime = Data.now;
    return persistPlayList.call(this, true);
}
