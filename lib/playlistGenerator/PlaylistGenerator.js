...
const hlsMaxDurationMsec = 60 * 60 * 1000 * config.get("playlistConfig").hlsMaxDurationInHours;
...
function applyHLSWindowFactor (val) {
    ...
    return calculatedWindowSizeMsec < hlsMaxDurationMsec ? calculatedWindowSizeMsec : hlsMaxDurationMsec + extraTimeMsec;
...
function PlaylistGenerator (entryObject, options) {
    ...
    if (_.has(options, 'recording') && options.recording == true){
        ...
        that.playListLimits = {
            manifestTimeWindowInMsec: Math.min(recordingMaxDurationMsec, hlsMaxDurationMsec) + extraTimeMsec
        }
