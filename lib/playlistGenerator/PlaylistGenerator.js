...
function PlaylistGenerator (entryObject, options) {
    ...
    setPlayList.call(that,new Playlist(that.loggerInfo, undefined, that.playlistType, maxClipsPerFlavor));
    that.gapsCounter = 0;
...
var checkRemoveChunks = function(flavor){
    ...
    let removedChunks = seq.checkExpires(that.playlistImp.getCurrentTime(), that.playlistImp.totalDuration);

    let removedChunks2 = seq.checkMaxAllowedClips(maxClipsPerFlavor);

    if (removedChunks2.length > 0) {
        removedChunks = removedChunks.concat(removedChunks2);
    }
...
PlaylistGenerator.prototype.update = function (chunklist) {
    ...
    return [Q.all([that.tsSaveQueue,persistPlayList.call(that)]) , Q.resolve({removedChunks: removedChunks})];
};
