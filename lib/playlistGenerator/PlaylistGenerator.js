...
function PlaylistGenerator (entryObject, options) {
    ...
    if ('isNewSession' in options){
        that.isNewSession = options.isNewSession
    }
    else{
        that.isNewSession = isNewSession;
    }
    let playlistType
    if ('isRecording' in options && options.isRecording == true){
        that.playListLimits =  {
            manifestTimeWindowInMsec: 86400000, //24 hours
            maxClipCountPerPlaylist: 'INF'
        };
        that.playlistPath = path.join(options.recordingPath, persistenceFormat.getMasterManifestName())
        playlistType = 'VOD'
    }
    else {
        that.playListLimits =  {
    ...
    setPlayList.call(that,new Playlist(that.loggerInfo, undefined, playlistType));
    that.gapsCounter = 0;
...
var handleClipError = function(removedChunks,fileInfo){
    ...
    if(!seq) {
        ...
        removedChunks.push(chunkName);
    } else if (!seq.checkFileExists(chunkName)){
        // avoid from downloader atempting to push this file again and again...
        that.logger.warn("PlaylistGenerator.handleClipError. scheduled for deletion chunk %j",fileInfo.path);
        removedChunks.push(chunkName);
    }
};
