...
PlaylistGenerator.getAbsoluteTimeFromRelative = function(playlist, relativeTime, forceClipTimes = true)
{
    let absoluteEndTime =  playlist.clipTimes[playlist.clipTimes.length-1] + playlist.durations[playlist.durations.length-1];
    relativeTime = Math.max(relativeTime ,0);
    let i = 0;
    if (forceClipTimes && playlist.clipFrom)
    {
        while (i < playlist.clipTimes.length && playlist.clipFrom > playlist.clipTimes[i])
            i++; //search for the first clip according the clipFrom attribute
        relativeTime += playlist.clipFrom - playlist.clipTimes[--i];
    }
    ...
    let ret = Math.min(playlist.clipTimes[i] + relativeTime, absoluteEndTime);
    if (forceClipTimes && playlist.clipTo)
        return Math.min(ret, playlist.clipTo);
    return ret;
}
