...
const DiagnosticsErrorCodes = require('./../Diagnostics/DiagnosticsAlerts').DiagnosticsErrorCodes;
...
function PlaylistGenerator (entryObject, options) {
    ...
    if (_.has(options, 'recording') && options.recording == true){
        that.loggerInfo = `[${entryObject.entryId}-${entryObject.recordedEntryId}]`;
        that.playListFormat = 'recording';
        that.recordedEntryId = entryObject.recordedEntryId;
...
PlaylistGenerator.prototype.getTotalDuration = function () { 
...
PlaylistGenerator.prototype.handleEvent = function (type,arg) {
    ...
    switch(type){
        ...
        case playlistUtils.ClipEvents.diagnosticsAlert:
            ...
            if ((arguments[2].errorCode === DiagnosticsErrorCodes.MissingTrackAlert) || (arguments[2].errorCode === DiagnosticsErrorCodes.InvalidKeyFramesAlert)) {
                that.emit('updateBeacons', { report: arguments[2] });
            }
            else {
