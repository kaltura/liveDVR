...
function PlaylistGenerator (entryObject,isNewSession,logger){
    ...
    setPlayList.call(that,new Playlist(that.logger));
    that.gapsCounter = 0;
...
var setPlayList = function (playlist){
    ...
    that.playlistImp.inner.lastModified = that.playlistImp.getCurrentTime();
    if(that.playListLimits.manifestTimeWindow) {
        that.playlistImp.inner.dvrWindow = that.playListLimits.manifestTimeWindow;
    }
};
...
var playlistIsModified = function(){
    ...
    if (playlist.durations.length > 0) {
        ...
        if (that.playlistImp.minMax && that.playlistImp.minMax.min === minMaxFirstClip.min && that.playlistImp.minMax.max === minMaxLastClip.max) {
            return false;
        }
