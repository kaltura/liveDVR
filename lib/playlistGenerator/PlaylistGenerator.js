...
function PlaylistGenerator (entryObject,isNewSession,logger){
    ...
    that.playListLimits =  {
        ...
        minRefreshPlaylistInterval:config.get('minRefreshPlaylistInterval') || 60000
    };
...
PlaylistGenerator.prototype.remove = function(flavor){
    var that = this;

    that.logger.info("PlaylistGenerator.remove(%s)",flavor);

    return that.stopPromise.promise
        .then( function () {
            that.logger.debug("PlaylistGenerator.remove. wait on persistPromise ? %s", Q.isPromise(that.persistPromise));
            return that.persistPromise;
        })
        .finally(function () {

            var innerPlaylist = that.playlistImp.inner;
            
            var idx = innerPlaylist.flavor2SeqIndex[flavor];

            that.logger.debug("PlaylistGenerator.remove. flavor=%s => idx=%d",flavor,idx);

            return Q.allSettled(_.map(innerPlaylist.sequences[idx],function(clip){
                if(clip.inner && clip.inner.scheduledForRemoval) {
                    return Q.allSettled(_.map(clip.inner.scheduledForRemoval,function(sr){
                        return removeFile.call(that,sr.path);
                    }));
                } else {
                    return qResolved;
                }
                ...
                .catch( function(err){
                    that.logger.warn("PlaylistGenerator.remove. error %s",err);
                })
                .finally (function() {

                idx = innerPlaylist.flavor2SeqIndex[flavor];

                innerPlaylist.sequences.splice(idx,1);
                delete innerPlaylist.flavor2SeqIndex[flavor];

                //update other flavor indices
                _.each(Object.keys(innerPlaylist.flavor2SeqIndex),function(k){
                    if(innerPlaylist.flavor2SeqIndex[k] > idx){
                        innerPlaylist.flavor2SeqIndex[k] -= 1;
                    }
                });

                if(innerPlaylist.sequences.length === 0) {
                    that.logger.info("PlaylistGenerator.remove. erasing empty playlist %s",that.playlistPath);
                    return qio.remove(that.playlistPath);
                } else {
                    return persistPlayList.call(that,isRunning.call(that));
                }
            });
        });
};
...
PlaylistGenerator.prototype.start = function start() {
    ...
    try {
        ...
        fs.exists(that.playlistPath,function(ok) {
            ...
            Playlist.prototype.serializeFrom(rs,function(newPlaylist){
