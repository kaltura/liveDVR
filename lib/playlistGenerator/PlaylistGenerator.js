...
PlaylistGenerator.prototype.remove = function(){
    var that = this;
    return that.stopPromise.promise
        ...
        .then(function () {
            return Q.allSettled(_.map(that.inner.sequences,function(seq){
                return Q.allSettled(_.map(seq.clips,function(clip){
                    if(clip.inner.basePath) {
                        return qio.removeDirectory(clip.inner.basePath);
                    } else {
                        return qResolved;
                    }
                }));
                ...
                .finally (function() {
                that.logger.info("PlaylistGenerator.remove. removing playlist from FS");
                return qio.remove(that.playlistPath);
            });
        });
};

PlaylistGenerator.prototype.endOfStream = function() {
    //TODO: add an eos indication to playlist
};
