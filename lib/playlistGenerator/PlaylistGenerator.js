...
const FSAccess = playlistUtils.playlistConfig.mockFS ?
    class FSMock {
    ...
    class FSAccessWorking {
        static writeFileAtomically() {
            return fsUtils.writeFileAtomically.apply(fsUtils,arguments)
        }
        static read() {
            return qio.read.apply(qio,arguments)
        }
    }
...
function PlaylistGenerator (entryObject, options) {
    ...
    if (_.has(options, 'recording') && options.recording == true){
        that.playlistType = 'VOD';
        that.playListLimits =  {
            manifestTimeWindowInMsec: config.get('recording').recordingMaxDurationInHours * 60 * 60 * 1000
        };
    ...
    setPlayList.call(that,new Playlist(that.loggerInfo, undefined, that.playlistType));
    that.gapsCounter = 0;
...
var doPersist =  function() {
    ...
    try {
        ...
        that.persistPromise = FSAccess.writeFileAtomically(that.playlistPath, jsonStr);
        if (playlistUtils.playlistConfig.humanReadable) {
            try {
                playlistUtils.playlistConfig.humanReadablePass = true;
                var p2 = FSAccess.writeFileAtomically(that.playlistPath + ".human", JSON.stringify(that.playlistImp, undefined, 1));
            } finally {
...
function startPlaylistGenerator() {
    ...
    that.startPromise = FSAccess.read(that.playlistPath)
        .then(function(data) {
