...
PlaylistGenerator.prototype.setFirstFlavor = function(firstFlavor)
{
    if (firstFlavor)
    {
        let tmpFlavorsArray = [firstFlavor];
        _.each(this.flavorsObjArray, (f)=> {
            if (f.name != firstFlavor.name)
                tmpFlavorsArray.push(f);
        });
        this.flavorsObjArray = tmpFlavorsArray;
    }
}
