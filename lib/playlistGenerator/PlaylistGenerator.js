    ...
    try {
        ...
        fs.readFile(that.playlistPath,function(err,data) {
            ...
            Playlist.prototype.serializeFrom(data,that.loggerInfo,function(newPlaylist){
                ...
                that.updateStreamStoppedState(StreamStoppedState.started);
...
// Stream stopped state has following states:
// 1) started - the DC started to provide the stream
// 2) stopped - the DC stopped providing the stream, but at least one other DC is still providing it.
// 3) ended - no DC is providing the stream
// input: StreamStoppedState
PlaylistGenerator.prototype.updateStreamStoppedState = function(stoppedState) {

    switch (stoppedState) {
        case StreamStoppedState.started:
            this.logger.debug(`>>>> stream stoppedStatus changed to <<< STARTED >>>`);
            this.playlistImp.inner.stopped = false;
            break;
        case StreamStoppedState.ended:
            this.logger.debug(`>>>> stream stoppedStatus changed to <<< ENDED >>>`);
            this.playlistImp.inner.stopped = false;
            break;
        case StreamStoppedState.stopped:
            this.logger.debug(`>>>> stream stoppedStatus changed to <<< STOPPED >>>`);
            this.playlistImp.inner.stopped = true;
            break;
        default:
            this.logger.error(`invalid stoppedStatus, ${stoppedState}. This is a bug! call developer.`);
            break;
    }
}
...
PlaylistGenerator.prototype.stop = function stop (){
    ...
    if(isRunning.call(that)){
        that.startPromise = null;
        // set the flag that signals packager
        // that the DC is no longer providing the stream.
        // All requests to that DC will result with 404.
        that.updateStreamStoppedState(StreamStoppedState.stopped);
...
PlaylistGenerator.prototype.end = function() {
    this.updateStreamStoppedState(StreamStoppedState.ended);
    return persistPlayList.call(this, true)
        .then(() => {
            this.logger.debug(`successfully ended stream of entry [${this.entryId}]`);
        })
        .catch((err) => {
