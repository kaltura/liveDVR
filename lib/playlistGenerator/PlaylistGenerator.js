...
PlaylistGenerator.prototype.update = function (chunklist) {
    ...
    let failedAddChunks = _.reduce(chunklist, (arr, chunk)=>{
        let failedChunk = addNewClip.call(that, chunk)
        if (failedChunk != undefined){
            arr.push(failedChunk)
        }
        return  arr
    },  [])
    var removedChunks = [];
    ...
    let result = {removedChunks: removedChunks,
        failedAddChunks : failedAddChunks
    }
    return [Q.all([that.tsSaveQueue,persistPlayList.call(that)]) , Q.resolve(result)];
};
