...
var playlistIsModified = function(){
    ...
    if (playlist.durations.length > 0) {
        ...
        if( minMax == that.playlistImp.minMax ){
            return false;
        }

        // only update manifest if all downloaders have contributed to max
        if ( that.playlistImp.minMax && minMax.max === that.playlistImp.minMax.max ) {
            return false;
        }

        that.playlistImp.collectObsoleteClips();

        minMax = that.playlistImp.recalculateOffsetsAndDuration();
...
var persistPlayList = function persistPlayList(force){
    ...
    if(shouldPersist) {
        ...
        if(playlistIsModified.call(that) || force) {
