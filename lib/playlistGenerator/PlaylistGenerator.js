...
var setPlayList = function (playlist){
    ...
    that.playlistImp.inner.presentationEndTime = that.playlistImp.getCurrentTime() + that.playListLimits.manifestTimeWindow;
...
PlaylistGenerator.prototype.handleEvent = function (type,arg) {
    ...
    switch(type){
        ...
        case playlistUtils.ClipEvents.modified:
            //check if all ready to split on gap
            if(arg === that.playlistImp.inner.gaps){
                that.playlistImp.inner.gaps.reduceGaps();
            }
            that.isDirty = true;
            break;
    }
