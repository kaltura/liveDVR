...
const hlsMaxAllowedWindowSizeMilliseconds = 24 * 60 * 60 * 1000;
// additional time that ensures play window grows to configured size (regardless chunk duration)
const extraTimeMilliseconds = 600000;

function applyHLSWindowFactor (val) {
    var calculatedWindowSizeMilliseconds = Math.min( val * hlsWindowProtectionFactor, val * (hlsWindowProtectionFactor-1) + extraTimeMilliseconds);
    return calculatedWindowSizeMilliseconds < hlsMaxAllowedWindowSizeMilliseconds ? calculatedWindowSizeMilliseconds : hlsMaxAllowedWindowSizeMilliseconds + extraTimeMilliseconds;
...
function PlaylistGenerator (entryObject, options) {
    ...
    if (_.has(options, 'recording') && options.recording == true){
        ...
        var recordingMaxAllowedWindowSizeMilliseconds = config.get('recording').recordingMaxDurationInHours * 60 * 60 * 1000;
        that.playListLimits = {
            manifestTimeWindowInMsec: Math.min(recordingMaxAllowedWindowSizeMilliseconds, hlsMaxAllowedWindowSizeMilliseconds) + extraTimeMilliseconds
        }
