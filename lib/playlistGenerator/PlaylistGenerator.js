...
function applyHLSWindowFactor (val) {
    return Math.min( val * hlsWindowProtectionFactor, val * (hlsWindowProtectionFactor-1) + 600000);
}
/*
...
PlaylistGenerator.prototype.update = function (chunklist, encodedBitrate) {
    var that = this;

    if(chunklist.length) {
        that.playlistImp.updateEncodedBitrate(chunklist[0].flavor, encodedBitrate);

        _.each(chunklist,addNewClip,that, encodedBitrate);
        ...
        .then(function(data) {
            Playlist.prototype.serializeFrom(data, that.loggerInfo, function (newPlaylist) {
                if(newPlaylist instanceof Error) {
                    newPlaylist = new Playlist(that.loggerInfo, undefined, that.playlistType);
                }
    ...
    _.each(this.playlistImp.inner.sequences, (s) => {
        _.each( s.clips, (c) => {
            _.each( c.sources, (src) => {
                src.inner.basePath = newBasePath
            })
        })
    })
