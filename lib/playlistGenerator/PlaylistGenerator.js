...
function PlaylistGenerator (entryObject,isNewSession,logger){
    ...
    that.eventsCB = handleEvents.bind(that);
}
...
var addNewClip = function addNewClip (fileInfo){
    ...
    var lastClip = that.playlistImp.getClipFromFileInfo(fileInfo,that.eventsCB);
    ...
    if(shouldPersist) {
        ...
        do {
            if (playlist.durations.length > 0) {
                ...
                // zero range clips?
                if (!(minMaxFirstClip && minMaxLastClip) && !force) {
                    break;
                }
...
var finalizeStart = function (){
    ...
    _.each(that.flavorsObjArray,function(flavorObj){
        that.playlistImp.getClipListForFlavor(flavorObj.name);
       });
        ...
        fs.readFile(that.playlistPath,function(err,data) {
            ...
            if(err){
                if(err instanceof Error && err.code !== 'ENOENT' ) {
                    that.logger.warn("PlaylistGenerator.start readFile error: ", err);
            ...
            Playlist.prototype.serializeFrom(data,that.logger,function(newPlaylist){
                that.playlistImp = newPlaylist;
                that.playlistImp.inner.presentationEnd = false;
                persistPlayList.call(that)
                    .then( function(){
                        finalizeStart.call(that);
                    });
...
var handleEvents = function (type,arg) {
    ...
    switch(type){
        ...
        case playlistUtils.ClipEvents.chunk_reorder:
            that.logger.warn("PlaylistGenerator.handleEvents. handle chunk_reorder(%s-%s)",arg.from,arg.to);
...
PlaylistGenerator.prototype.deliverEndOfPresentation = function() {
    var that = this;
    that.playlistImp.inner.presentationEnd = true;
    return persistPlayList.call(that,true);
};
