...
var dbgAssert = function(){
    if(playlistConfig.debug){
        ...
        logger.warn(new Error('assertion failed at').stack);
    }
};
...
var TimeRange = function (min,max){
    this.min = min || Number.MIN_VALUE;
    this.max = max ||  Number.MAX_VALUE;
};

Object.defineProperty(TimeRange.prototype , "valid", {
    get: function get_Value() {
        var ok =  this.min && this.min !== Number.MIN_VALUE && this.max && this.max !== Number.MAX_VALUE;
...
var ClipErrors = {
    unknown: 'unknown',
    undefStartTime: 'undefStartTime',
    overlapUnsup: 'overlapUnsup'
};

function BadClipError(reason) {
    this.reason = reason || ClipErrors.unknown;
    Error.prototype.constructor.apply(this,arguments);
}

util.inherits(BadClipError,Error);

module.exports.BadClipError = BadClipError;

module.exports.ClipErrors = ClipErrors;
