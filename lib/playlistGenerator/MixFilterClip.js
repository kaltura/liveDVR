...
var validateInput = function(fileInfo){
    ...
    if( _.any( that.inner.sources, (src) => { 
            let track = src.isVideo ? fileInfo.video : fileInfo.audio; 
            if(track) {
                return true;
            }
            that.logger.warn('track information is missing for %j',src.inner.tracks[0]);
            that.emit(playlistUtils.ClipEvents.diagnosticsAlert, that, 
                new diagnosticsAlerts.MissingTrackAlert(fileInfo.flavor,
                    'missing data for track ' + src.inner.tracks[0],
                    fileInfo.chunkName));
            return false;
        })
    ){
