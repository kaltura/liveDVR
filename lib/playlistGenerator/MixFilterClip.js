...
var checkNewSource = function(type,fileInfo){
    ...
    if( that.inner.sources.every( function(s){
        ...
        var newSrc = new ConcatSource(that.logger,type+'1',fileInfo.path)
...
MixFilterClip.prototype.insert = function (fileInfo,expires) {
    ...
    for( var idx = 0; idx < that.inner.sources.length; idx++ ) {
        ...
        that.logger.debug('MixFilterClip.insert. clip=%s track=%s duration=%d. first_dts=%d enc_dts=%d expires=%s',
            fileInfo.path,src.inner.tracks,track.duration.toFixed(2),track.firstDTS,track.firstEncoderDTS,expires);
...
// update offsets for sources based on firstClipTime value. makes for offset to playlist window for specific source's media track
...
MixFilterClip.prototype.toJSON = function () {
    ...
    if(!playlistUtils.playlistConfig.humanReadablePass){
        ...
        if(jsonReps.length) {
            var l = jsonReps[0].items.length;
            for (var i = 0; i < l; i++) {
                var media = {};
                _.each(jsonReps, function (src,index) {
                    var item = src.items[i];
                    item.paths = path.join(that.inner.sources[index].inner.basePath,item.paths);
