...
function validateInput (fileInfo){
    ...
    for( var idx = 0; idx < that.inner.sources.length; idx++ ) {
        ...
        var result = src.checkDiscontinuity(fileInfo,track);

        if (result.discontinuity) {

            that.logger.warn('%s alert, PTS discontinuity', fileInfo.chunkName);
            var ptsHigh = src.inner.lastEncoderDTS;
            var ptsNew = track.firstEncoderDTS;

            return new diagnosticsAlerts.PtsDiscontinuityAlert(fileInfo.flavor, fileInfo.chunkName, ptsHigh, ptsNew);

        }  // if uncommented, there is always overlap problem with stream from streaming client
        // check overlap:
        //todo Lilach: what is the correct behavoir to presist stream even if PTS jump back in time???
        else if (result.lastDTS - playlistUtils.playlistConfig.maxAllowedPTSDrift > result.firstDTS) {
                var ptsOverlap = result.lastDTS - result.firstDTS;
                // todo Lilach: verify which units is the overlap seconds/milliseconds/other
                that.logger.warn("%s checkOverlap. overlap not supported : %j milliseconds", fileInfo.chunkName, ptsOverlap);

                return new diagnosticsAlerts.OverlapPtsAlert(fileInfo.flavor, fileInfo.chunkName, result.firstDTS, result.lastDTS);
        }
    }

    if(!fileInfo){
        that.logger.warn('%s alert, Bad fileInfo', fileInfo.chunkName);
        return new diagnosticsAlerts.MissingReferenceAlert(this.seq.inner.id,
            undefined,
            'fileInfo');
    }

    // todo: check with Igor where is the best location for timestamp verification
    if(!fileInfo.startTime){
        that.logger.warn('%s alert, FileInfo.startTime is not set to unix time', fileInfo.chunkName);
        return new diagnosticsAlerts.NoID3TagAlert(fileInfo.flavor, fileInfo.chunkName);
    }

    if(!fileInfo.flavor){
        that.logger.warn('%s alert, Bad fileInfo.flavor');
        return new diagnosticsAlerts.MissingReferenceAlert(this.seq.inner.id,
            fileInfo.chunkName,
            'fileInfo.flavor');
    }
    ...
    if(!fileInfo.sig || typeof fileInfo.sig !== 'string'){
        that.logger.warn('%s alert, Bad fileInfo.sig', fileInfo.chunkName);
    ...
    if(!media.length){
        that.logger.warn('%s alert, !fileInfo.video && !fileInfo.audio', fileInfo.chunkName);
        return new diagnosticsAlerts.MissingReferenceAlert(fileInfo.flavor,fileInfo.chunkName,
    ...
    let retVal = _.chain(that.inner.sources).map((src) => {
            ...
            that.logger.warn('%s alert, fileInfo.chunkNametrack information is missing for %j', fileInfo.chunkName, src.inner.tracks);
    ...
    if (videoPts !== null && audioPts !== null) {
        ...
        if (gap > config.get('diagnostics').ptsMaxMisalignmentAllowedInMSec) {
            that.logger.warn("%s alert, misaligned PTS, video PTS=%d, audio PTS=%d", fileInfo.flavor, fileInfo.chunkName, videoPts, audioPts);
            return new diagnosticsAlerts.PtsMisalignmentAlert(fileInfo.flavor, fileInfo.chunkName, videoPts, audioPts, gap);
        }
    }
