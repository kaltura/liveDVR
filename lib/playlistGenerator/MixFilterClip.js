...
*/
function MixFilterClip(loggerInfo, playlist, index, listener, serializationCtx) {

    this.loggerInfo = loggerInfo;
    this.logger = loggerModule.getLogger("MixFilterClip", loggerInfo + " ");
...
var createConcatSource = function(type,path){
    var that = this;

    return new ConcatSource(that.loggerInfo,that.playlist,that.clipIndex,type,path);
};
...
var checkNewSource = function(type,fileInfo){
    ...
    if( that.inner.sources.every( function(s){
        ...
        var newSrc = createConcatSource.call(that,type+'1',fileInfo.path);
...
MixFilterClip.prototype.onUnserialize = function () {
    ...
    that.inner.sources = _.map(that.inner.sources,function(s){
        return createConcatSource.call(that,s);
    });
};
...
MixFilterClip.prototype.split = function (r) {
    ...
    that.logger.info("Split mix clip: %d", that.clipIndex);

    var newInst =  new MixFilterClip(that.loggerInfo, that.playlist, that.index + 1);

    _.each(that.inner.sources, function(src,index) {
        var newSrc =  checkNewSource.call(that, src.tracks[0], {path: src.basePath + 'dum'});
        newInst.inner.sources[index].split(newSrc,r);
        newInst.inner.sources.push(newSrc);
    });
...
Object.defineProperty(MixFilterClip.prototype , "lastClipStartTime", {
    ...
    get: function get_lastClipStartTime() {
        var that = this;
        return _.min(that.inner.sources, function (src) {
            return src.durationsMan.itemCount ? src.durationsMan.firstDTS[src.durationsMan.itemCount - 1] : -1;
        });
    }
});
