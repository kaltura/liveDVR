...
const ClipValidationFailedAlert = diagnosticsAlerts.ClipValidationFailedAlert;
const CVFAR = diagnosticsAlerts.ClipValidationFailedAlertReasons;
...
function validateMediaTrack(fileInfo,m){
    ...
    if (typeof track.firstDTS !== 'number') {
        that.logger.warn(`Typeof fileInfo.${m}.firstDTS !== 'number'`);
        return new ClipValidationFailedAlert(fileInfo.flavor,fileInfo.chunkName,
            CVFAR.InvalidType,
            [`fileInfo.${m}.firstDTS`,'number',typeof track.firstDTS ]);
    }

    if (typeof track.firstEncoderDTS !== 'number') {
        that.logger.warn(`Typeof fileInfo.${m}.firstEncoderDTS !== 'number'`);
        return new ClipValidationFailedAlert(fileInfo.flavor,fileInfo.chunkName,
            CVFAR.InvalidType,
            ['fileInfo.${m}.firstEncoderDTS','number',typeof track.firstEncoderDTS ]);
    }

    if (typeof track.duration !== 'number') {
        that.logger.warn(`Typeof fileInfo.${m}.duration !== 'number'`);
        return new ClipValidationFailedAlert(fileInfo.flavor,fileInfo.chunkName,
            CVFAR.InvalidType,
            ['fileInfo.${m}.duration','number',typeof track.duration ]);
    }
    if (track.duration <= 0) {
        that.logger.warn(`FileInfo.${m}.duration  <= 0`);
        return new ClipValidationFailedAlert(fileInfo.flavor,fileInfo.chunkName,
            CVFAR.ValueOutOfRange,
    ...
    if(fileInfo.targetDuration && fileInfo.targetDuration * exceedFactor <  track.duration){
        ...
        return new ClipValidationFailedAlert(fileInfo.flavor,fileInfo.chunkName,
            CVFAR.TargetdurationExceeded,
            [`fileInfo.${m}.duration`,fileInfo.targetDuration,track.duration]);
    }
    if(m === 'video') {
        if (!fileInfo.video.keyFrameDTS || !fileInfo.video.keyFrameDTS.length) {
            ...
            return new ClipValidationFailedAlert(fileInfo.flavor,fileInfo.chunkName,
                CVFAR.Alert,
                [new diagnosticsAlerts.InvalidKeyFramesAlert(fileInfo.flavor,fileInfo.chunkName, [])]);
        }
    }
    return ClipValidationFailedAlert.None;
}
...
function validateInput (fileInfo){
    ...
    if(!fileInfo){
        ...
        return new ClipValidationFailedAlert(this.seq.inner.id,
            undefined,
            CVFAR.MissingReference,
            'fileInfo');
    }
    ...
    if(_.any(media,(m)=>{
            ...
            return alertObj !== ClipValidationFailedAlert.None;
    })){
        return alertObj;
    }
    ...
    if (videoPts !== null && audioPts !== null) {
        ...
        if (gap > config.get('diagnostics').ptsMaxMisalignmentAllowedInMSec) {
            that.emit(playlistUtils.ClipEvents.diagnosticsAlert, that, 
                new diagnosticsAlerts.PtsMisalignmentAlert(fileInfo.flavor, fileInfo.chunkName, videoPts, audioPts, gap));
        }
    }
...
var createConcatSource = function(index,type,path){
    ...
    return new ConcatSource(that.id + '[s-' + index + ']',that.playlist,that, type,path);
};
...
MixFilterClip.prototype.insert = function (fileInfo) {
    ...
    if(ClipValidationFailedAlert.None !== failedVal){
...
MixFilterClip.prototype.getDTSRange = function(){
   ...
   var filteredSrc = []; /*_.filter(that.inner.sources,function(s){
       return s.isVideo;
   });*/
