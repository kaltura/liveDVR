...
MixFilterClip.prototype.clearClip = function() {
    ...
    _.each(this.inner.sources, (s) => {
        ...
        let paths = s.ClearAll();
        if (paths.length) {
            this.appendFilesToBeRemoved(paths, filesToRemove);
        }
    });

    return _.unique(filesToRemove);
}

MixFilterClip.prototype.appendFilesToBeRemoved = function (pathsOfRemovedChunks, filesToRemove) {

    // only remove file if no source uses it...
    _.chain(pathsOfRemovedChunks).filter(file => {
        return !this.checkFileExists(file);
    }).reduce((filesToRemove, p) => {
        filesToRemove.push(p);
        return filesToRemove;
    }, filesToRemove).values();
