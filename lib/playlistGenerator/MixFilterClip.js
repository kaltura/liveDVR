...
var validateInput = function(fileInfo){
    ...
    if(!fileInfo.startTime){
        ...
        that.emit(playlistUtils.ClipEvents.diagnosticsAlert, that, new diagnosticsAlerts.NoID3TagAlert(fileInfo));
    }
    ...
    if (videoPts !== null && audioPts !== null) {
        ...
        if (gap > config.get('diagnostics').ptsMaxMisalignmentAllowedInSec) {
            that.emit(playlistUtils.ClipEvents.diagnosticsAlert, that, new diagnosticsAlerts.PtsMisalignmentAlert({ fileInfo : fileInfo, videoPts : videoPts, audioPts : audioPts, gap : gap }));
        }
    }
