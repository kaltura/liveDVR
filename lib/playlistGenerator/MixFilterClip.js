...
function MixFilterClip(loggerInfo, index, playlist,  seq, clipTime, serializationCtx) {
    ...
    this.id = loggerInfo.appendQuote(index,'c');
...
var createConcatSource = function(index,type,path){
    ...
    return new ConcatSource(that.id.appendQuote(index,'s'),that.playlist,that.clipTime,type,path);
};
...
MixFilterClip.prototype.insert = function (fileInfo) {
    ...
    if(!validateInput.call(that,fileInfo)){
        throw new playlistUtils.BadClipError('bad input fileInfo');
    }
...
MixFilterClip.prototype.getDTSRange = function(){
    ...
    _.each(that.inner.sources,function(s) {
        ...
        retVal.min = Math.max(range.min,retVal.min);
        retVal.max = Math.min(range.max ,retVal.max);
    });
...
MixFilterClip.prototype.validateGap = function (gap) {
    var that = this;

    return _.any(that.inner.sources,function(src){
       return src.validateGap(gap);
    });
};
