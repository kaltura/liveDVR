...
*/
function MixFilterClip(logger,playlist,index,listener,serializationCtx) {
...
var createConcatSource = function(type,path){
    ...
    return new ConcatSource(that.logger,that.playlist,that.clipIndex,type,path);
};
...
MixFilterClip.prototype.split = function (r) {
    ...
    that.logger.info("MixFilterClip(%d).split", that.clipIndex);

    var newInst =  new MixFilterClip(that.logger,that.playlist,that.index+1);
