...
var validateInput = function(fileInfo){
    ...
    if (videoPts && audioPts) {
        if (Math.abs(videoPts - audioPts) > config.get('diagnostics').ptsMaxMisalignmentAllowed) {
            that.emit(playlistUtils.ClipEvents.diagnosticsAlert, that, new diagnosticsAlerts.PtsMisalignmentAlert({ VideoPts : videoPts, AudioPts : audioPts }));
        }
    }
