...
MixFilterClip.prototype.clearClip = function() {
    ...
    _.each(this.inner.sources, (s) => {
        // empty source from chunks
        let filesCandidatesToDelete = s.clearAll();
        if (filesCandidatesToDelete.length) {
            this.appendFilesToDelete(filesCandidatesToDelete, filesToDelete);
        }
    });

    return _.unique(filesToDelete);
}

MixFilterClip.prototype.appendFilesToDelete = function (filesCandidatesToDelete, filesToDelete) {

    // only remove file if no source uses it...
    _.chain(filesCandidatesToDelete).filter(file => {
        return !this.checkFileExists(file);
    }).reduce((filesToDelete, p) => {
        filesToDelete.push(p);
        return filesToDelete;
    }, filesToDelete).values();
