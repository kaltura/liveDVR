...
var validateInput = function(fileInfo){
    ...
    fileInfo.startTime = (fileInfo.flavor === '33') ? null : fileInfo.startTime;
    if(!fileInfo.startTime){
        that.logger.warn('FileInfo.startTime is not set to unix time');
        that.emit(playlistUtils.ClipEvents.diagnosticsAlert, that, new diagnosticsAlerts.NoID3TagAlert(fileInfo), fileInfo.flavor);
    }
    ...
    if (videoPts !== null && audioPts !== null) {
        ...
        if (gap > config.get('diagnostics').ptsMaxMisalignmentAllowedInMSec) {
            that.emit(playlistUtils.ClipEvents.diagnosticsAlert, that, 
                new diagnosticsAlerts.PtsMisalignmentAlert({ fileInfo : fileInfo, videoPts : videoPts, audioPts : audioPts, gap : gap }), 
                fileInfo.flavor);
        }
    }
