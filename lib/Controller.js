...
module.exports = (function(){
    ...
    function processAlreadyLiveEntries(currentlyLiveEntries) {
        ...
        _.forEach(alreadyLiveEntries, function(e) {
            ...
            if (entry === 'cancelPending') {
                ...
                entry.update(that.wowzaDiagnostics.entriesJson[e.entryId]);
            }
        });
    }
    ...
    function getEntriesDiagnostics() {
        let that = this;
        let currTime = new Date();
        if ((currTime - that.wowzaDiagnostics.lastUpdated) >= that.wowzaDiagnosticsInterval) {
            return adapter.getEntriesDiagnostics("entries")
                .then((res)=> {
                    that.wowzaDiagnostics.lastUpdated = new Date();
                    logger.debug("Received entries diagnostics from Wowza");
                    that.wowzaDiagnostics.entriesJson = res;
                });
        }
        return Q.resolve();
    }
    ...
    function handleEntries(){
        ...
        return adapter.getLiveEntries()
            ...
            .then(function(liveEntries) {
                return getEntriesDiagnostics.call(that)
                    .then(()=> {
                        return liveEntries;
                    })
            })
