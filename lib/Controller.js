...
var regressionConfig = require('./Adapters/RegressionAdapter/RegressionConfig.js').getConfig();
...
var adapter = require('./Adapters/AdapterFactory.js');
...
module.exports = (function(){
    ...
    function Controller(prefix, unitTestEventsEmitter){
        ...
        this.shouldRun = true;
        this.unitTestEventsHandler = unitTestEventsEmitter;
        this.monitorServer = new MonitorServer(() => {
            return self.getStat();
        });
        if (!module.parent) {
            this.monitorServer.listen(config.get('monitorPort') + prefix);
        }
    ...
    Controller.prototype.start = function() {
        ...
        this.once('exit', function(code) {
            if (that.unitTestEventsHandler) {
                that.unitTestEventsHandler.emit('exit', code);
            } else {
                process.exit(code);
                // Todo: check if a bug --> stop() doesn't work as expected
                // that.stop();
            }
        });
        adapter = adapter.getAdapter(that);
        ...
        return Q.fcall(function() {
            if (regressionConfig.command && regressionConfig.command.record ||
                config.get('preserveOriginalHLS')) {
                that.setRecordingTimer();
            }
    ...
    Controller.prototype.setRecordingTimer = function() {

        var that = this;
        var durationMillisec = 60000 * parseInt(config.get('recordingDurationMinutes'));
