...
module.exports = (function(){
    ...
    function _registerToPushForPartnersOfLiveEntries(liveEntries)
    {
        const pushMgr = new pushManager.PushManager(this);
        _.forEach(liveEntries, function(entry)
        {
            logger.debug(`Registering partner ${entry.partnerId} for push with id [${entry.entryId}]`);
            pushMgr.registerEntry(entry.partnerId, entry.entryId);
        });
    }
        ...
        return adapter.getLiveEntries()
            ...
            .then(function(liveEntries){
                if (that.shouldRun) {
                    ...
                    _registerToPushForPartnersOfLiveEntries.call(that, liveEntries);
