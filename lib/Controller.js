...
var CrossDCsStreamPersistence = require('./entry/CrossDCsStreamPersistece');
...
module.exports = (function(){
    ...
    function Controller(prefix){
        ...
        this.crossDCsStreamPersistence = new CrossDCsStreamPersistence();

        this.monitorServer.register('/stat',(req,res)=> {
            res.writeHead(200, {"Content-Type": "application/json"});
            res.end(JSON.stringify(self.getStat()));
        });
        //ability to call entry methods from webservice
        this.monitorServer.register('/entry/(.*)/(.*)',(req,res)=> {
            if (req.args && req.args.length > 1) {
                let entryId = req.args[0];
                let method=req.args[1];
                let entry = self.handledEntries[entryId];
                if (entry && _.isObject(entry)) {
                    ...
                    if (_.isFunction(entry[method])) {
                        ...
                        res.end("Method " + method+" on  " + entryId + " was called");
                    } else {
                    ...
                    res.writeHead(404, {"Content-Type": "text/plain"});
                    res.end("Entry " + entryId + " not found");
                }
            }
        });
    ...
    function createLiveEntry(e){
        ...
        var cleanupEntryFolderForNewSession = function(){
            var entryFolderPath = persistenceFormat.getEntryBasePath(e.entryId);
        ...
        this.crossDCsStreamPersistence.removeEntry(e.entryId)
            .then((removed) => {
                if (removed) {
                    this.logger.debug(`>>>> successfully removed entry [${e.entryId}] from cross DCs persistent list.`);
                }
            })
            .catch((err) => {
                this.logger.error(`>>>> failed to remove entry [${e.entryId}] from cross DCs persistent list!!!. error: [${ErrorUtils.error2string(err)}]`);
            })
            .done();
        ...
        _.forEach(noLongerLiveEntries, function (noLongerLiveEntry) {
            ...
            if (liveEntry === 'pending') {
                ...
                return liveEntry.stop()
                    .then(() => {
                        return that.crossDCsStreamPersistence.addEntry(liveEntry.playlistGenerator);
                    })
                    .then(function() {
    ...
    Controller.prototype.start = function() {
        ...
        return Q.fcall(function() {
            ...
            that.crossDCsStreamPersistence.init()
                .then(function(){
