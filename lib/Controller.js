...
module.exports = (function(){
    ...
    function processAlreadyLiveEntries(currentlyLiveEntries) {
        ...
        _.forEach(alreadyLiveEntries, function(e) {
            var liveEntryState = that.handledEntries[e.entryId];
            if (liveEntryState === 'cancelPending') {
            ...
            if (liveEntryState !== 'pending')
                e.updateLiveTimestamp();
        });
    }
        ...
        return adapter.getLiveEntries()
            ...
            .then(function(liveEntries){
                if (that.shouldRun) {
                    ...
                    // start new timer
                    that._intervalObj = setTimeout(handleEntries.bind(that), that.pollingInterval);
                }
            }).catch(function(err) {
