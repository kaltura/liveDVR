...
module.exports = (function(){
        ...
        return sessionManager.isNewSession(e.entryId, logger)
            ...
            .then(function(isNewSession) {
                    ...
                    return liveEntry.start()
                        ...
                        .catch(function(err) {
                            err.entry = liveEntry;
                            logger.debug("[%s] Failed to start, assigning entry to handled entry list for cancellation", e.entryId);
                            return Q.reject(err);
                        });
            })
            .then(function(entry) {
                if (that.handledEntries[e.entryId] !== 'cancelPending') {
                    ...
                    entry.stop()
                    .catch(function(err) {
                        logger.error("[%s] Aborting liveEntry object creation - %j", e.entryId, err);
                    })
                    .then(function() {
                    ...
                    function(err) {
                        ...
                        if (that.handledEntries[e.entryId] === 'cancelPending') {
                            ...
                            if (entry) {
                                entry.stop()
                                    .then(function () {
                                        logger.debug("[%s] Stopped entry. Removing it from list of handled entries", e.entryId);
                                        delete that.handledEntries[e.entryId];
                                    });
                            }
                        }
                ...
                return liveEntry.stop()
                    .then(function() {
                        ...
                        return that.endListAppender.addEntry(noLongerLiveEntry) // Indicate entry is down and should (later) have end-list appended
                            ...
                            .finally(function () {
                                logger.info("[%s] Removing playlist", noLongerLiveEntry);
    ...
    Worker.prototype.start = function() {
        ...
        logger.warn("Log files %s; %s",config.get('logFileName'),config.get('webServerParams:logFileName'));
