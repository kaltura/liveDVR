        ...
        return sessionManager.isNewSession(e.entryId, logger)
            ...
            .then(function(entry) {
                if (that.handledEntries[e.entryId] !== 'cancelPending') {
                    ...
                    logger.debug("[%s] Removing entry from endlistAppender", e.entryId);
                    that.endListAppender.removeEntry(e.entryId) // Entry is alive
                        .catch(function(err){
                            logger.error("[%s] Error adding entry to endlist appender processing - %s", e.entryId, err.message);
                        });
                }
                else {
                    ...
                    entry.stop()
                        .catch(function(err) {
                            logger.error("[%s] Aborting liveEntry object creation - %s", e.entryId, err);
                        })
                        .finally(function() {
        ...
        _.forEach(unhandledLiveEntries, function(e) {
            ...
            if (!that.handledEntries[e.entryId] && (!that.phantomEntries[e.entryId] || that.phantomEntries[e.entryId] < maxEntryInitializationTryCount)) {
                ...
                createLiveEntry.call(that, e)
                    ...
                    function(err) {
                        if (that.handledEntries[e.entryId] === 'cancelPending') {
                            logger.info("[%s] Ignoring error since entry stopped streaming", e.entryId);
                            delete that.handledEntries[e.entryId];
                            return;
                        }
                        logger.error("[%s] Error starting entry object: %s, %s", e.entryId, err.message, err.stack);
                        // If the liveEntry creation was unsuccessful, allow it to be created again
                        // in the next round
                        // TODO: Check this retry mechanism. Currently it doesn't work at all, why? -> Gad
                        var liveEntryCreationTryCount = that.phantomEntries[e.entryId] || 0;
                        if (!that.phantomEntries[e.entryId]) {
                            logger.info("[%s] Adding entry to phantom entries list", e.entryId);
                        }

                        that.phantomEntries[e.entryId] = liveEntryCreationTryCount + 1;
                        logger.info("[%s] Entry creation retry count: %s", e.entryId, that.phantomEntries[e.entryId]);
                        logger.info("[%s] Removing entry from the handled entries list", e.entryId);
                        delete that.handledEntries[e.entryId];
                    });
            }
        });
    ...
    function handleEntries(){
        ...
        return adapter.getLiveEntries()
            .then(function(liveEntries) {
                ...
                logger.debug("Entries received from Wowza: %j", JSON.stringify(relevantEntries));
