        ...
        return sessionManager.isNewSession(e.entryId, logger)
            ...
            .then(function(isNewSession) {
                    ...
                    return liveEntry.start()
                        ...
                        .catch(function(err) {
                            logger.debug("Live entry [%s] Failed to start", e.entryId);
                            if (err){
                                err.entry = liveEntry;
                            }
                            return Q.reject(err);
            ...
            .then(function(entry) {
                if (that.handledEntries[e.entryId] !== 'cancelPending') {
                    ...
                    that.endListAppender.removeEntry(e.entryId) // Entry is alive
                        .catch(function(err){
                            logger.error("[%s] Error adding entry to endlist appender processing - %s", e.entryId,  ErrorUtils.printError(err));
                        });
                }
                else {
                    ...
                    entry.stop()
                    .catch(function(err) {
                        logger.error("[%s] Aborting liveEntry object creation - %s", e.entryId, ErrorUtils.printError(err));
                    })
                    .then(function() {
        ...
        _.forEach(unhandledLiveEntries, function(e) {
            if (!that.handledEntries[e.entryId] && (!that.phantomEntries[e.entryId] || (now - that.phantomEntries[e.entryId] > timeToCoolDown))) {
                ...
                createLiveEntry.call(that, e)
                    ...
                    function(err) {
                        logger.error("[%s] Error starting entry object: %s", e.entryId, ErrorUtils.printError(err));
                        if (that.handledEntries[e.entryId] === 'cancelPending') {
                ...
                return liveEntry.stop()
                    .then(function() {
                        ...
                        return that.endListAppender.addEntry(noLongerLiveEntry) // Indicate entry is down and should (later) have end-list appended
                            .catch(function (err) {
                                logger.error("[%s] Error adding entry to endlistAppender processing - %s", noLongerLiveEntry, ErrorUtils.printError(err));
                            })
                            .finally(function () {
