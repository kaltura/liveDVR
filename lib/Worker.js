        ...
        return sessionManager.isNewSession(e.entryId, logger)
            ...
            .then(function(isNewSession) {
                    ...
                    return liveEntry.start()
                        ...
                        .catch(function(err) {
                            logger.error("Live entry [%s] Failed to start", e.entryId);
                            if (_.isObject(err)){
                                err.entry = liveEntry;
                            }
                            return Q.reject(err);
            ...
            .then(function(entry) {
                if (that.handledEntries[e.entryId] !== 'cancelPending') {
                    ...
                    that.endListAppender.removeEntry(e.entryId) // Entry is alive
                        .catch(function(err){
                            logger.error("[%s] Error adding entry to endlist appender processing - %s", e.entryId,  ErrorUtils.error2string(err));
                        });
                }
                else {
