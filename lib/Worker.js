...
module.exports = (function(){
    ...
    var createEntryDownloader = function(e){
        ...
        var cleanupEntryFolderForNewSession = function(){
            ...
            return qio.isDirectory(entryFolderPath).then(function (res) {
                if (res) {
                    ...
                    var renamedFolderName = entryFolderPath + '_DELETE';
                    var renameFolder = qio.rename(entryFolderPath, renamedFolderName);
                    renameFolder.then(function(){
                        return qio.removeTree(renamedFolderName);
                    }).catch(function(err){
                        logger.debug('Error deleting renamed folder ' + renamedFolderName + ': ' + err);
                    });
                    return renameFolder;
                }
            }).then(function () {
        ...
        return sessionManager.isNewSession(e.entryId)
            ...
            .then(function() {
                var entryDownloader = new entryDownloaderCtor(e.entryId, that.hostname, that.port, that.applicationName, e.dvrWindow, that.maxChunkCount);
