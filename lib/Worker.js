        ...
        _.forEach(unhandledLiveEntries, function(e) {
            if (!that.handledEntries[e.entryId] && (!that.phantomEntries[e.entryId] || (now - that.phantomEntries[e.entryId] > timeToCoolDown))) {
                ...
                that.endListAppender.removeEntry(e.entryId)
                    .catch(function(err) {
                        logger.error("[%s] Error removing entry to endlistAppender: %s", ErrorUtils.error2string(err));
                    });
                ...
                return liveEntry.stop()
                    .then(function() {
                        ...
                        return that.endListAppender.addEntry(noLongerLiveEntry) // Indicate entry is down and should (later) have end-list appended
                            .catch(function (err) {
                                logger.error("[%s] Error adding entry to endlistAppender processing - %s", noLongerLiveEntry, ErrorUtils.error2string(err));
                            })
                            .finally(function () {
