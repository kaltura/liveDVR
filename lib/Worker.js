...
module.exports = (function(){
    ...
    function Worker(prefix){
        var self=this;
        this.logger= loggerDecorator(logger, "[Worker] ");
        this.prefix = prefix;
        ...
        this.monitorServer = http.createServer(function(request, response) {
            try {
                ...
                this.logger.info("Exception returning response to monitor server", e, e.stack);
            }
        }).listen(4380+prefix);
    }
    ...
    function createLiveEntry(e){
        ...
        /*var onLiveEntryStopped = function(entryId){
            var that = this;
            that.logger.info("Entry %s notified it stopped - removing it from the list of handled entries", entryId);
        ...
        return sessionManager.isNewSession(e.entryId, logger)
            ...
            .then(function(entry) {
                if (that.handledEntries[e.entryId] !== 'cancelPending') {
                    that.logger.debug("adding entry %s to list of handled entries", e.entryId);
                    that.handledEntries[e.entryId] = entry;
                    //entry.on('stopped', onLiveEntryStopped.bind(that));
                    that.logger.debug("removing entry %s from endlistAppender", e.entryId);
                    that.endListAppender.removeEntry(e.entryId) // Entry is alive
                        .catch(function(err){
                            that.logger.error("Error adding entry to endlist appender processing - %s", err.message);
                        });
                }
                else {
                    that.logger.info("Entry creation aborted for entry %s", e.entryId);
                    entry.stop()
                        .catch(function(err) {
                            that.logger.error("error aborting liveEntry object creation - %s", err);
                        })
                        .finally(function() {
                            that.logger.info("Removing liveEntry object for entry %s from handled entries list", e.entryId);
    ...
    function processNewLiveEntries(currentlyLiveEntries){
        ...
        _.forEach(unhandledLiveEntries, function(e) {
            ...
            if (!that.handledEntries[e.entryId] && (!that.phantomEntries[e.entryId] || that.phantomEntries[e.entryId] < maxEntryInitializationTryCount)) {
                ...
                that.logger.info("Entry %s started playing - creating an entry object for it", e.entryId);
                createLiveEntry.call(that, e)
                    .then(function() {
                        ...
                        if (that.phantomEntries[e.entryId]) {
                            that.logger.info("Successfully created liveEntry object for entry %s removing it from the phantom list", e.entryId);
                            delete that.phantomEntries[e.entryId];
                        }
                    },
                    function(err) {
                        that.logger.error("Error starting entry object for %s: %s, %s", e.entryId, err.message, err.stack);
                        ...
                        if (!that.phantomEntries[e.entryId]) {
                            that.logger.info("Adding entry %s to the phantom entries list", e.entryId);
                        }

                        that.phantomEntries[e.entryId] = liveEntryCreationTryCount + 1;
                        that.logger.info("Entry %s creation retry count: %s", e.entryId, that.phantomEntries[e.entryId]);
                        that.logger.info("Removing entry %s from the handled entries list", e.entryId);
                        delete that.handledEntries[e.entryId];
                    }
                );
            }
    ...
    function processNoLongerLivePhantomEntries(currentlyLiveEntries){
        ...
        _.each(noLongerLivePhantomEntries, function(pe){
            that.logger.info('Phantom entry ' + pe + ' is no longer "live" - removing it from the phantom entries list');
    ...
    function processNoLongerLiveEntries(currentlyLiveEntries){
        ...
        _.forEach(noLongerLiveEntries, function (noLongerLiveEntry) {
            ...
            if (liveEntry === 'pending') {
                that.logger.info("Entry %s status changed to not live before an entry object could be created for it. Entry object creation cancellation pending", noLongerLiveEntry);
                that.handledEntries[noLongerLiveEntry] = 'cancelPending';
            }
            else if (liveEntry === 'cancelPending') {
                that.logger.info("Entry object stop requested for entry %s when creation cancellation for it is pending", noLongerLiveEntry);
            }
            else {
                that.logger.info("[%s] Entry is no longer live, stopping it", noLongerLiveEntry);
                return liveEntry.stop()
                    .then(function() {
                        that.logger.debug("[%s] Removing entry from list of handled entries", noLongerLiveEntry);
                        delete that.handledEntries[noLongerLiveEntry];
                        that.logger.debug("[%s] Adding entry to endlistAppender", noLongerLiveEntry);
                        return that.endListAppender.addEntry(noLongerLiveEntry) // Indicate entry is down and should (later) have end-list appended
                            .catch(function (error) {
                                that.logger.error("[%s] Error adding entry to endlistAppender processing - %s", noLongerLiveEntry, error.message);
                            })
                            .then(function () {
                                that.logger.info("[%s] Removing master manifest", noLongerLiveEntry);
                    ...
                    .catch(function() {
                        that.logger.debug("[%s] Waiting %s seconds grace period", noLongerLiveEntry, config.get('flavorDownloaderTeardownInterval')/1000);
                    })
                    .done();
            }
        });
    ...
    function handleEntries(){
        ...
        return adapter.getLiveEntries()
            .then(function(liveEntries) {
                ...
                that.logger.debug('relevant live entries: ' + JSON.stringify(relevantEntries));
                return relevantEntries;
            })
            .then(function(liveEntries){
                if (that.shouldRun) {
                    that.logger.debug("handled live entries: %j", _.keys(that.handledEntries));
                    var phantomEntryIds = Object.keys(that.phantomEntries);
                    that.logger.debug("phantom live entries: %j" , phantomEntryIds);
                    that.logger.debug("Active flavor downloaders: %j" , flavorDownloaderCtor.activeInstances);
