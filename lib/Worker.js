        ...
        return sessionManager.isNewSession(e.entryId)
            ...
            .then(function(entry) {
                if (that.handledEntries[e.entryId] !== 'cancelPending') {
                    ...
                    logger.info("Entry creation aborted for entry " + e.entryId);
                    entry.stop()
                        .catch(function(err) {
                            logger.error('error aborting liveEntry object creation - ' + err);
                        })
                        .finally(function() {
                            logger.info("Removing liveEntry object for entry " + e.entryId + " from handled entries list");
    ...
    var processNoLongerLiveEntries = function(currentlyLiveEntries){
        ...
        _.forEach(noLongerLiveEntries, function (noLongerLiveEntry) {
            ...
            if (liveEntry === 'pending') {
                logger.info("Entry " + noLongerLiveEntry + " status changed to not live before an entry object could be created for it. Entry object creation cancellation pending");
                that.handledEntries[noLongerLiveEntry] = 'cancelPending';
            }
            else if (liveEntry === 'cancelPending') {
                logger.info("Entry object stop requested for entry " + noLongerLiveEntry + " when creation cancellation for it is pending");
            }
            else {
                return sessionManager.ShouldCallLiveEntryStop(noLongerLiveEntry)
                    .then(function (ShouldCallLiveEntryStop) {
                        if (ShouldCallLiveEntryStop) {
                            Q.fcall(function () {
                                    logger.info("Entry " + noLongerLiveEntry + " is no longer live - calling liveEntry.stop");
                                    return liveEntry.stop();
                                })
                                .catch(function (err) {
                                    logger.error('Error stopping liveEntry for ' + noLongerLiveEntry + " err: " + err);
                                })
                                .then(function () {
                            ...
                            logger.debug("Entry " + noLongerLiveEntry + " is not live, but wait grace period before calling stop");
                        }
                    });
            }
        });
