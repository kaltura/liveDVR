...
var nginxLive = require('./nginxLive');
            ...
            return downloadFlavorManifest().catch(function (err) {
                ...
                if (manifestUpdatePromiseResult.state === 'fulfilled') {
                    ...
                    if (currentManifest.items.PlaylistItem.length > 0) {
                        ...
                        nginxLiveInput.offset = nginxLiveInput.offset + removedChunksDuration;
                        var nginxInput = nginxLive.generateInput(that.downloadedChunks, that.destFolderPath, currentManifest);
                        resultPromises.push(qfs.write(path.join(that.destFolderPath, that.flavor + "_audio.json"), util.inspect(nginxInput.audio, {depth:null})));
                        resultPromises.push(qfs.write(path.join(that.destFolderPath, that.flavor + "_video.json"), util.inspect(nginxInput.video, {depth:null})));
                    }
                }
                return results;
