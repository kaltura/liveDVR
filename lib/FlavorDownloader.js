...
var FlavorDownloader = (function () {
    ...
    function FlavorDownloader(m3u8Url, destPath, entryId, flavor, newPlaylistName, dvrWindow, maxChunkCount) {
        var that = this;
        ...
        this.m3u8generator = twoPhasedChunklistManifestGenerator(downloadedChunksGetter, this.destFolderPath, newPlaylistName, dvrWindow, maxChunkCount, this.logger);
    }
    ...
    var downloadChunks = function (that, baseUrl, chunksToDownload) {
        return Q.allSettled(_.map(chunksToDownload, function (chunkName) {
            ...
            return that.httpUtils.downloadFile(chunkUrl, path.join(that.destFolderPath, chunkName))
                .then(function(){
                    that.chunksToDownload = _.without(that.chunksToDownload, chunkName);
                    that.downloadedChunks.push(chunkName);
                });
    ...
    var downloadPlaylist = function (that) {
        ...
        var parsedUrl, baseUrl, playlistName, playlistDestPath;
        ...
        var updateListOfNewChunksToProcess = function(downloadedM3u8) {
            var newChunks = _.chain(downloadedM3u8.items.PlaylistItem)
                .filter(function (item) {
                    return !_.contains(that.downloadedChunks, item.get('uri')) && !_.contains(that.chunksToDownload, item.get('uri'));
                }).value();

            // update that these new chunks need to be downloaded
            _.each(newChunks, function (c) {
                that.chunksToDownload.push(c.get('uri'));
            });
        ...
        var processObsoleteChunks = function(results) {
            ...
            if (manifestUpdatePromiseResult.state === 'fulfilled')
            {
                ...
                _.each(manifestUpdatePromiseResult.value.removedChunks, function(chunk){
                    that.downloadedChunks = _.without(that.downloadedChunks, chunk.get('uri'));
                })
            }
        }
        ...
        return Q.fcall(function(){
            ...
            var allDownloadPendingChunks = that.chunksToDownload.concat(newChunkNames);
        ...
        return qfs.makeTree(this.tmpM3u8Folder)
            ...
            .then(function(){
                ...
                return pGlob(path.join(that.destFolderPath, '*.ts')).then(function(downloadedChunks){
                    that.downloadedChunks = _.map(downloadedChunks, function(c){
