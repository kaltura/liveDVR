            ...
            return downloadFlavorManifest().catch(function (err) {
                ...
                if (manifestUpdatePromiseResult.state === 'fulfilled') {
                    ...
                    if (currentManifest.items.PlaylistItem.length > 0) {
                        ...
                        qfs.write(path.join(that.destFolderPath, that.flavor + "_audio.json"), util.inspect(nginxInput.audio, {depth:null})).done();
                        qfs.write(path.join(that.destFolderPath, that.flavor + "_video.json"), util.inspect(nginxInput.video, {depth:null})).done();
                    }
                }
                return results;
