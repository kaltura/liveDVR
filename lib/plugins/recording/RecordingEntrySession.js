...
class RecordingEntrySession {
    ...
    constructor(config, entryObj, flavorsObjArray){
        ...
        const createPlaylistGenerator = (entryObj, flavors) => {
            ...
            let options = {
                ...
                'recordingMaxDurationInMs' : this.recordingMaxDurationInMs
            }
            if (flavors) {
        ...
        this.recordingUpdateEntryTimeIntervalMs = config.recordingUpdateEntryTimeIntervalInMin * 60 * 1000
        this.recordingRootPath = config.recordingFolderPath;
        this.recordingMaxDurationInMs = config.recordingMaxDurationInHours * 60 * 60 * 1000 - 5*60*1000;
        this.modeDirName = modeDirName
        let recordingSessionPath  = path.join(this.recordingRootPath, modeDirName, entryId.slice(-1), entryId, entryObj.recordedEntryId)
        this.flavors = []
        ...
        this.recordingSessionDuration = config.recordingSessionDurationInSec * 1000;
    ...
    updateEntryDuration(force){
        ...
        if (force || duration>this.lastUpdateDuration + this.recordingUpdateEntryTimeIntervalMs) {
    ...
    changeRecordedEntry(recordedEntryId){
        ...
        this.recordingSessionPath  = path.join(this.recordingRootPath, this.modeDirName, this.entryId.slice(-1), this.entryId, recordedEntryId)
