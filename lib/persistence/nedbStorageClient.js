...
module.exports = (function(){
    ...
    var addEntryIfAbsent = function(entryId, flavors){
        return entryExists(entryId).then(function(exists){
            if (!exists) {
                var objToInsert = {
                    _id : entryId,
                    id : entryId,
                ...
                _.each(flavors, function(f){
                    objToInsert.downloadedChunks[f] = [];
                    objToInsert.chunksToDownload[f] = [];
                })
    ...
    var entryExists = function(entryId){
        return pFindOne({
            _id : entryId
        }).then(function(doc){
            return !!doc;
    ...
    return {
        addEntryIfAbsent : addEntryIfAbsent,
        getEntry : getEntry,
