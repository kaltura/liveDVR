...
var EntryDownloader = (function () {
    ...
    function EntryDownloader(entryObject, hostname, port, applicationName, isNewSession) {
        ...
        this.manifestGenerator = flavorsBasedManifest(entryObject.entryId, hostname, port, applicationName, logger, entryObject.flavorParamsIds.split(','));
    ...
    var checkTranscodingProfileChange = function(entryId, flavorsPromiseObj) {
        ...
        return qio.listDirectoryTree(entryPath)
            ...
            .then(function(result) {
                var playingFlavorsString = flavorsPromiseObj.flavorsList.join();
                if (result.join() !== playingFlavorsString) {
                    logger.info("Transcoding profile changed, cleaning entry " + entryId + "folder");
                    return fsUtils.cleanFolder(entryPath);
                }
            })
            .then(function() {
                return flavorsPromiseObj;
            })
    ...
    var savePlaylist = function (flavorsPromiseObj, entryId) {
        ...
        return fsUtils.writeFileAtomically(destPath, flavorsPromiseObj.playlist)
            .then(function() {
                return flavorsPromiseObj.flavorsObj
            })
    };
