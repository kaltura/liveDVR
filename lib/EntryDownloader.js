...
var nginxLiveEntry = require('./nginxLiveEntry');
...
var EntryDownloader = (function () {
    ...
    function EntryDownloader(entryId, hostname, port, applicationName, manifestTimeWindow, manifestMaxChunkCount) {
        ...
        this.masterManifestGenerator = masterManifestGeneratorCreator(entryId, hostname, port, applicationName, logger);
    }
    ...
    EntryDownloader.prototype.start = function () {
        ...
        return that.masterManifestGenerator.getAllFlavors()
            .then(function(flavors) {
                ...
                var entryFolderPath = persistenceFormat.getEntryFullPath(that.entryId);
                setInterval(function() {
                    nginxLiveEntry.generateInput(that.flavorsDownloaders)
                        .then(function (entryData) {
                            return qfs.write(path.join(entryFolderPath, that.entryId + ".json"), JSON.stringify(entryData));
                        })
                        .catch(function (err) {
                            logger.error('Error generating nginx entry input: ' + err);
                        })
                }, config.get('nginxEntryInputCreationInterval'));
            });
    };

    EntryDownloader.prototype.stop = function () {
        var that = this;
        logger.info("Entry stop requested for entry " + this.entryId);
