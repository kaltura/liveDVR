...
class PushManager {
        ...
        if (!this.registeredObjects[objectId] || !this.registeredObjects[objectId].templates[templateName] || !this.registeredObjects[objectId].templates[templateName].init)
        {
            BackendClient.registerToPush(templateName, partnerId, params).then((registerResult) => {
                ...
                if (this.socket)
                {
                    ...
                    this.registeredObjects[objectId].templates[templateName] = {'init': false};
                    this.registeredQueues[registerResult.queueKey] = {'objectId': objectId, 'templateName': templateName, 'deferredObject': deferred};
    ...
    _initSocket(url)
    {
        ...
        this.socket.on('connected', (queueKey, queueKeyHash) =>
        {
            ...
            let objectId = this.registeredQueues[queueKeyHash].objectId;
            let templateName = this.registeredQueues[queueKeyHash].templateName;
            ...
            this.registeredQueues[queueKey] = queueKeyHash;
            deferred.resolve();
        });
    }
    ...
    _messageReceived(queueKey, msg)
    {
        ...
        {
            let queueKeyHash = this.registeredQueues[queueKey];
