...
var simpleLogger = require('./logger/logger')(module);
var path = require('path');
var touch = require('touch');
var loggerDecorator = require('./utils/log-decorator');
module.exports = (function(){
    var sessionFileName = "sessionTimestamp";
    var logger= loggerDecorator(simpleLogger, "[SessionManager] ");
    ...
    function lastTimestampSessionModification(lastModificationTime, isFromDisk, entryId) {
        ...
        var message;
        if (isFromDisk) {
            logger.debug("Checking if this is a new session for entry %s", entryId);
            timeToWait = config.get('sessionDuration');
            message = "Is this a new session result: ";
        }
        else {
            logger.debug("Checking if to call 'stop' for entry %s", entryId);
        ...
        logger.debug("Entry %s was last live at %s; time passed = %d sec; Grace period duration: %d seconds", entryId, lastModificationTime, (currentTime - lastModificationTime)/1000, timeToWait/1000);
        ...
        return Q.fcall(function() {
            ...
            .then(function(timestampFileDetails) {
                if (!timestampFileDetails) {
                    logger.info("No timestamp file found in entry folder - starting a new session for entry %s!", entryId);
                    return true;
                }
