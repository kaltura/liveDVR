...
const numOfFrameRateSamples = config.get('diagnostics').numOfFrameRateSamples;
const decimalDigitsCount = config.get('diagnostics').decimalDigitsForFrameRateCalculation;
...
var FlavorDownloader = (function() {
    ...
    function FlavorDownloader(flavorObj, entryObj, playlistGenerator, recordingEnable) {
        ...
        this.mediaInfo = { actualFramerateArray: [] };
    ...
    var downloadIndefinitely = function(onIterationEndCallback) {
        ...
        return downloadPlaylist.call(that)
            .catch(function(err) {
                ...
                that.logger.error("iteration error: %s", util.inspect(err));
                    ...
                    return m3u8Handler.parseM3U8(result.content,{ verbatim: true})
                        ...
                        .then(function(results) {
                            ...
                            if (results.newMp4Files.length > 0) {
                                ...
                                {
                                    if (that.entryObj.recordingStatus == kalturaTypes.KalturaRecordingStatus.ACTIVE)
                                    {
                                        try
                                        {
                                            useSavedChunks.call(that);
                                    ...
                                    {
                                        that.savedMp4Files.push(results.newMp4Files);
                                        while (that.savedMp4Files.length > config.get('recordingNotStartedMaxQueueSize'))
                                            that.savedMp4Files.shift();
                                    }
                                }
                            }
    ...
    function useSavedChunks(){
        const that = this;
        if (this.recordingStartTime)
        {
            this.logger.debug(`About to add saved chunks ${this.savedMp4Files.length}`);
            for (let mp4Files of this.savedMp4Files)
            {
                let mp4FilesToUse = _.filter(mp4Files, (currMp4file)=> {
                    let currDuration = 0;
                    if (currMp4file.video)
                    {
                        currDuration = currMp4file.video.duration;
                    }
                    else if (currMp4file.audio)
                    {
                        currDuration = currMp4file.audio.duration;
                    }
                    return ((currMp4file.startTime + currDuration) > that.recordingStartTime)
                });
                RecordingManager.addNewChunks(mp4FilesToUse, this.entryId, this.flavor)
                    .catch((err)=>
                    {
                        that.logger.error("Error while trying to run RecordingManager.addNewChunks on saved chunks %s", ErrorUtils.error2string(err));
                    });
            }
            this.savedMp4Files = [];
            this.recordingStartTime = false;
    ...
    FlavorDownloader.prototype.releaseSavedChunks = function(useFromSecs) {
        if ( this.recordingStartTime === false)
        {
            this.recordingStartTime = useFromSecs * 1000;
        }
    }
