...
var FlavorDownloader = (function() {
    ...
    function FlavorDownloader(flavorObj, entryObj, destPath, playlistGenerator) {
        ...
        this.recordStatus = entryObj.recordStatus ===1 && flavorObj.name ==='32';
        if (this.recordStatus){
            this.liveSessionExpirationDuration = entryObj.liveSessionExpirationDuration;
        }
                    ...
                    return m3u8Handler.parseM3U8(result.content,{ verbatim: true})
                        ...
                        .then(function(results) {
                            ...
                            if (results.newMp4Files.length > 0) {
                                ...
                                if (that.recordStatus !==kalturaTypes.KalturaRecordStatus.DISABLED ){ //if recording is enable, and flavor is source, send event
                                    try{
                                        RecordingManager.addNewChunks(results.newMp4Files, that.entryId, that.liveSessionExpirationDuration);
                                    } catch (err) {
