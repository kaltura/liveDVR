...
var FlavorDownloader = (function() {
    ...
    function FlavorDownloader(flavorObj, entryObj, playlistGenerator) {
        ...
        if (this.recordStatus !==kalturaTypes.KalturaRecordStatus.DISABLED && entryObj.serverType === kalturaTypes.KalturaEntryServerNodeType.LIVE_PRIMARY){
            this.recordingEntrtyObj = entryObj; //todo check only primary recording
        }
                        ...
                        .then(function(results) {
                            ...
                            if (results.newMp4Files.length > 0) {
                                ...
                                if (that.recordStatus !==kalturaTypes.KalturaRecordStatus.DISABLED ){ //if recording is enable, and flavor is source, send event
                                    try{
                                        RecordingManager.addNewChunks(results.newMp4Files, that.entryId, that.flavor, duration / 1000);
                                    } catch (err) {
    ...
    FlavorDownloader.prototype.start = function (onIterationEndCallback) {
        ...
        return qfs.makeTree(this.tmpM3u8Folder)
            ...
            .then(function () {
                if (that.recordStatus !== kalturaTypes.KalturaRecordStatus.DISABLED ) { //if recording is enable, and flavor is source, send event
                  return  RecordingManager.startRedording(that.recordingEntrtyObj, that.flavor) //todo promise? its not depending on nothing !!! so dont wait faild ? what to do ?
                }
            })
            .then(function() {
