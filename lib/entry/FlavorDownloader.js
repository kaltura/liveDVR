...
var FlavorDownloader = (function() {
                ...
                .then(function(result) {
                    return m3u8Handler.parseM3U8(result.content,{ verbatim: true})
                        ...
                        .then(function(mp4Files){
                            ...
                            let encodedBitrate = getEncodedBitrate.call(that);
                            var playlistGeneratorPromises = that.playlistGenerator.update(validMp4files, encodedBitrate);
    ...
    function getEncodedBitrate() {
        if (this.encoderInformation) {
            return {
                videoBitrate : this.encoderInformation.Encoder.videoBitrate,
                audioBitrate : this.encoderInformation.Encoder.audioBitrate
            }
        }
        return null;
    }

    function getTotalBitrate() {
        let bitrateObj = getEncodedBitrate.call(this);
        if (bitrateObj) {
            return bitrateObj.videoBitrate + bitrateObj.audioBitrate;
        }
        else {
            return 'N/A';
        }
    ...
    function diagnosticsPushData(mp4Data) {
        ...
        this.mediaInfo.bitrate_kbps = getTotalBitrate.call(this);
