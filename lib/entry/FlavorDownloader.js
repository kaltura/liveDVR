...
var FlavorDownloader = (function() {
    ...
    var downloadChunks = function(baseUrl, downloadDesc) {
        var that = this;
        return Q.allSettled(_.map(downloadDesc.newChunks, function(newChunk) {
            var chunkUrl = url.resolve(baseUrl + '/', newChunk.uri),
                chunkName = newChunk.cmpURI;
                ...
                .then(function({fileFullPath, hash}) {
                    ...
                    return that.tsHttpUtils.downloadConvert(chunkUrl, path.join(fileFullPath, chunkName))
                        .then(function(mp4File) {
                            ...
                            if(downloadDesc.m3u8.properties.targetDuration) {
                                mp4File.targetDuration = downloadDesc.m3u8.properties.targetDuration * 1000;
                                mp4File.windowSize = downloadDesc.m3u8.items.PlaylistItem.length * mp4File.targetDuration * 2;
                            }
    ...
    var downloadPlaylist = function() {
        ...
        var updateListOfNewChunksToProcess = function(downloadedM3u8) {
            var newChunks = _.chain(downloadedM3u8.items.PlaylistItem)
                ...
                .filter((newChunk) => {
                    return !that.downloadedChunks[newChunk.cmpURI] && !that.chunksToDownload[newChunk.cmpURI];
                })
                .value();
            ...
            _.each(newChunks, function (newChunk) {
                ...
                that.chunksToDownload[newChunk.cmpURI] = true;
            });
        ...
        var processObsoleteChunks = function(results) {
            ...
            if (manifestUpdatePromiseResult.state === 'fulfilled') {
                ...
                _.each(manifestUpdatePromiseResult.value.removedChunks, function (chunkName) {
                    ...
                    chunkName = persistenceFormat.getTSChunknameFromMP4FileName(chunkName);
                ...
                .then(function(result) {
                    return m3u8Handler.parseM3U8(result.content,{ verbatim: true})
                        ...
                        .then(function(newChunks) {
                            ...
                            return downloadChunks.call(that, baseUrl, newChunks);
                        })
