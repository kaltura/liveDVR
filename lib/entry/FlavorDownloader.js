...
var FlavorDownloader = (function() {
    ...
    function FlavorDownloader(flavorObj, entryObj, playlistGenerator, recordingEnable) {
        ...
        this.preserveHlsPath = preserveOriginalHLS ? persistenceFormat.getFlavorPreserveHlsFullPath(this.entryId, this.flavor) : null;

        var lastM3U8Content = null;
        this._lastM3U8Time = 0;

        this.tsHttpUtils = httpUtils.HttpDownloader(this.loggerInformation, undefined, this.preserveHlsPath);
    ...
    var downloadIndefinitely = function(onIterationEndCallback) {
        ...
        return downloadPlaylist.call(that)
            .catch(function(err) {
                ...
                that.logger.error(`Download indefinitely iteration ended with error. Error: ${ErrorUtils.error2string(err)}`);
            ...
            .finally(function() {
                ...
                if (that.shouldRun) {
                    ...
                    that.logger.warn("should not run, hence cancelled scheduling downloadIndefinitely!!!");
                    downloaderEnd.resolve();
                }
            });
    };
/*
    function saveChunklist(m3u, result) {
        let that = this;
        if (that.preserveHlsPath) {
            that.logger.debug('saveChunklist');
            if (result && m3u) {
                let chunklistPath = `${that.preserveHlsPath}/chunklist_${m3u.properties.mediaSequence}.m3u8`;

                return qio.write(chunklistPath, result.content.toString())
                    .catch((err) => {
                        that.logger.error(`Failed to save chunklist to ${chunklistPath}. Error: ${ErrorUtils.error2string(err)}`);
                    })
                    .finally(() => {
                        return m3u;
                    });
            } else {
                that.logger.error(`Failed to save chunklist, bad or undefined content`);
                return m3u;
            }
        } else {
            return m3u;
    ...
    function processConversionError (err,chunkName,preventDownloadTimout) {
        ...
        try {
            ...
            if (err instanceof MP4ConversionError) {
                that.logger.error("Error in MP4 conversion: %s, chunkName: %s", ErrorUtils.error2string(err), chunkName);
                let alertObj = new diagnosticsAlerts.TsConversionFailureAlert(that.flavor, err.message, chunkName);
        ...
        return Q.allSettled(_.map(chunks, function(chunkName) {
            ...
            return persistenceFormat.createHierarchyPath(that.destFolderPath, "flavor", that.lastFilePathHash)
                .then(function({fullPath, hash}) {
                    ...
                    let tsPath = that.preserveHlsPath ? path.join(that.preserveHlsPath, chunkName) : null;
                    //return the promise of the downloaded chunk
                    return that.tsHttpUtils.downloadConvert(chunkUrl, path.join(fullPath, chunkName), null, tsPath)
    ...
    function downloadPlaylist() {
        ...
        var downloadFlavorManifest = function() {
            ...
            let playlistPreservePath = that.preserveHlsPath ? path.join(that.preserveHlsPath, playlistName) : null;
            return that.m3u8HttpUtils.downloadFile(that.streamUrl, playlistDestPath, playlistPreservePath).then(function(content) {
                    ...
                    return m3u8Handler.parseM3U8(result.content,{ verbatim: true})
                        .then(function(m3u) {
                            if (result) {
                                ...
                                that.logger.error(`Received M3U8: error, failed to parse chunklist`);
                            }
                            //checkM3U8.call(that, m3u);

                            //return saveChunklist.call(that, m3u, result.content);
                            return m3u;
                        })
                        .then(function(m3u) {
                        ...
                        .then(function(mp4Files){
                            ...
                            that.logger.info('calling playlistGenerator.update');
    ...
    FlavorDownloader.prototype.start = function (onIterationEndCallback) {
        ...
        return qfs.makeTree(this.tmpM3u8Folder)
            ...
            .then(() => {
                if (that.preserveHlsPath) {
                    // preserving hls stream files. Clean path or create if doesn't exist
                    return qio.removeTree(that.preserveHlsPath)
                        .catch(err => {
                            if (err.code != 'ENOENT') {
                                that.logger.error(`cannot clean ${that.preserveHlsPath}. Error: ${ErrorUtils.error2string(err)}`);
                            } else {
                                return qio.makeTree(that.preserveHlsPath)
                                    .catch(err => {
                                        that.logger.error(`failed to create path for hls preservation, ${that.preserveHlsPath}. Error: ${ErrorUtils.error2string(err)}`);
                                    });
                            }
                        });
                }
