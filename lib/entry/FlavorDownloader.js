...
var FlavorDownloader = (function() {
    ...
    function FlavorDownloader(flavorObj, entryObj, playlistGenerator) {
        ...
        this.loggerInformation = preserveOriginalHLS ? `@@@< HLS PRESERVATION MODE >@@@ ` + this.loggerInformation : this.loggerInformation;
        this.logger =  loggerModule.getLogger("FlavorDownloader",  this.loggerInformation);
        // sequence number used to save chunklists for automation tests
        this.internalSequence = -1;
        this.testAdapter = preserveOriginalHLS ? require('./../Adapters/TestAdapter.js').TestAdapter.instance : undefined;
                    ...
                    return m3u8Handler.parseM3U8(result.content,{ verbatim: true})
                        .then(function(m3u) {
                            ...
                            if (preserveOriginalHLS) {
                                fs.renameSync(result.path, util.format("%s/chunklist_%d.m3u8", that.destFolderPath, m3u.properties.mediaSequence));
                            }
                            return m3u;
                        })
                        .then(function(m3u) {
    ...
    FlavorDownloader.prototype.saveChunklist = function(m3u, resultFullPath) {

        let deferred = Q.defer();
        this.internalSequence = this.internalSequence != -1 ? this.internalSequence + 1 : m3u.properties.mediaSequence;
        let preservedChunlistFullPath = `${this.destFolderPath}/chunklist_${this.internalSequence}.m3u8`;

        qfs.rename(resultFullPath, util.format(preservedChunlistFullPath))
            .then(() => {
                deferred.resolve(m3u);
            })
            .catch((err) => {
                //todo chnage sequence number and impl below
                // log error
                this.logger.error(`failed to save ${preservedChunlistFullPath}. Error: [${ErrorUtils.error2string(err)}]`);
                // emit error event to adapter
                this.testAdapter.emit('exit', err);
                deferred.reject(err);
            });
