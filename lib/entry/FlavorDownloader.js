...
const m3u8Handler = require('../manifest/promise-m3u8');
const util = require("util");
const events = require("events");
const config = require('../../common/Configuration');
const glob = require('glob');
const httpUtils=require('../utils/http-utils');
const MP4ConversionError = require('../MP4WriteStream').ConversionError;
...
const sourceFlavorId = config.get('flavorSourceId');
...
var FlavorDownloader = (function() {
    ...
    function FlavorDownloader(flavorObj, entryObj, playlistGenerator, recordingEnable) {
        ...
        this.reportedAlertsFlags = {
            isKeyframeIntervalOn: false,
            isHighFpsOn: false
        };
    ...
    function processConversionError (err,chunkName,preventDownloadTimeout) {
        var that = this;
        ...
        try {
            ...
            that.logger.info("wait %d msec before removing chunk %s from downloadedChunks list", preventDownloadTimeout, chunkName );
            Q.delay(preventDownloadTimeout).then(()=> {
    ...
    function inspectMp4FileAlerts(mp4File) {
        // Perform check only for source flavor
        // Keep track of which alerts were reported to diagnostics module, and only if in case
        // alert was reported, notify diagnostics to remove it.
        if (this.flavor === sourceFlavorId) {
            let reportedAlertsObj = { report: [], remove: [] };

            let keyFrameInterval = mp4File.metaData.keyFrameDistance;
            let keyFrameIntervalAlert = new diagnosticsAlerts.InvalidKeyFrameInterval(mp4File.flavor, mp4File.chunkName, keyFrameIntervalExpected / 1000, Math.round(keyFrameInterval) / 1000);
            if (Math.abs(keyFrameIntervalExpected - keyFrameInterval) > keyFrameIntervalDiffAllowed) {
                this.logger.warn(`Invalid keyFrame interval of [${keyFrameInterval}] on chunk [${mp4File.chunkName}]. Reporting alert!`);
                reportedAlertsObj.report.push(keyFrameIntervalAlert);
                this.reportedAlertsFlags.isKeyframeIntervalOn = true;
            }
            else if (this.reportedAlertsFlags.isKeyframeIntervalOn) {
                reportedAlertsObj.remove.push(keyFrameIntervalAlert);
                this.reportedAlertsFlags.isKeyframeIntervalOn = false;
            }

            let highFpsAlert = new diagnosticsAlerts.HighFpsRate(mp4File.flavor, mp4File.chunkName, mp4File.metaData.framerate);
            if (mp4File.metaData.framerate > maxFramesPerSeconds) {
                this.logger.warn(`Frame rate is [${mp4File.metaData.framerate}] - higher than max recommended [${maxFramesPerSeconds}]. Reporting alert!`);
                reportedAlertsObj.report.push(highFpsAlert);
                this.reportedAlertsFlags.isHighFpsOn = true;
            }
            else if (this.reportedAlertsFlags.isHighFpsOn) {
                reportedAlertsObj.remove.push(highFpsAlert);
                this.reportedAlertsFlags.isHighFpsOn = false;
            }

            if (reportedAlertsObj.report.length || reportedAlertsObj.remove.length) {
                this.emit('updateBeacons', reportedAlertsObj);
            }
        }
    }
                ...
                .then(function({fullPath, hash}) {
                    ...
                    return that.tsHttpUtils.downloadConvert(chunkUrl, path.join(fullPath, chunkName))
                        .then(function(mp4File) {
                            ...
                            if(mp4File.metaData) {
                                // Check for alerts on mp4 file
                                inspectMp4FileAlerts.call(that, mp4File);
