...
const keyFrameIntervalConfigured = config.get('diagnostics').keyFrameIntervalConfiguredInMSec;
...
var FlavorDownloader = (function() {
    ...
    function inspectMp4FileAlerts(mp4File) {
        ...
        if (Math.abs(keyFrameIntervalConfigured - keyFrameInterval) > keyFrameIntervalDiffAllowed) {
            this.logger.warn(`Invalid keyFrame interval of [${keyFrameInterval}] on chunk [${mp4File.chunkName}]. Reporting alert!`);
            alertsToReport.push(new diagnosticsAlerts.InvalidKeyFrameInterval(mp4File.flavor, mp4File.chunkName, keyFrameIntervalConfigured, keyFrameInterval));
        }
        if (mp4File.metaData.framerate > maxFramesPerSeconds) {
            ...
            alertsToReport.push(new diagnosticsAlerts.HighFpsRate(mp4File.flavor, mp4File.chunkName, mp4File.metaData.framerate))
        }

        this.emit('updateBeacons', { report: alertsToReport });
    }
