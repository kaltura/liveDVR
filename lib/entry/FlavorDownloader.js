...
var FlavorDownloader = (function() {
    ...
    FlavorDownloader.prototype.setLanguage = function(language) {
        this.language = language;
        this.playlistGenerator.setSequenceLanguage(this.flavor, language);

        //if recording is enabled apply audio language to recording playlist as well
        if (that.recordingEnable) { 
            try{
                RecordingManager.setAudioLanguage(that.entryId, that.flavor, language);
            } catch (err) {
                that.logger.error("Error while trying to run RecordingManager.updateLang %s", ErrorUtils.error2string(err))
            }
        }
    };
