...
function processObsoleteChunks(removedChunks) {
    ...
    _.each(removedChunks, function (chunkName) {

        if(that.downloadedChunks[chunkName]) {
            delete this.downloadedChunks[chunkName];
            if (this.chunksToDownload[chunkName]) {
                this.logger.error("Removing chunk " + chunkName + " from chunksToDownload - this shouldn't happen");
...
var FlavorDownloader = (function() {
    ...
    function FlavorDownloader(flavorObj, entryObj, playlistGenerator, recordingEnable) {
        ...
        this.playlistGenerator.flvOn('plyst-clips-removed',processObsoleteChunks.bind(this));
