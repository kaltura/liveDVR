...
var FlavorDownloader = (function () {
    ...
    function FlavorDownloader(m3u8Url, destPath, entryId, flavor, newPlaylistName, manifestTimeWindow, manifestMaxChunkCount) {
        ...
        this.shouldRun = true;
        this.runStatus = 'uninitialized';       //TODO: need to check all runStatus, also shouldRun
    ...
    var downloadIndefinitely = function (onIterationEndCallback) {
        ...
        return downloadPlaylist.call(that)
                ...
                if (shouldAbort) {
                    ...
                    that.stop().catch(function (err) {
                        ...
                        that.m3u8generator.remove()
                            .catch(function (err) {
                                that.logger.error("Failed to remove manifest: " + err);
                            })
                    });
            ...
            .then(function () {
                if (that.shouldRun) {
                    that.logger.debug('scheduling downloadIndefinitely to be run in %s ms', that.pollingInterval);
                    setTimeout(function () {
    ...
    var downloadChunks = function(baseUrl, chunksToDownload) {
        ...
        return Q.allSettled(_.map(chunksToDownload, function(chunkName) {
            ...
            .then(function(results) {
                ...
                var numOfPromisesFulfilled = 0;
                var rejectedPromises = _.filter(results, function(p) {
                        if (p.state === 'fulfilled') {
                            numOfPromisesFulfilled++;
                            return false;
                        }
                        return true;
                    });
                // TODO: Decide whether to raise event per file or for an array of files
                this.emit('newTsFiles', numOfPromisesFulfilled);
