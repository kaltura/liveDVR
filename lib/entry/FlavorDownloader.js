...
var FlavorDownloader = (function() {
    ...
    function FlavorDownloader(flavorObj, entryObj, destPath, playlistGenerator) {
        var that = this;
    ...
    var downloadChunks = function(baseUrl, chunksToDownload) {
        ...
        return Q.allSettled(_.map(chunksToDownload, function(chunkName) {
            var chunkUrl = url.resolve(baseUrl + '/', persistenceFormat.getTSfromMP4chunkName(chunkName));

            //return the promise of the downloaded chunk
            return that.tsHttpUtils.downloadConvert(chunkUrl, path.join(that.destFolderPath, chunkName))
    ...
    var removeFile = function (pathToDelete) {
        ...
        return qfs.remove(pathToDelete)
            .then(function () {
                that.logger.info('Successfully deleted chunk ' + path.basename(pathToDelete));
            }).catch(function () {
                that.logger.info('Error deleting a chunk which was removed from a manifest - ' + path.basename(pathToDelete));
            });
    };
    ...
    var downloadPlaylist = function() {
        ...
        var updateListOfNewChunksToProcess = function(downloadedM3u8) {
            var newChunks = _.chain(downloadedM3u8.items.PlaylistItem)
                .map(function(item){
                    return persistenceFormat.getMP4fromTSchunkName(item.get('uri'));
                })
        ...
        var processObsoleteChunks = function(results) {
            ...
            if (manifestUpdatePromiseResult.state === 'fulfilled') {
                ...
                _.each(manifestUpdatePromiseResult.value.removedChunks, function (itemToDelete) {
                    ...
                    var pathToDelete = path.join(that.destFolderPath, itemToDelete);
                ...
                _.each(manifestUpdatePromiseResult.value.removedChunks, function (chunkName) {
                    that.logger.debug('Delete ' + chunkName + ' from the list of chunks to download');
                ...
                .then(function(playlistDestPath) {
                    return m3u8Handler.parseM3U8(playlistDestPath)
                        ...
                        .then(function(newChunks) {
                            ...
                            return downloadChunks.call(that, baseUrl, Object.keys(that.chunksToDownload));
                        })
                        .then(function(mp4Files){
                            // generate updated playlist
                            var playlistGeneratorPromise = that.playlistGenerator.update(mp4Files);
                            return Q.allSettled(playlistGeneratorPromise);
                        })
                        .then(function(results) {
                            // notify of ready files
                            var cc = that.playlistGenerator.getChunkCount();
                            if ( cc > 0) {
                                that.logger.info("Finished downloading new chunks, raising 'newTsFiles' event");
                                that.emit('newTsFiles', that, cc);
                            }
                            return results;
                        })
                        .then(function(results) {
    ...
    FlavorDownloader.prototype.start = function (onIterationEndCallback) {
        ...
        that.logger.debug("Polling interval: %s seconds", this.pollingInterval/1000);
        ...
        return qfs.makeTree(this.tmpM3u8Folder)
            ...
            .catch(function(err){
                that.logger.error("Error occurred while starting flavor downloader: %s", err);
