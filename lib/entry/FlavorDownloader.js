...
const errorsInfoWindowSize = config.get('diagnostics').errorsWindowSizeInMSec;
...
var FlavorDownloader = (function() {
    ...
    function FlavorDownloader(flavorObj, entryObj, playlistGenerator) {
        ...
        this.playlistGenerator.on('diagnosticsAlert', handleDiagnosticsAlert.bind(this));
            ...
            return persistenceFormat.createHierarchyPath(that.destFolderPath, that.lastFilePathHash)
                ...
                .catch(function(err) {
                    ...
                    if(err instanceof MP4ConversionError) {
                        ...
                        delete that.chunksToDownload[chunkName];
                        handleDiagnosticsAlert.call(that, new diagnosticsAlerts.TsConversionFailureAlert(err), that.flavor);
    ...
    function checkM3U8(m3u8) {
        ...
        _.each(m3u8.items.PlaylistItem, (chunk) => {
            if (chunk.get('duration') < config.get('minChunkDurationInSec') || chunk.get('duration') > config.get('maxChunkDurationInSec')) {
                ...
                handleDiagnosticsAlert.call(that, 
                    new diagnosticsAlerts.InvalidM3u8Alert({ chunkName : chunk.get('uri'), Duration : chunk.get('duration') }),
                    this.flavor);
            }
        })
    }
    ...
    function diagnosticsCheckRollingWindow() {
        ...
        while (this.alerts.length > 0) {
            ...
            if (currTime - this.alerts[0].time < errorsInfoWindowSize) {
                break;
            }
            this.alerts.pop();
        }
    ...
    function handleDiagnosticsAlert(diagnosticsAlertObj, flavor) {
        if (flavor === this.flavor) {
            this.alerts = this.alerts || [];

            diagnosticsCheckRollingWindow.call(this);
            this.alerts.push(diagnosticsAlertObj);
        }
    }
