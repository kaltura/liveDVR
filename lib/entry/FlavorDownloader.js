...
var FlavorDownloader = (function() {
    ...
    function FlavorDownloader(flavorObj, entryObj, playlistGenerator, recordingEnable) {
        ...
        this.successPollingInterval;
        this.failPollingInterval;
    ...
    function initPollingIntervals (entryObj) {
        if (_.isNumber(entryObj.segmentDuration)) {
            this.successPollingInterval = Math.floor(entryObj.segmentDuration/5);
            this.failPollingInterval = Math.floor(entryObj.segmentDuration/2);
        } else {
        ...
        // limit highest stream download intensity
        this.successPollingInterval = flavorMinPollingInterval < this.successPollingInterval ? this.successPollingInterval : flavorMinPollingInterval;
        this.failPollingInterval = flavorMinPollingInterval < this.failPollingInterval ? this.failPollingInterval : flavorMinPollingInterval;

        // limit lowest stream download intensity
        this.successPollingInterval = flavorMaxPollingInterval > this.successPollingInterval ? this.successPollingInterval : flavorMaxPollingInterval;
        this.failPollingInterval = flavorMaxPollingInterval > this.failPollingInterval ? this.failPollingInterval : flavorMaxPollingInterval;

        this.logger.debug("[segmentDuration: %d] flavor download interval milliseconds:[success: %d] [failure: %d]", entryObj.segmentDuration, this.successPollingInterval, this.failPollingInterval);
