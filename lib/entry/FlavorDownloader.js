                    ...
                    return m3u8Handler.parseM3U8(result.content,{ verbatim: true})
                        .then(function(m3u) {
                            ...
                            if (preserveOriginalHLS) {
                               ...
                               let chunklistPath = util.format("%s/chunklist_%d.m3u8", that.destFolderPath, m3u.properties.mediaSequence);
                               fs.rename(result.path, chunklistPath, (err) => {
                                   if (err) {
                                       that.logger.error(`failed to preserve playlist ${chunklistPath}. Error ${ErrorUtils.error2string(err)}`);
                                   } else {
                                       let fileSaveTime = Date.now() - startTime;
                                       that.logger.debug(`Saving file ${chunklistPath} took ${fileSaveTime} ms`);
                                   }
                               });
