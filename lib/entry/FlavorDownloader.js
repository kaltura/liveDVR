...
var FlavorDownloader = (function() {
    ...
    downloadIndefinitely = function (onIterationEndCallback) {
        ...
        return downloadPlaylist.call(that)
            .catch(function failure(err) {
                ...
                that.logger.error("iteration error: %s", ErrorUtils.error2string(err));
    ...
    var downloadChunks = function(baseUrl, chunksToDownload) {
        ...
        return Q.allSettled(_.map(chunksToDownload, function(chunkName) {
            ...
            .then(function(results) {
                ...
                var failed=_.filter(results, function(p) { return (p.state !== 'fulfilled') });

                if (failed.length > 0) {
                    //group list of errors:
                    var errorsList = _.map(failed, function(i) {
                        return i.reason;
                    });
                    that.logger.error("Error occurred while downloading Chunks: %s", errorsList);
                }
            });
    };
    ...
    var downloadPlaylist = function() {
        ...
        var updateListOfNewChunksToProcess = function(downloadedM3u8) {
            ...
            if (_.keys(that.chunksToDownload).length >= maxAmountOfDownloadPendingChunks) {
