...
var FlavorDownloader = (function() {
            ...
            return persistenceFormat.createHierarchyPath(that.destFolderPath, that.lastFilePathHash)
                ...
                .catch(function(err) {
                    ...
                    if(err instanceof MP4ConversionError) {
                        ...
                        delete that.chunksToDownload[chunkName];
                        let alertObj = new diagnosticsAlerts.TsConversionFailureAlert({ flavor : that.flavor, errObj : err, chunkName : err.chunkName });
    ...
    function checkM3U8(m3u8) {
        ...
        _.each(m3u8.items.PlaylistItem, (chunk) => {
            let minChunkDurationInSec = config.get('diagnostics').minChunkDurationInMSec / 1000;
            let maxChunkDurationInSec = config.get('diagnostics').maxChunkDurationInMSec / 1000;
            if (chunk.get('duration') < minChunkDurationInSec || chunk.get('duration') > maxChunkDurationInSec) {
                ...
                let alertObj = new diagnosticsAlerts.InvalidM3u8Alert({ Flavor :  that.flavor, chunkName : chunk.get('uri'), Duration : chunk.get('duration') });
                that.emit('diagnosticsAlert', alertObj);
            }
        })
    }
    ...
    function diagnosticsPushData(mp4Data) {
        ...
        this.mediaInfo.lastChunkPath = path.dirname(mp4Data.path);
    }
