...
var diagnosticsErrorCodes = {
    "noID3tag" : 0,
    "ptsMisalignment" : 1,
    "mp4ConversionFailure" : 2,
    "emptyFile" : 3,
    "invalidM3u8" : 4,
    properties : {
        0 : "No ID3 Tag",
        1 : "PTS Misalignment",
        2 : "MP4 Conversion Failure",
        3 : "Empty File",
        4 : "Invalid M3U8"
    }
};
...
var FlavorDownloader = (function() {
    ...
    function FlavorDownloader(flavorObj, entryObj, playlistGenerator) {
        ...
        initializeDiagnosticsEvents.call(this);
        this.mediaInfo = {};
    ...
    function initializeDiagnosticsEvents() {
        this.playlistGenerator.on('diagnosticsPushError', onDiagnosticsPushError.bind(this));
    }
    ...
    function downloadChunks(baseUrl, chunksToDownload, downloadedM3u8) {
        var that = this;
            ...
            return persistenceFormat.createHierarchyPath(that.destFolderPath, that.lastFilePathHash)
                ...
                .catch(function(err) {
                    ...
                    if(err instanceof MP4ConversionError) {
                        let errObject = {
                            code : diagnosticsErrorCodes.mp4ConversionFailure,
                            time : new Date(),
                            arguments : {
                                errorMsg : err,
                                chunk : err.chunkName
                        ...
                        delete that.chunksToDownload[chunkName];
                        onDiagnosticsPushError.call(that, errObject);
    ...
    function checkM3U8(m3u8) {
        ...
        _.each(m3u8.items.PlaylistItem, (chunk) => {
            if (chunk.duration < config.get('minChunkDurationInSec') || chunk.duration > config.get('maxChunkDurationInSec')) {
                let msg = "Invalid Manifest! Chunk: " + chunk.get('uri') + " has an invalid duration of [" + chunk.get('duration') + " sec]";
                diagnosticsPushError.call(that, msg);
            }
        })
    }
    ...
    function diagnosticsPushData(data) {
        //this.mediaInfo = _.clone(data.metaData);
    ...
    function diagnosticsPushError(error, time, msg) {
        let that = this;
        if (!this.errorsInfo) {
            this.errorsInfo = [];
        }
        that.errorInfo = _.filter(that.errorsInfo, (e) => {
            let currTime = new Date();
            // Display only the errors that occurred in the last hour
            // TODO: Change back the window size and move it to config.template -> Gad
            return (currTime - e.Time < 3600*1000);
        });
        that.errorsInfo.push({ Error : error, Time : time, Message : msg});
    }

    function onDiagnosticsPushError(diagnosticsErrorObj) {
        let that = this;
        let errorCode = diagnosticsErrorCodes;
        let msg;

        switch (diagnosticsErrorObj.code) {
            case errorCode.noID3tag:
                msg = "No ID3 tag in chunk " + diagnosticsErrorObj.arguments.chunk;
                break;
            case errorCode.mp4ConversionFailure:
                msg = diagnosticsErrorObj.arguments.errorMsg + "; " + diagnosticsErrorObj.arguments.chunk;
                break;
        }
        
        diagnosticsPushError.call(that, errorCode.properties[diagnosticsErrorObj.code], diagnosticsErrorObj.time, msg);
    }
