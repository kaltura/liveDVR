...
var FlavorDownloader = (function() {
    ...
    function FlavorDownloader(flavorObj, entryObj, destPath, newPlaylistName) {
        ...
        this.m3u8generator = twoPhasedChunklistManifestGenerator(downloadedChunksGetter, this.destFolderPath, newPlaylistName, entryObj.dvrWindow, entryObj.maxChunkCount, this.loggerInformation);
            ...
            return downloadFlavorManifest()
                ...
                .then(function(result) {
                    return m3u8Handler.parseM3U8(result.content,{ verbatim: true})
                        ...
                        .then(function(results) {
                            // TODO: Think if to raise event only if manifest was updated, to avoid calling for nothing -> Gad
                            var newItemsCount= results[1].value.newItemsCount;
                            if (newItemsCount > 0) {
