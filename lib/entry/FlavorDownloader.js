...
var FlavorDownloader = (function() {
    ...
    function getTotalBitrate(fallbackBitrate) {
        if (this.encoderInformation && this.encoderInformation.Encoder) {
            // Support all types of stream playing: Regular, video only and audio only
            switch (checkStreamType.call(this)) {
                case 'full_stream':
                    return this.encoderInformation.Encoder.videoBitrate + this.encoderInformation.Encoder.audioBitrate;
                case 'video_only':
                    return this.encoderInformation.Encoder.videoBitrate;
                case 'audio_only':
                    return this.encoderInformation.Encoder.audioBitrate;
                default:
                    break;
            }
        }
    ...
    function checkStreamType() {
        if (this.encoderInformation.Encoder.videoBitrate && this.encoderInformation.Encoder.audioBitrate)
            return 'full_stream';
        else if (this.encoderInformation.Encoder.videoBitrate)
            return 'video_only';
        else
            return 'audio_only';
    ...
    function diagnosticsPushData(mp4Data) {
        ...
        this.mediaInfo.bitrate_kbps = getTotalBitrate.call(this, mp4Data.metaData.bitrate_kbps);
    ...
    FlavorDownloader.prototype.setEncodedBitrate = function(encoderInformation) {
        ...
        switch (checkStreamType.call(this)) {
            case 'full_stream':
                encoderBitrate.videoBitrate = this.encoderInformation.Encoder.videoBitrate * 1024;
                encoderBitrate.audioBitrate = this.encoderInformation.Encoder.audioBitrate * 1024;
                break;
            case 'video_only':
                encoderBitrate.videoBitrate = this.encoderInformation.Encoder.videoBitrate * 1024;
                break;
