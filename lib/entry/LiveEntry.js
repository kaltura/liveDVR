...
function LiveEntry(entryObject, isNewSession) {
    ...
    this.lastStreamInfoUpdate = null;
}
        ...
        if (this.recovered) {
            ...
            this.FSM.play()
                .finally(function() {
                    ...
                    if ((currTime - that.lastStreamInfoUpdate) < config.get('streaInfoUpdateInterval'))
                        updateStreamInfo();
                });
        }
    }
...
function updateStreamInfo() {
    ...
    return getEntryServerNodeId
        .then(function() {
            return backendClient.updateStreamInfo(that.entryId, that.entryServerNodeId, that.flavorsObjArray)
                .then(function(result) {
                    if (result) {
                        that.logger.debug("Flavors updated in DB for entry server node [%s]", that.entryId, that.entryServerNodeId);
                        that.lastStreamInfoUpdate = new Date().getTime();
                    }
                });
        })
        .catch(function(err) {
            that.logger.warn("entryServerNodeId not found, stream info will not updated error: %s",  ErrorUtils.error2string(err));
        });
}
    ...
    var getLiveEntryServerNodeIdPromise =
        that.entryServerNodeId ? Q.resolve()
            ...
            .then(function (liveEntryServerNodes) {
                ...
                return that.entryServerNodeId ? Q.resolve() : Q.reject("Couldn't find entryServerNodeId");
            })
            .catch(function (err) {
...
LiveEntry.prototype.stop = function() {
    ...
    return suspendPromise
        .then(function() {
            ...
            // Regardless of the stopping result controller must remove entry from handledEntries list
