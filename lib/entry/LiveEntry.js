...
LiveEntry.prototype._pushMessageReceived = function(msg)
{
    ...
    if (entry.viewMode == kalturaTypes.KalturaViewMode.ALLOW_ALL && origViewMode != kalturaTypes.KalturaViewMode.ALLOW_ALL)
    {
        this._startDVR(entry);
    }
    if (entry.recordingStatus == kalturaTypes.KalturaRecordingStatus.ACTIVE && origRecordingStatus != kalturaTypes.KalturaRecordingStatus.ACTIVE)
    {
        this._startRecording(entry);
    }
    else if (entry.recordingStatus == kalturaTypes.KalturaRecordingStatus.STOPPED && origRecordingStatus != kalturaTypes.KalturaRecordingStatus.STOPPED)
    {
...
LiveEntry.prototype._stopRecording = function(entry)
{
    ...
    RecordingManager.stopRecording(entry.id);
}
