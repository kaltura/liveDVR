...
function getEntryServerNodeId() {
    ...
    entryServerNodeFilter.serverTypeEqual = this.entryServerType;
    var getLiveEntryServerNodeIdPromise =
           BackendClient.getSingleEntryServerNode(this.entryId, entryServerNodeFilter)
            .then((entryServerNode)=> {
                this.entryServerNodeId = entryServerNode.id;
            })
            .catch(function (err) {
                // todo: check if set entryServerNodeId value to 0 here is correct???
                this.entryServerNodeId = 0;
                return Q.reject(err);
            });
