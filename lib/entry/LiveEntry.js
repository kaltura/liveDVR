...
function createFlavorDownloaders() {
    ...
    var promises = _.map(that.flavorsDownloaders, function(f) {
        ...
        f.on('diagnosticsAlert', handleDiagnosticsAlert.bind(that));
        return f.start();
    });
...
function handleDiagnosticsAlert(diagnosticsAlertObj ) {
    this.diagnostics.pushAlert(diagnosticsAlertObj);
}
...
LiveEntry.prototype.toJSON = function () {
    var diag = { 
                 ...
                 'alerts' : this.diagnostics.getAlerts() };
    return diag;
};
