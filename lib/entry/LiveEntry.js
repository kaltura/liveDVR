...
function LiveEntry(entryObject) {
    ...
    this.playlistGenerator.on('updateBeacons', this.diagnostics.updateBeacons.bind(this.diagnostics));
...
LiveEntry.prototype.start = function() {
    ...
    return this.playlistGenerator.initializeStart()
        ...
        .then(()=> {
            return this.getEntryServerNodeId()
              .catch((err)=> {
                  this.logger.warn(`failed to get entryServerNideId. Error: [${ ErrorUtils.error2string(err)}]`);
                  return Q.resolve();
              });
        })
        .then(()=> {
            if (this.recordingEnable) {
                    return RecordingManager.startRecording(this.entryObject, this.entryServerNodeId)
                        .then(()=> {
...
LiveEntry.prototype.stop = function() {
    ...
    let stopLiveInRecordingPromise;
    return suspendPromise
        .then(()=> {
            // Wait for all flavorDownloaders to stop
            this.suspended = true;
            if (this.recordingEnable) {
                stopLiveInRecordingPromise = RecordingManager.stopLive(this.entryId);
            } else {
                stopLiveInRecordingPromise = Q.resolve();
            }
            return stopLiveInRecordingPromise.then(()=> {
                    return this.cleanup();
                })
                .finally(()=> {
                    ...
                    return stopPromise
                        .catch((err)=> {
                            this.logger.error(`Error calling FSM.stop: ${ErrorUtils.error2string(err)}`);
                            return Q.resolve();
                        });
                })
        })
        .catch((err)=> {
            if (err == null) {
                this.logger(`It is not time to stop entry!`)
            }
            else {
                this.logger.error(`Error stopping entry: ${ErrorUtils.error2string(err)}`);
            }
            return Q.reject(err);
        });
};
...
LiveEntry.prototype.getEntryServerNodeId = ()=> {
    var getLiveEntryServerNodeIdPromise =
        this.entryServerNodeId ? Q.resolve()
            : BackendClient.getEntryServerNodeId(this.entryId)
            .then((entryServerNodeId)=> {
                this.entryServerNodeId = entryServerNodeId;
            })
            .catch((err)=> {
                this.logger.error(`failed retrieving serverEntryId. Error: ${ErrorUtils.error2string(err)}`);
                this.entryServerNodeId = null;
                return Q.reject(err);
            });
