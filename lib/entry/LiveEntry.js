...
function LiveEntry(entryObject, isNewSession) {
    ...
    this.recovered = entryObject.liveStatus === kalturaTypes.KalturaEntryServerNodeStatus.PLAYABLE;
...
LiveEntry.prototype.start = function() {
    ...
    return startPromise
        .then(function() {
             ...
             return that.playlistGenerator.createPlaylist();
        })
        ...
        .then(function(results) {
            ...
            if (errs.numErrors > 0) {
                ...
                that.logger.error("Failed  to start flavor downloaders for some of the flavors: %s ", ErrorUtils.printError(errs.err));
            }
        });
};
        ...
        .then(function() {
            ...
            return Q.all(flavorsPromises)
                .then(function() {
                    ...
                    return stopPromise
                        .catch(function(err) {
                            that.logger.error("Error calling FSM.stop: %s" ,ErrorUtils.printError(err));
                            return Q.resolve();
                        });
                })
                .catch(function(err) {
                    that.logger.error("Error stopping entry: %s" ,ErrorUtils.printError(err));
                    return Q.resolve();
                });
        })
        .catch(function(err) {
