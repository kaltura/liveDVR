...
function onNewTsFilesDownloaded(flavorObj, streamDurationAdditionInSec) {
    ...
    if (flavorObj === this.flavorsDownloaders[0]) {
        ...
        else if (flavorObj.isDirty)
        {
            this.logger.info("FlavorDownloader is dirty - going to play");
            flavorObj.isDirty = false;
            this.FSM.play();
        }
        else if (this.cumulativeDurationInSec.toFixed(2) >= this.startPlayingDurationSec && ((flavorObj.isDirty && this.FSM.current === "suspending") || this.FSM.current !== "suspending")) {
...
LiveEntry.prototype.entryObjectUpdated = function(entry, force = false)
{
    ...
    if (entry.viewMode == kalturaTypes.KalturaViewMode.ALLOW_ALL
        && (force || origViewMode != kalturaTypes.KalturaViewMode.ALLOW_ALL))
    {
