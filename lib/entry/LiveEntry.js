...
function LiveEntry(entryObject) {
    ...
    this.recordingEnable =  entryObject.recordStatus != kalturaTypes.KalturaRecordStatus.DISABLED  &&
        ...
        entryObject.serverType === kalturaTypes.KalturaEntryServerNodeType.LIVE_PRIMARY
}
        ...
        .then(function (flavorsObjArray) {
            ...
            if (that.recordingEnable) {
                  return RecordingManager.startRecording(that.entryObject, that.flavorsObjArray)
                      ...
                      .catch(()=>{
                          that.logger.warn("Start recording was failed, change recording enable to false")
            ...
            that.cleanup()
                .finally(function() {
                    ...
                    return stopPromise
                        .then(function () {
                            if (that.recordingEnable) {
                                return RecordingManager.updateServer(that.entryObject, that.flavorsObjArray)
                            }
                        })
                        .catch(function(err) {
