...
LiveEntry.prototype.entryObjectUpdated = function(entry)
{
        ...
        && origViewMode != kalturaTypes.KalturaViewMode.ALLOW_ALL)
    {
        ...
        const persistePlaylistPromise = this._removeClipTo();
        this._updateIsLiveUser(entry, true, persistePlaylistPromise);
    }
...
LiveEntry.prototype._updateIsLiveUser = function(entry, isLiveUser, persistePlaylistPromise)
{
    this.logger.debug(`Waiting for persist playlist to finis in order to update isLiveUser`);
    persistePlaylistPromise.then((result)=> {
        this.logger.info(`Updating is live user to [${isLiveUser}`);
        BackendClient.updateIsLiveUser(entry.id, isLiveUser);
