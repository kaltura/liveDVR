...
function updateFlavorsLanguage() {
    ...
    return BackendClient.getFlavorsMultiStream(this.entryObject, flavors)
        .then((result)=> {
            this.logger.debug(`Recieved MultiStream info from server`);
            _.each(this.flavorsObjArray, (f)=> {
                let flavorMultiStream = result[f.name];
                this.logger(`Flavor: ${f.name}, MultiStream: ${flavorMultiStream}`);
                if ((flavorMultiStream !== undefined) && (flavorMultiStream !== "")) {
        ...
        .catch((error)=> {
            this.logger.error("Error retrieving Multi Stream for flavors: %s", ErrorUtils.error2string(error));
        });
}
