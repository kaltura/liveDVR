...
LiveEntry.prototype.start = function() {
    ...
    return that.playlistGenerator.initializeStart()
        ...
        .then(function (flavorsObjArray) {
            ...
            if (that.entryObject.recordStatus !== kalturaTypes.KalturaRecordStatus.DISABLED  && that.entryObject.serverType === kalturaTypes.KalturaEntryServerNodeType.LIVE_PRIMARY ) { //if recording is enable
                return  RecordingManager.startRecording(that.entryObject, that.flavorsObjArray) //todo promise? its not depending on nothing !!! so dont wait faild ? what to do ?
            }
        })
        .then(function() {
    ...
    return suspendPromise
        .then(function() {
            ...
            return Q.all(flavorsPromises)
                .then(function() {
                    that.logger.info("Stopping PlaylistGenerator");
