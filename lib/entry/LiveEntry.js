...
function onNewTsFilesDownloaded(flavorObj, streamDurationAdditionInSec) {
    ...
    if (flavorObj === this.flavorsDownloaders[0]) {
        ...
        this.logger.debug("New files duration: [%d sec]; Total stream duration: [%d sec], min duration to update to PALYING [%d sec]" , streamDurationAdditionInSec.toFixed(2), this.cumulativeDurationInSec.toFixed(2), this.startPlayingDurationSec);
        if (this.recovered) {
            ...
            this.logger.info(`Report PLAYING to server ${this.startPlayingDurationSec} seconds since entry started playing`);
            this.FSM.play();
        }
    }
}
...
LiveEntry.prototype.update = function(diagnosticsObj) {
    ...
    this.logger.debug("calling \"this.FSM.update()\"");
    this.FSM.update();
};
