...
function getFlavorLanguage(flavor) {
    let that = this;
    return BackendClient.getFlavorExtraParams(that.entryObject, flavor.flavor)
        .then((result)=> {
            if (result !== "") {
                let flavorExtraParamsJson = JSON.parse(result);
                if (flavorExtraParamsJson.language) {
                    flavor.setLanguage(flavorExtraParamsJson.language);
                    that.isMultipleAudio = true;
                }
            }
        })
        .catch((error)=> {
            that.logger.error("Error checking Multiple Audio stream: %s", ErrorUtils.error2string(error));
        });
}
...
function createFlavorDownloaders() {
    ...
    that.flavorsDownloaders = _.chain(that.flavorsObjArray)
        ...
        .each((flavor)=> {
            // Iterate through all flavors and check if language is configured on flavor
            getFlavorLanguage.call(that, flavor);
        })
        .value();
