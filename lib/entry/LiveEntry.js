    ...
    return that.playlistGenerator.initializeStart()
        ...
        .then(function (flavorsObjArray) {
            ...
            if (that.recordingEnable) {
                    return RecordingManager.startRecording(that.entryObject, that.flavorsObjArray, that.transcodingProfileChanged).then(()=> {
                        that.logger.info("Successfully start recording")
                    })
                .catch((err)=> {
                    that.recordingEnable = false
                    that.logger.warn("Start recording has failed %s, changed \'recordingEnable\' to \'false\' ", ErrorUtils.error2string(err))
                })
            }
