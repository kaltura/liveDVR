...
let BackendClient = require('../BackendClientFactory.js').getBackendClient();
...
function updateFlavorsLanguage() {
    ...
    _.uniq(this.flavorsObjArray);
    let flavors = _.map(this.flavorsObjArray, (f)=> {
        return f.name;
    });
    return BackendClient.getFlavorsExtraParams(this.entryObject, flavors)
        .then((result)=> {
            _.each(this.flavorsObjArray, (f)=> {
                let flavorExtraParmas = result[f.name];
                if (flavorExtraParmas !== "") {
                    let flavorExtraParamsJson = JSON.parse(flavorExtraParmas);
                    if (flavorExtraParamsJson.language) {
                        f.language = flavorExtraParamsJson.language;
        ...
        .catch((error)=> {
            this.logger.error("Error retrieving Extra Params for flavors: %s", ErrorUtils.error2string(error));
        });
}

function createFlavorDownloaders() {
    this.flavorsDownloaders = _.map(this.flavorsObjArray, (flavor)=> {
...
LiveEntry.prototype.start = function() {
    ...
    return this.playlistGenerator.initializeStart()
        .then ((flavorsObjArray)=> {
            this.flavorsObjArray = flavorsObjArray;
        ...
        .then(()=> {
            if (this.recordingEnable) {
                    return RecordingManager.startRecording(this.entryObject, this.flavorsObjArray).then(()=> {
                        this.logger.info("Successfully start recording")
                    })
                .catch((err)=> {
                    this.logger.warn("Start recording was failed %s, change recording enable to false ", ErrorUtils.error2string(err));
