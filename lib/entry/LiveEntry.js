...
function onNewTsFilesDownloaded(numOfTsFilesDownloaded) {
    if (!this.playbackRaised) {
        if (numOfTsFilesDownloaded >= config.get('minChunksForPlayback')) {
            ...
            // TODO: Call onNotifyPlaying and raise isLive
        }
    }
}

// TODO: Change function to fit the updated flow of entry streaming
...
function createFlavorDownloaders(flavorsObjArray) {
    ...
    that.flavorsDownloaders = _.chain(flavorsObjArray)
        ...
        .map(function(flavor) {
            ...
            // TODO: Change flavorDownloaderCtr to receive the entryObj instead of thousand parameters
...
LiveEntry.prototype.stop = function() {
    ...
    that.stopPromise = Q.all(stopPromises).then(function() {
        ...
        // TODO: Can downloader variable be recognized in this scope?
    ...
    // TODO: determine the stopping process
    var p = that.FSM.stop();
