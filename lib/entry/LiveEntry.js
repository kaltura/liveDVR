...
function createFlavorDownloaders() {
    ...
    that.flavorsDownloaders = _.chain(that.flavorsObjArray)
        ...
        .map(function(flavor) {
            ...
            return new flavorDownloader(flavor, that.entryObject, that.playlistGenerator, that.recordingEnable, that.adapterDiagnosticsObj);
        })
        .value();
    //start the download
...
function saveFlavorDownloadersConfiguration() {
    _.each(this.flavorsDownloaders, (f)=> {
        f.encoderInformation = this.adapterDiagnosticsObj.outputs[f.flavor];
    })
}
