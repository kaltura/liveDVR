...
function LiveEntry(entryObject, isNewSession) {
    ...
    this.FSM = FSM(entryObject, this.masterManifestGenerator, this.logger);
    this.flavorsDownloaders = [];
}
util.inherits(LiveEntry, events.EventEmitter);

function onNewTsFilesDownloaded(flavor, numOfTsFilesDownloaded) {
    ...
    if ((flavor === '32') && (numOfTsFilesDownloaded >= config.get('minChunksForPlayback'))) {
