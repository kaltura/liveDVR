...
function LiveEntry(entryObject) {
    ...
    this.startPlayingBufferTimeSec = _.isNumber(entryObject.segmentDuration) ? 3*1000*entryObject.segmentDuration : config.get('minDurationForPlaybackInSec');
...
function onNewTsFilesDownloaded(flavorObj, streamDurationAdditionInSec) {
    ...
    if (flavorObj === this.flavorsDownloaders[0]) {
        ...
        else if (this.cumulativeDurationInSec >= this.startPlayingBufferTimeSec && this.FSM.current !== "suspending") {
