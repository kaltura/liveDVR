...
function LiveEntry(entryObject, isNewSession) {
    ...
    this.FSM = FSM(entryObject, this.playlistGenerator);
...
function onNewTsFilesDownloaded(flavorObj, numOfTsFilesDownloaded) {
    ...
    if (flavorObj === this.flavorsDownloaders[0]){
        ...
        this.logger.debug("onNewTsFilesDownloaded Number of new chunks added: %d, cumulative new chunks: %d" ,numOfTsFilesDownloaded ,this.cumulativeChunk);
        if ( this.cumulativeChunk >= config.get('minChunksForPlayback') && this.FSM.current !== "suspending") {
            this.logger.info("Entry contains %d chunks (min configured chunks: %d) report PLAYING to server, with state %s", this.cumulativeChunk ,config.get('minChunksForPlayback'), this.FSM.current);
            this.FSM.play();
        }
    }
