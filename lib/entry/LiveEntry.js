...
const flavorMinPollingInterval = config.get("flavorMinPollingInterval");
...
function LiveEntry(entryObject) {
    ...
    this.flavorSuccessPollingIntervalMilliseconds = config.get("flavorSuccessPollingInterval");
    this.flavorFailPollingIntervalMilliseconds = config.get("flavorFailPollingInterval");
...
function createFlavorDownloaders() {
    ...
    that.flavorsDownloaders = _.chain(that.flavorsObjArray)
        ...
        .map(function(flavor) {
            ...
            return new flavorDownloader(flavor, that.entryObject, that.playlistGenerator, that.recordingEnable, that.flavorSuccessPollingIntervalMilliseconds, that.flavorFailPollingIntervalMilliseconds);
        })
        .value();
    //start the download
...
LiveEntry.prototype.update = function(diagnosticsObj) {
    try {
        ...
        this.logger.error("Error occurred while running Diagnostics analysis: %s", ErrorUtils.error2string(error));
    }
    this.FSM.update();
};
...
LiveEntry.prototype.initPollingIntervals = function () {
    
    if (this.entryObject.segmentDuration) {
        this.flavorSuccessPollingIntervalMilliseconds = Math.floor(this.entryObject.segmentDuration/5);
        this.flavorFailPollingIntervalMilliseconds = Math.floor(this.entryObject.segmentDuration/2);
    }

    this.flavorSuccessPollingIntervalMilliseconds = this.flavorSuccessPollingIntervalMilliseconds < flavorMinPollingInterval ? flavorMinPollingInterval : this.flavorSuccessPollingIntervalMilliseconds;
    this.flavorFailPollingIntervalMilliseconds = this.flavorFailPollingIntervalMilliseconds < flavorMinPollingInterval ? flavorMinPollingInterval : this.flavorFailPollingIntervalMilliseconds;

    this.logger.debug("[segmentDuration: %d] flavor download interval milliseconds:[success: %d] [failure: %d]", this.entryObject.segmentDuration, this.flavorSuccessPollingIntervalMilliseconds, this.flavorFailPollingIntervalMilliseconds);
