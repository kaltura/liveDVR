...
{
    this.logger.debug(`Registering partner ${this.entryObject.partnerId} for push`);
...
LiveEntry.prototype.entryObjectUpdated = function(entry, firstTime = false)
{
    ...
    if (entry.viewMode == kalturaTypes.KalturaViewMode.ALLOW_ALL
        && (firstTime || origViewMode != kalturaTypes.KalturaViewMode.ALLOW_ALL) )
    {
        this._startDVR(entry);
        this._removeClipTo(entry);
    }
    if (entry.viewMode == kalturaTypes.KalturaViewMode.PREVIEW
        && (firstTime || origViewMode != kalturaTypes.KalturaViewMode.PREVIEW))
    {
        this._setClipTo(entry);
    }
    if (entry.recordingStatus == kalturaTypes.KalturaRecordingStatus.ACTIVE
        && (firstTime || origRecordingStatus != kalturaTypes.KalturaRecordingStatus.ACTIVE))
    {
        this._startRecording(entry);
    }
    else if (entry.recordingStatus == kalturaTypes.KalturaRecordingStatus.STOPPED
        && (firstTime || origRecordingStatus != kalturaTypes.KalturaRecordingStatus.STOPPED))
    {
...
LiveEntry.prototype._setClipTo = function(entry)
{
    this.playlistGenerator.setClipTo(new Date().getTime());
}

LiveEntry.prototype._removeClipTo = function(entry)
{
