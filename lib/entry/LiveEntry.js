...
function LiveEntry(entryObject) {
    ...
    this.transcodingProfileChanged = false;
    ...
    return this.playlistGenerator.initializeStart()
        ...
        .then(()=> {
            if (this.recordingEnable) {
                    return RecordingManager.startRecording(this.entryObject, this.flavorsObjArray, this.transcodingProfileChanged).then(()=> {
                        this.logger.info("Successfully start recording")
                    })
                .catch((err)=> {
                    this.logger.warn("Start recording has failed %s, changed \'recordingEnable\' to \'false\' ", ErrorUtils.error2string(err));
