...
function extractFlavorLanguage(flavorDownloader, extraParamsString) {
    let extraParamsJson = JSON.parse(extraParamsString);
    if (extraParamsJson.language) {
        flavorDownloader.setLanguage(extraParamsJson.language);
        this.isMultipleAudio = true;
    }
}

function getFlavorsLanguage() {
    let that = this;
    let flavors = _.map(this.flavorsDownloaders, (fd)=> {
        return fd.flavor;
    });
    return BackendClient.getFlavorsExtraParams(this.entryObject, flavors)
        .then((result)=> {
            _.each(that.flavorsDownloaders, (fd)=> {
                let flavorParams = _.find(result, (obj)=> {
                    return obj.id.toString() == fd.flavor;
                });
                if (flavorParams.extraParams !== "") {
                    extractFlavorLanguage.call(that, fd, flavorParams.extraParams);
                }
            });
        })
        .catch((error)=> {
            that.logger.error("Error retrieving Extra Params for flavors: %s", ErrorUtils.error2string(error));
        });
}
...
function createFlavorDownloaders() {
    ...
    return getFlavorsLanguage.call(that)
        .then(()=> {
            //start the download
            let promises = _.map(that.flavorsDownloaders, function(f) {
