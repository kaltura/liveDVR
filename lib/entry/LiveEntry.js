...
var PlaylistGenerator = require('./../manifest/PlaylistGenerator');
...
var playlist = require('../Playlist/PackagerPlayList');
...
function LiveEntry(entryObject, isNewSession) {
    ...
    this.playlistGenerator = new PlaylistGenerator(entryObject, this.logger);
    this.FSM = FSM(entryObject, this.playlistGenerator, this.logger);
...
function onNewTsFilesDownloaded(flavorObj, numOfTsFilesDownloaded) {
    ...
    if ((flavorObj === this.flavorsDownloaders[0]) && (numOfTsFilesDownloaded >= config.get('minChunksForPlayback'))) {
        this.logger.info("Entry contains %d chunks (min configured chunks: %d) report PLAYING to server", numOfTsFilesDownloaded,config.get('minChunksForPlayback'));
        this.FSM.play();
    }
}
...
function createFlavorDownloaders(flavorsObjArray) {
    ...
    that.flavorsDownloaders = _.chain(flavorsObjArray)
        ...
        .map(function(flavor) {
            ...
            return new flavorDownloader(flavor, that.entryObject, destPath, playlistGenerator);
        })
        .value();
    //start the download
    ...
    var startPromise = that.FSM.start();
        return startPromise[1]
            .then(function() {
                 ...
                 return that.playlistGenerator.start();
            })
