...
function LiveEntry(entryObject, isNewSession) {
    ...
    this.cumulativeDurationInMs = 0;
}
util.inherits(LiveEntry, events.EventEmitter);

function onNewTsFilesDownloaded(flavorObj, streamDurationAdditionInMs) {
    ...
    if (flavorObj === this.flavorsDownloaders[0]) {
        this.cumulativeDurationInMs = this.cumulativeDurationInMs + streamDurationAdditionInMs;
        this.logger.debug("New files' duration: [%d sec]; Total stream duration: [%d sec]" , streamDurationAdditionInMs / 1000 , this.cumulativeDurationInMs / 1000);
        if (this.recovered) {
            this.logger.info("Entry recovered after process crashed, change state to PLAYING");
            this.recovered = false;
            this.FSM.play();
        }
        else if (this.cumulativeDurationInMs >= config.get('minDurationForPlaybackInSec') && this.FSM.current !== "suspending") {
