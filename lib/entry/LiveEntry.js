...
function LiveEntry(entryObject) {
    ...
    this.observers = EventManager.getEvents(entryObject)
}
util.inherits(LiveEntry, events.EventEmitter);

function onNewTsFilesDownloaded(flavorObj, streamDurationAdditionInSec, newMp4Files) {
    ...
    // that entry is still alive every 60 seconds.
        try{
            triggerEvent.call(this, "addNewChunks",newMp4Files, flavorObj.entryId, flavorObj.flavor);
        } catch (err) { //todo fix it
            logger.error("Error while trying to run RecordingManager.addNewChunks %s", ErrorUtils.error2string(err))
        }
...
function triggerEvent(event, ...params) {
    ...
    _.each(this.observers, (observer)=>{
        if (observer.hasOwnProperty(event)) {
            let promise = observer[event](...params);
            promises.push(promise)
        }
    })
    return Q.all(promises) //todo since no error handling maybe better to retun resolve anyway
}
...
function createFlavorDownloaders() {
    ...
    that.flavorsDownloaders = _.chain(that.flavorsObjArray)
        ...
        .map(function(flavor) {
            ...
            return new flavorDownloader(flavor, that.entryObject, that.playlistGenerator);
        })
        .value();
    //start the download
...
LiveEntry.prototype.start = function() {
    ...
    return that.playlistGenerator.initializeStart()
        ...
        .then(function (flavorsObjArray) {
            ...
            return triggerEvent.call(that, "startLive", that.entryObject, that.flavorsObjArray)
                    .then(()=> {
                    that.logger.info("Successfully start recording") //todo fix it
                })
                .catch((err)=> {
...
LiveEntry.prototype.stop = function() {
    ...
    return triggerEvent.call(that, "suspendLive", that.entryObject, that.flavorsObjArray)
        .then(function () {
