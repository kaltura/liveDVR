...
function getEntryServerNodeId() {
    let getLiveEntryServerNodeIdPromise =
           BackendClient.getSingleEntryServerNode(this.entryId, this.entryServerType)
            .then((entryServerNode)=> {
                this.entryServerNodeId = entryServerNode.id;
            })
            .catch(function (err) {
                this.entryServerNodeId = null;
                return Q.reject(err);
...
LiveEntry.prototype.start = function() {
    ...
    return this.playlistGenerator.initializeStart()
        ...
        .then(()=> {
            return getEntryServerNodeId.call(this)
                .catch((err)=> {
                    this.logger.error(`failed to get server node id. Recording duration won't update. Error: ${ErrorUtils.error2string(err)}`);
                    return Q.resolve();
                });
        })
        .then(()=> {
            if (this.recordingEnable) {
                    return RecordingManager.startRecording(this.entryObject, this.entryServerNodeId).then(()=> {
