...
function getEntryServerNodeId() {
    var getLiveEntryServerNodeIdPromise =
        this.entryServerNodeId ? Q.resolve()
            : backendClient.getLiveEntryServerNodes(this.entryId)
            .then((liveEntryServerNodes)=> {
                _.each(liveEntryServerNodes, (liveEntryServerNode)=> {
                    //note that serverType and entryServerType can be string or int values
                    if (liveEntryServerNode.serverType == this.entryServerType) {
                        this.logger.info("Saving SeverNodeId for entry: [%s]", liveEntryServerNode.id);
                        this.entryServerNodeId = liveEntryServerNode.id;
                        return false;
                    }
                });
                return this.entryServerNodeId ? Q.resolve() : Q.reject("Couldn't find entryServerNodeId");
            })
            .catch(function (err) {
