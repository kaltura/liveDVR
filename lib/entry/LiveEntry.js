...
function onNewTsFilesDownloaded(numOfTsFilesDownloaded) {
    ...
    // that entry is still alive.
    if (!this.playbackRaised) {
        if (numOfTsFilesDownloaded >= config.get('minChunksForPlayback')) {
            logger.info("Entry contains " + config.get('minChunksForPlayback') + " chunks, calling onNotifyPlaying");
            this.playbackRaised = true;
            this.FSM.readyForPlaying();
        }
    }
    else {
        logger.info("Entry " + this.entryId + " is still alive, updating database");
        this.FSM.onupdate();
    }
