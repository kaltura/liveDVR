...
LiveEntry.prototype.start = function() {
    ...
    return this.playlistGenerator.initializeStart()
        ...
        .then(()=> {
            if (this.recordingEnable && (!this.entryObject.explicitLive || this.entryObject.recordingStatus == 2) ) {//@@NA replace 2 with enum from updated client
...
LiveEntry.prototype._registerToPush = function()
{
    ...
    this.logger.debug(`Registering partner ${this.entryObject.partnerId} for push with id [${this.entryObject.entryId}]`);
    pushMgr.registerEntry(this.entryObject.entryId, this.entryObject.partnerId, config.get('explicitLivePushNotificationTemplateName'), (...args) => this._pushMessageReceived(...args));
}

LiveEntry.prototype._pushMessageReceived = function(entry)
{
...
{
    this.entryObject.explicitLive = entry.explicitLive;
...
{
    this.logger.debug(`starting dvr [${entry.id}]`);
    const newPlayWindow = Math.ceil(this.entryObject.playWindow * 1000);
    if (newPlayWindow > this.playlistGenerator.playListLimits.manifestTimeWindowInMsec)
    {
        this.playlistGenerator.playListLimits.liveWindowDurationMsec = newPlayWindow;
        this.playlistGenerator.playListLimits.manifestTimeWindowInMsec = PlaylistGenerator.applyHLSWindowFactor(newPlayWindow);
        this.playlistGenerator.reset();
    }
    else
...
LiveEntry.prototype._startRecording = function(entry)
{
    ...
    if (this.entryObject.recordingStatus == 2) { //@@NA need to add recording status to the entryObject
        RecordingManager.startRecording(this.entryObject).then(()=> {
            logger.info(`Successfully started recording [${entry.id}]`);
        })
            .catch((err)=> {
                logger.warn("Start recording has failed %s, change recordingEnable status to false ", ErrorUtils.error2string(err));
            });
    }
}
