...
module.exports = function(liveEntry) {
    ...
    function checkSteramRedundancy(entryServerNodesList) {
        if (entryServerNodesList.length > 1) {
            // Only of both entry server nodes are not marked for deletion we can determine redundancy
            return entryServerNodesList.every(esn => esn.status !== kalturaTypes.KalturaEntryServerNodeStatus.MARKED_FOR_DELETION);
        }
        return false;
    }
    ...
    function updateEntryServerNodesInfo() {
        return backendClient.getLiveEntryServerNodes(liveEntry.entryId)
            .then(entryServerNodesList => {
                liveEntry.entryObject.isRedundancy = checkSteramRedundancy(entryServerNodesList);
