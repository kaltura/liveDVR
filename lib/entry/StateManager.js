...
module.exports = function(liveEntry) {
    ...
    function isNewSession() {
        let newSessionPromises = [];
        newSessionPromises.push(sessionManager.isNewSession(liveEntry.entryId));
        newSessionPromises.push(liveEntry.playlistGenerator.checkTranscodingProfileChange());

        return Q.any(newSessionPromises)
            .then(function() {
                logger.info("New session Started, Cleaning entry folder");
                return fsUtils.cleanFolder(persistenceFormat.getEntryBasePath(liveEntry.entryId), liveEntry.entryId)
                    .catch((error) => {
                        logger.error("Error cleaning folder because of transcoding profile change: %s", ErrorUtils.error2string(error));
                    })
                    .then(function() {
