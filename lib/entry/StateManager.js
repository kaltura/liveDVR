    ...
    function onbroadcast(event, from, to) {
        ...
        return (entryObject.liveStatus === 1) ? Q.resolve() : sync.exec(function() {
            return backendClient.registerEntryInDatabase(entryObject, KalturaLiveStatus.BROADCASTING, event)
                .catch(function(err) {
                    logger.error("Failed to register media server BROADCAST: %s", ErrorUtils.printError(err));
    ...
    function onstop(event, from, to) {
        ...
        return sync.exec(function() {
            return backendClient.unregisterEntryInDatabase(entryObject)
                .catch(function(err) {
                    logger.error("Failed to unregister from media server: %s", ErrorUtils.printError(err));
                    //TODO: what to do here? -> Gad
             });
        });
    }
                ...
                .then(function() {
                    ...
                    return sync.exec(function() {
                        return backendClient.registerEntryInDatabase(entryObject, KalturaLiveStatus.PLAYABLE, event)
                            .catch(function (err) {
                                logger.error("Failed to register media server PLAYING: %s", ErrorUtils.printError(err));
                                //TODO: what to do here? -> Gad
                            });
                    });
                })
                .catch(function (err) {
                    logger.error("Error refreshing timestamp for entry, err: %j stack: %s", ErrorUtils.printError(err));
                });
        }
    }
