...
module.exports = function(liveEntry) {
    ...
    function updateStreamInfo(currentTime) {
        let timeElapsedMsec = Math.abs(currentTime.getTime() - lastStreamInfoUpdateTime.getTime());
        
        if (timeElapsedMsec >= streamInfoUpdateInterval) {
    ...
    function getEntryServerNodeId() {
        ...
        var getLiveEntryServerNodeIdPromise =
            that.entryServerNodeId ? Q.resolve()
               : backendClient.getSingleEntryServerNode(liveEntry.entryId, liveEntry.entryServerType)
                    .then((entryServerNode)=> {
                        that.entryServerNodeId = entryServerNode.id;
                    })
                    .catch((err)=> {
                        logger.error(`failed retrieving serverEntryId. Error: ${ErrorUtils.error2string(err)}`);
