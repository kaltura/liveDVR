...
module.exports = function(liveEntry) {
        ...
        return checkNewSession()
            ...
            .then(function() {
                ...
                return (liveEntry.entryObject.liveStatus === 1) ? Q.resolve() : sync.exec(function() {
                    return backendClient.registerEntryInDatabase(liveEntry.entryObject, KalturaLiveStatus.BROADCASTING, event, msgId)
                        .then((entryObj) => {
    ...
    function onstop(event, from, to) {
        ...
        return sync.exec(function() {
            return backendClient.unregisterEntryInDatabase(liveEntry.entryObject, msgId)
                .catch(function(err) {
    ...
    function onplay(event, from, to) {
        ...
        if ((lastServerUpdate === null) || ((currTime - lastServerUpdate) >= serverUpdateInterval)) {
            ...
            return sessionManager.refreshSessionTimestamp(liveEntry.entryId)
                .then(function() {
                    return sync.exec(function() {
                        return updateStreamInfo(currTime, msgId)
                            .finally(function() {
                                return backendClient.registerEntryInDatabase(liveEntry.entryObject, KalturaLiveStatus.PLAYABLE, event, msgId)
                                    .then(function() {
                                        lastServerUpdate = new Date();
                                    })
                                    .catch(function (err) {
                                        logger.error("[msgId %s] Error calling registerEntryInDatabase - PLAY for entry. %s", msgId, ErrorUtils.error2string(err));
                                        //TODO: what to do here? -> Gad
                                    });
                            })
                    });
                ...
                .catch(function (err) {
                    logger.error("[msgId %s] Error refreshing timestamp for entry. %s", msgId, ErrorUtils.error2string(err));
                });
        }
    }
    ...
    function updateStreamInfo(currentTime, msgId) {
        if ((currentTime - lastStreamInfoUpdateTime) >= config.get('streamInfoUpdateInterval')) {
            return getEntryServerNodeId(msgId)
                .then(function () {
                    return backendClient.updateStreamInfo(liveEntry.entryId, liveEntry.entryServerNodeId, liveEntry.flavorsDownloaders, msgId)
                        .then(function() {
                            logger.debug("[msgId %s] Flavors updated in DB for entry server node [%s]", msgId, liveEntry.entryServerNodeId);
                            lastStreamInfoUpdateTime = new Date();
                            return Q.resolve();
                        });
                })
                .catch(function (err) {
                    logger.warn("[msgId %s] entryServerNodeId not found, stream info will not update! Error: %s", msgId, ErrorUtils.error2string(err));
    ...
    function getEntryServerNodeId(msgId) {
        var getLiveEntryServerNodeIdPromise =
            liveEntry.entryServerNodeId ? Q.resolve()
                : backendClient.getLiveEntryServerNodes(liveEntry.entryId, msgId)
