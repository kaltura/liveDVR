...
module.exports = function(liveEntry) {
    ...
    let lastStreamInfoUpdateTime = new Date(1970,1,1);
    ...
    function updateStreamInfo(currentTime) {
        let timeDiff = Math.abs(currentTime.getTime() - lastStreamInfoUpdateTime.getTime());

        if (+timeDiff >= +streamInfoUpdateInterval) {
            // todo: decide whether to remove or leave, since called directly from liveEntry
            // can the server node id change during live session after short stream stop start (without stopping session)?
            return getEntryServerNodeId()
                ...
                .catch(function (err) {
                    logger.warn("entryServerNodeId not found, stream info will not be updated! Error: %s", ErrorUtils.error2string(err));
                    // todo: check why is serverNodeId reset required here????
                    liveEntry.entryServerNodeId = null;
                    liveEntry.entryObject.serverNodeId = null;
                    return Q.reject(err);
                });
        }
        else {
    ...
    function getEntryServerNodeId() {
        var getLiveEntryServerNodeIdPromise =
            liveEntry.entryServerNodeId ? Q.resolve()
                ...
                .then(function (liveEntryServerNodes) {
                    _.each(liveEntryServerNodes, function (liveEntryServerNode) {
                        ...
                        if (liveEntryServerNode.serverType == liveEntry.entryServerType) {
                            ...
                            liveEntry.entryObject.serverNodeId = liveEntryServerNode.id;
                            return false;
                        }
                    });
                ...
                .catch(function (err) {
                    // todo: why is serverEntryNode reset required here??? + it's duplicate code
                    liveEntry.entryServerNodeId = 0;
                    liveEntry.entryObject.serverNodeId = 0;
                    return Q.reject(err);
                });
