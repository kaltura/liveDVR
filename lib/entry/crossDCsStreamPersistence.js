...
class CrossDCsStreamPersistence {
    ...
    constructor() {
        ...
        this._presentationEndTimeRegex = /("presentationEndTime":\s*)(\d+)/;
    ...
    streamPersistenceCheck(entryId, newEntry = false) {
        ...
        let ended = false;
        let now = Date.now();
        let nowStr = new Date(now).toLocaleString();
        let presentationEndTime = now + 7200000;
        let presentationTimeStr = new Date(presentationEndTime).toLocaleString();
        ...
        return backendClient.getLiveEntryServerNodes(entryId)
            ...
            .then((content) => {
                ...
                this._playlistsData[entryId].lastProcessedTime = now;
                if (newEntry) {
                    logger.debug(`====> [streamPersistenceCheck] [${entryId}] STOPPED STREAMING expiration time set to [${nowStr}]`);
                    playlist = playlist.replace(this._expireTimeRegex, `$1${now}`);
                }
                if (entryIsLive) {
                    logger.debug(`====> [streamPersistenceCheck] [${entryId}] is LIVE in other DC, presentation end time set to [${presentationTimeStr}]`);
                    playlist = playlist.replace(this._presentationEndTimeRegex, `$1${presentationEndTime}`);
                } else {
                    ended = true;
                    logger.debug(`====> [streamPersistenceCheck] [${entryId}] ENDED. Presentation end time set to [${nowStr}]`);
                    playlist = playlist.replace(this._presentationEndTimeRegex, `$1 ${now}`);
                }
                return playlist;
            })
            .then((playlist) => {
    ...
    addEntry(entryId) {
        ...
        if (this._entriesToProcess[entryId]) {
            ...
            return this.streamPersistenceCheck(entryId, true)
                .then((ended) => {
                    logger.info(`====> [addEntry][${entryId}] successfully added to stream persistence list (ended=${ended}), [playlistPath=${this._playlistsData[entryId]}]`);
    ...
    removeEntry(entryId) {
        ...
        if (this._entriesToProcess.hasOwnProperty(entryId)) {
            ...
            if (Object.keys(this._entriesToProcess).length === 0) {
                return this.flushEntryList(this._entriesToProcess)
                    .catch((err) => {
                        logger.info(`====> [removeEntry] error flushing stopped entries. Error: [${ErrorUtils.error2string(err)}]`);
                    })
                    .then(() => {
                      return Q.resolve(true);
                    });
    ...
    processStoppedEntries() {
        ...
        if (startTime - this._lastProcessedTimestampe >= this._persistenceCheckIntervalMillisec) {
            ...
            let pAll = _.map(this._entriesToProcess, (entryId) => {
                ...
                let time2EndStreamMillisec = this._playlistsData[entryId].lastProcessedTime - startTime;
