...
const entryPattern = new RegExp(/^[0,1]_[\w]+/);
...
class CrossDCsStreamPersistence {
    ...
    init() {
        ...
        return qio.makeDirectory(this._persistenceFileName.substr(0, this._persistenceFileName.lastIndexOf('/')))
            ...
            .then((stoppedEntries) => {
                if (stoppedEntries && stoppedEntries.trim().length) {
                    let pAll = _.map(stoppedEntries.split(','), (entryId) => {
                        let matched = entryPattern.exec(entryId);
                        if (matched && matched[0] == entryId) {
                            logger.debug(`Adding [${entryId}], STOPPED ENTRY, from ${this._persistenceFileName}, live entries persistence list`);
                            this._entriesToProcess[entryId] = entryId;
                        } else {
                            logger.warn(`${entryId} is invalid entry id, hence won't be added to live entries persistence list`);
                        }
                    });
    ...
    flushEntryList() {
        ...
        let persistenceListStr = this.toJSON().toString();
