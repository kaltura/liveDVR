...
class RecordingEntrySession {
    ...
    getNextTimeToUpdateEntry(){

        let durationMs = this.getDuration() ;
        if (durationMs == 0){
            this.nextTimeToUpdate = recordingUpdateEntryTimeIntervalMs
        }
        else{
            this.nextTimeToUpdate = Math.ceil(durationMs/recordingUpdateEntryTimeIntervalMs) * recordingUpdateEntryTimeIntervalMs
        }
        this.logger.info("getNextTimeToUpdateEntry, duration %s, nextTimeToUpdate %s ms ", durationMs, this.nextTimeToUpdate)
    ...
    updateEntry(){
        ...
        return backendClient.updateEntry(this.recordedEntryId, duration).then(()=>{
            ...
            .then((Mp4FilesUpdated) => {
                ...
                return Q.allSettled(this.playlistGenerator.update(Mp4FilesUpdated))
                    .then((results)=> {
                        ...
                        if (this.getDuration()  > this.nextTimeToUpdate){
                            this.getNextTimeToUpdateEntry()
