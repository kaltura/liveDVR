...
class RecordingEntrySession {
    ...
    constructor(entryObj) {
        ...
        const createPlaylistGenerator = (entryObj, recordingPath) => {
            ...
            if (recordingPath) {
                options.flavors = entryObj.flavorsObjArray
            }
    ...
    updateEntryDuration(force){
        ...
        if (force || duration > this.lastUpdateDuration + recordingUpdateEntryTimeIntervalMs) {
            ...
            this.logger.info(`[${this.entryId}] New last update duration ${duration}`);
            return backendClient.updateEntryDuration(this.entryId, duration).then(()=> {
                    this.logger.info(`[${this.entryId}] Successfully updated liveEntry with duration ${duration}`)
                })
        }
        return Q.resolve();
    }

    reportServerMaxRecordingDuration() {
        return backendClient.resetRecordingEntry(this.entryObj, this.getDuration());
    }
