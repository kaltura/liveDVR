        ...
        return this.playlistGenerator.initializeStart().then(()=> {
            return this.playlistGenerator.finalizeStart().then(()=> {
                if (this.maxDurationReached()) {
                    ...
                    // todo: RecordingManager.addNewChunks() handles split recording in case max recording duration reached
                    // return Q.reject(msg)
                }
    ...
    updateEntryDuration(force) {
        ...
        if (force == true || durationDiff >= recordingUpdateEntryTimeIntervalMs) {
            ...
            return this.sync.exec(()=> {
                return backendClient.updateEntryDuration(this.entryId, reportDurationMsec, this.entryObject.recordedEntryId, this.entryServerNodeId).then(()=> {
