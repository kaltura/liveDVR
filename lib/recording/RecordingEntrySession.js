...
class RecordingEntrySession {
    constructor(entryObj, entryServerNodeId, mp4FilesQueue = null, flavorsObjArray = null) {
        ...
        this.logger = loggerBase.getLogger('RecordingEntrySession', `[${this.entryObject.entryId}-${this.entryObject.recordedEntryId}] `);
        ...
        return this.playlistGenerator.initializeStart().then(()=> {
            return this.playlistGenerator.finalizeStart().then(()=> {
                ...
                if (this.getDurationMSec() > recordingMaxDurationInMs) {
                    let msg = util.format("Error, starting recorded entry with duration greater than the limit for recording session, limit: %s, got %s. The VOD might fail to process", recordingMaxDurationInMs, this.getDurationMSec());
