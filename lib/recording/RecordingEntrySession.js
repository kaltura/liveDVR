...
class RecordingEntrySession {
    ...
    isContinuousRecordingEnabled() {
        return backendClient.getEntryInfo(this.entryObject.entryId)
            .then((entryObj) => {
                if (entryObj.capabilities && entryObj.capabilities.indexOf('FEATURE_UNLIMITED_RECORDING') >= 0) {
                    this.logger.info(`Feature unlimited recording is enabled. Recording will be split`);
                    return Q.resolve(true);
                }
                else {
                    this.logger.info(`Recording will stop, as unlimited recording feature is disabled!!!`);
                    return Q.resolve(false);
                }
            })
            .catch((err)=> {
                this.logger.error(`Failed to get entry info. Recording will stop, assuming unlimited recording feature is disabled. Error: ${ErrorUtils.error2string(err)}`);
                return Q.resolve(false);
            });
    }
