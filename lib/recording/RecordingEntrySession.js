...
class RecordingEntrySession {
    ...
    getNewRecordedEntryPrimary() {
        return this.sync.exec(() => {
                this.logger.info('Primary serverNode -> Resetting recording information in server to complete split process');
            ...
            .then(newRecordedEntryId => {
                ...
                this.entryObject.recordedEntryId = newRecordedEntryId;
                return Q.resolve();
            });
    }

    getNewRecordedEntryBackup() {
        // Backup serverNode should not reset the recordedEntryId.
        // In case recordedEntryId is still the same run API call again
        return this.sync.exec(() => {
                this.logger.info('Backup serverNode -> Get new recordedEntryId from server');
                return backendClient.getLiveEntry(this.entryObject.entryId)
            })
            .then(liveEntry => {
                if (liveEntry.recordedEntryId !== this.recordedEntryId) {
                    this.logger.info(`Retrieved new recorded entry from database: [${liveEntry.recordedEntryId}]; restart session!`);
                    this.entryObject.recordedEntryId = liveEntry.recordedEntryId;
                    return Q.resolve();
                }
                else {
                    this.logger.info('RecordedEntryId is still the same, databases are not yet synced. Calling API server again');
                    return this.getNewRecordedEntryBackup();
                }
            });
    }

    handleReachedMaxRecordingDuration() {
        return (this.entryObject.serverType == kalturaTypes.KalturaEntryServerNodeType.LIVE_PRIMARY) ?
            this.getNewRecordedEntryPrimary() : this.getNewRecordedEntryBackup();
    }
