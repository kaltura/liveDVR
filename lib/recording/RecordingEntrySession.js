...
class RecordingEntrySession {
    ...
    constructor(entryObj, flavorsObjArray, streamNotMatchTranscodingProfile){
        ...
        this.recordingSessionPath  = persistenceFormat.getRecordedSessionPath(recordingRootPath, entryObj.entryId, entryObj.recordedEntryId, entryObj.recordStatus)
        this.flavors = []
        ...
        this.restore_files_patterns = '+([0-9])/*.mp4';
        this.restore_files_from_live_pattern = '+([0-9])/+([0-9])/*.mp4';
        if (entryObj.flavorsNotMatchedTranscodingProfile) {
            this.restore_files_patterns = '32/*.mp4';
            this.restore_files_from_live_pattern = '32/+([0-9])/*.mp4';
        }

        if (!streamNotMatchTranscodingProfile) {
            this.playlistGenerator = createPlaylistGenerator(entryObj, this.recordingSessionPath) //todo check that its not passed the limit
        } else {
            this.playlistGenerator = createPlaylistGenerator(entryObj, this.recordingSessionPath, flavorsObjArray)
        }
        ...
        // this flag is very important for the edge case that user started streaming
        // with low bitrate and then after stop start of live stream, started to stream
        // according to the live entry's transcoding profile.
        // In that case recording of flavor '32' alone, must continue as before.
        this.streamNotMatchTranscodingProfile = streamNotMatchTranscodingProfile;
                            ...
                            return qio.symbolicLink(accessibleRecordingFolder, this.recordingSessionPath, 'directory')
                                .catch((err) => {
                                   if (err.code && err.code === 'EEXIST') {
                                       this.logger.debug("didn't create symlink %s -> %s, already exit", accessibleRecordingFolder, this.recordingSessionPath);
                                       return Q.resolve();
                                   }  else {
                                       this.logger.error("failed to create symlinc. Error; %s", ErrorUtils.error2string(err))
                                   }
                                })
                        }
                    })
    ...
    checkFilesFromLiveFolder (){
        ...
        return this.findMissingChunksAndGetMetadata(path.join(liveEntryPath, this.restore_files_from_live_pattern))
            .then((Mp4Files)=> {
                if (Mp4Files) {
    ...
    checkFilesFromRecordingFolder() {
            return this.findMissingChunksAndGetMetadata(path.join(this.recordingSessionPath, this.restore_files_patterns)).then((missingChunks)=> {
                if (missingChunks) {
    ...
    changeRecordedEntry(entryObj){
        this.recordedEntryId = entryObj.recordedEntryId;
        this.recordingSessionId = this.entryId + '-' + recordedEntryId;
        this.recordingSessionPath  = persistenceFormat.getRecordedSessionPath(recordingRootPath, entryId, entryObj.recordedEntryId, entryObj.recordStatus)
