...
class RecordingEntrySession {
    ...
    updateEntryDuration(force) {
        ...
        if (force || durationDiff >= recordingUpdateEntryTimeIntervalMs) {
            this.lastUpdateDuration = duration;
            this.logger.info(`time to update recording duration. New duration: ${duration} ms`);
            return this.sync.exec(()=> {
                return backendClient.updateEntryDuration(this.entryId, duration, this.recordedEntryId, this.entryServerNodeId).then(()=> {
