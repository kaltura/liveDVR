...
class RecordingEntrySession {
        ...
        return this.playlistGenerator.initializeStart().then(()=> {
            return this.playlistGenerator.finalizeStart().then(()=> {
                if (this.maxDurationReached()){
                    let msg=util.format("שרמןמע, starting recorded entry with duration greater than the limit for recording session, limit: %s, got %s. The VOD might fail to process", recordingMaxDurationInMs, this.getDurationMSec());
    ...
    splitRecordedEntry(entryObject, reason) {
        if (entryObject.serverType == kalturaTypes.KalturaEntryServerNodeType.LIVE_PRIMARY) {
            ...
            return qio.isDirectory(unknownRecordedEntryIdPath)
                .then(dirExists=> {
                    if (dirExists) {
                        let archiveDir = `${Date.now()}_${unknownEntry}`;
                        let targetArchivePath =  persistenceFormat.getRecordingSessionPath(this.entryId, this.recordStatus, archiveDir);
                        this.logger.warn(`Due to recording split in LIVE_BACKUP, renaming [${unknownEntry}] recording content dir to [${archiveDir}], archive path [${targetArchivePath}]`);
                ...
                .then(()=> {
                    if (entryObject.recordedEntryId == this.recordedEntryId && entryObject.recordedEntryId != unknownEntry) {
    ...
    isUnlimitedRecordingDurationEnabled() {
        return backendClient.getEntryInfo(this.entryObject.entryId)
            .then((entryObj) => {
                if (entryObj.capabilities && entryObj.capabilities.indexOf('FEATURE_UNLIMITED_RECORDING_DURATION') >= 0) {
