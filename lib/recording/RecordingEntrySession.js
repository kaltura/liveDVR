...
class RecordingEntrySession {
    ...
    is24By7RecordingEnabled() {
        return backendClient.getEntryInfo(this.entryObject.entryId)
            .then((entryObj) => {
                if (entryObj.capabilities && entryObj.capabilities.indexOf('FEATURE_KALTURA_24_BY_7_RECORDING') >= 0) {
                    this.logger.info(`Feature 24 by 7 is recording is enabled. Recording will be split`);
                    return Q.resolve(true);
                }
                else {
                    this.logger.info(`Recording will stop, as 24 by 7 is recording feature is disabled!!!`);
                    return Q.resolve(false);
                }
            })
            .catch((err)=> {
                this.logger.error(`Failed to get entry info. Recording will stop, assuming recording 24 by 7 feature is disabled. Error: ${ErrorUtils.error2string(err)}`);
                return Q.resolve(false);
            });
    }
