...
class RecordingEntrySession {
        ...
        return this.playlistGenerator.initializeStart().then(()=> {
            return this.playlistGenerator.finalizeStart().then(()=> {
                if (this.getDurationMSec() >  recordingMaxDurationInMs){
                    ...
                    // this masks bugs when streaming to backup and not receiving new recordedEntryId
                    return Q.reject(msg)
                }
    ...
    splitRecordedEntry(entryObject, reason) {
        if (entryObject.serverType == kalturaTypes.KalturaEntryServerNodeType.LIVE_PRIMARY) {
            ...
            if (entryObject.recordedEntryId == this.recordedEntryId) {
                this.logger.error(`Duration passed limit but recordedEntryId didn't change stopping recording in BACKUP!!!! [${unknownEntry}]`);
    ...
    isContinuousRecordingEnabled() {
        return backendClient.getEntryInfo(this.entryObject.entryId)
            .then((entryObj) => {
                if (entryObj.capabilities && entryObj.capabilities.indexOf('FEATURE_KALTURA_CONTINUOUS_RECORDING') >= 0) {
                    this.logger.info(`Feature continuous recording is enabled. Recording will be split`);
                    return Q.resolve(true);
                }
                else {
                    this.logger.info(`Recording will stop, as continuous recording feature is disabled!!!`);
                    return Q.resolve(false);
                }
            })
            .catch((err)=> {
                this.logger.error(`Failed to get entry info. Recording will stop, assuming continuous recording feature is disabled. Error: ${ErrorUtils.error2string(err)}`);
                return Q.resolve(false);
            });
    }
