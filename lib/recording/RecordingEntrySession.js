...
class RecordingEntrySession {
        ...
        return this.playlistGenerator.initializeStart().then(()=> {
            return this.playlistGenerator.finalizeStart().then(()=> {
                return this.getEntryServerNodeId()
                    .catch((err)=> {
                        this.logger.info(`failed to get EntryServerNodeId. Error: [${ErrorUtils.error2string(err)}]`);
                        return Q.resolve();
                    })
                    .then(()=> {
    ...
    getEntryServerNodeId() {
        return this.sync.exec(() => {
            if (this.entryServerNodeId) {
                return Q.resolve();
            }
            else {
                return backendClient.getSingleEntryServerNode(this.entryObject.entryId, this.entryServerType)
                    .then((entryServerNode)=> {
                        this.entryServerNodeId = entryServerNode.id;
                        this.logger.debug(`successfully got entryServerNodeId [${this.entryServerNodeId}]`)
                        return Q.resolve();
                    })
                    .catch(function (err) {
                        return Q.reject(err);
                        this.logger.error(`failed to get entryServerNodeId. Error: ${ErrorUtils.error2string(err)}`);
                    });
            }
        });
    }
