...
const recordingMaxDurationInMs = config.get('recording').recordingMaxDurationInHours * 60 * 60 * 1000 - 5*60*1000;
class RecordingEntrySession {
    ...
    checkFilesFromRecordingFolder() {
            return this.findMissingChunksAndGetMetadata(path.join(this.recordingSessionPath, '*/*.mp4')).then((missingChunks)=> {
                if (missingChunks) {
                    ...
                    return Q.allSettled(this.updateJson(missingChunks))
                }
            })
    }
    ...
    updateJson(Mp4Files){
       let chunksNameChained = _.reduce(Mp4Files, (str, obj)=> {
        ...
        return Q.allSettled(this.playlistGenerator.update(Mp4Files))
            .then((results)=> {
                this.logger.debug("About to parse result of chunks: %s", chunksNameChained)
                this.parseUpdateJsonResult(results[1].value)
    ...
    parseUpdateJsonResult(result){
        if (result.removedChunks.length > 0){
            this.logger.warn("Remove chunks from playlist! %s", result.removedChunks)
        }
        if (result.failedAddChunks.length > 0){
            _.each(result.failedAddChunks, (errorObj)=>{
                this.logger.error("Failed to add chunk to playlist : %s",ErrorUtils.error2string(errorObj))
                if ('path' in errorObj){
                    let newPath = errorObj.path + '.' + new Date().getTime().toString()
                    this.logger.info("Rename %s to %s", errorObj.path, newPath)
                    qio.rename(errorObj.path, newPath) //dont wait for result...
                }
            })
        }
    }
    ...
    linkAndUpdateJsonSerialize(Mp4Files, flavorId) {
        ...
        return this.linkFiles(Mp4Files)
