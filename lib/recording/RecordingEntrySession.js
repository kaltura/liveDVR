...
class RecordingEntrySession {
    constructor(entryObj, entryServerNodeId, mp4FilesQueue = null, flavorsObjArray = null) {
        ...
        if (entryObj.recordStatus === kalturaTypes.KalturaRecordStatus.APPENDED) {
            this.modeDirName = appendDirName
        }
        else {
            this.modeDirName = newSessionDirName
        }
        this.recordingSessionPath = path.join(recordingRootPath, this.modeDirName, entryId.slice(-1), entryId, entryObj.recordedEntryId);
    ...
    accessibleRecording() {
        ...
        return this.playlistGenerator.initializeStart().then(()=> {
            return this.playlistGenerator.finalizeStart().then(()=> {
                if (this.getDurationMSec() > recordingMaxDurationInMs) {
                    let msg = util.format("Recorded entry is passed the limit of recording session, limit: %s, got %s", recordingMaxDurationInMs, this.getDurationMSec());
                    return Q.reject(msg)
                }
                ...
                let updatedDurationPromise = this.updateEntryDuration(true)
                        .catch((err)=> {
                            this.logger.info(`failed to update new recording session duration. Error: [${ErrorUtils.error2string(err)}]`);
                            return Q.resolve();
                        });
    ...
    updateEntryDuration(force) {
        ...
        if (force == true || durationDiff >= recordingUpdateEntryTimeIntervalMs) {
            ...
            this.logger.info(`${reason} update recording duration. Recording duration [${duration}] msec, this session duration [${currentSessionDuration}] msec`);
            return this.sync.exec(()=> {
                return backendClient.updateEntryDuration(this.entryId, reportDurationMsec, this.recordedEntryId, this.entryServerNodeId).then(()=> {
                    this.logger.info(`successfully updated recorded entry with duration ${reportDurationMsec}`)
                });
            });
        }
        else {
    ...
    handleReachedMaxRecordingDuration() {
        return this.sync.exec(() => {
            this.logger.info('Primary serverNode -> Reset recording information in server and complete split process');
            return backendClient.resetRecordingEntry(this.entryObject);
        })
    }
    ...
    updateRecordingFiles(newMp4Files, flavorId) {
        ...
        if (this.mp4FilesQueueLock) {
            this.logger.debug('Queue is locked. Not updating playlist!');
            return;
        }
    ...
    linkAndUpdateJsonSerialize(Mp4Files, flavorId) {
        return this.linkFiles(Mp4Files, flavorId)
            ...
            .then(()=> {
                ...
                return this.checkIfSplitSession(currentTime);
            })
            .catch((err) => {
    ...
    changeRecordedEntry(recordedEntryId) {
        // Todo: decide to use only one member containg recordedEntryId be it the this.entryObject or the this.recordedEntryId
        this.recordedEntryId = recordedEntryId;
    ...
    getRecordingSessionPath(recordedEntryId) {
        return path.join(recordingRootPath, this.modeDirName, this.entryId.slice(-1), this.entryId, recordedEntryId);
    }

    checkIfSplitSession(currentTime) {
        ...
        if (+timeElapsed >= +getEntryInfoIntervalMSec) {
            ...
            return this.sync.exec(()=> {
                return backendClient.getEntryInfo(this.entryId)
                    .then((entryObj) => {
                        if (entryObj.recordedEntryId != this.recordedEntryId) {
