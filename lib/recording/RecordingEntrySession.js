...
class RecordingEntrySession {
    ...
    handleRecordingReachedMaxDuration() {
        return this.sync.exec(()=> {
            this.logger.info('Resetting recording information in server to complete split process');
    ...
    updateRecordingFiles(newMp4Files, flavorId) {
        ...
        if (this.mp4FilesQueueLock) {
            this.logger.debug(`Split session is in process. No Update!`);
            return;
        }
