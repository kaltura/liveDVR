...
class RecordingEntrySession {
    constructor(entryObj, flavorsObjArray){
        ...
        this.logger = loggerBase.getLogger('RecordingEntrySession', '[' + recordingSessionId + '] ');
    ...
    checkFilesFromLiveFolder (){
        ...
        return this.findMissingChunksAndGetMetadata(liveEntryPath).then((Mp4Files)=> {
            if (Mp4Files) {
    ...
    findMissingChunksAndGetMetadata(directoriesToCheck){
        ...
        return pGlob(path.join(directoriesToCheck))
    ...
    getMetaData(missingChunks) {
        this.logger.debug("About to parse meta data for the file %s", missingChunks) //todo verify that all chunks are sorted
