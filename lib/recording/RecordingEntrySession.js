...
class RecordingEntrySession {
    ...
    splitRecordedEntry(entryObject = this.entryObject) {
 ...
 /*   verifyStreamMatchPreviousFlavorSet(newEntryObj) {
        return qio.exists(this.recordingSessionPath)
            .then((sessionExist)=> {
                if (!sessionExist) {
                    ...
                    let pattern = /^\d+$/;
                    return fsUtils.isSameFlavorSet(this.recordingSessionPath, newFlavorNames, [newEntryObj.recordedEntryId], pattern);
                }
            })
            .catch((err) => {
                this.logger.error(`failed to verify that current flavors match previous recording. Error: ${err}`);
    ...
    verifyFlavorSetMatchPlaylistFlavors(newEntryObj) {
        let playlistFilePath = path.join(this.recordingSessionPath, 'playlist.json');
        return qio.read(playlistFilePath)
            .catch((err) =>{
            ...
            .then((data) => {
                let playlistJSON = JSON.parse(data);
                return _.map(playlistJSON.sequences, (sequence) => {
                    return sequence.id;
                });
            })
            .then((flavorsFromPlaylist)=> {
                ...
                let flavorsDiff = _.difference(flavorsFromPlaylist, newFlavorNames);
