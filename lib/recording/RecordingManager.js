...
// todo Assumptions:
    ...
    getRecordingEntrySession(entryObj, mp4FilesQueue = null) {
        ...
        if (!this.recordingList[entryId]) {
            ...
            return Q.resolve({session:recordingEntrySession, started:false});
        }
            ...
            return Q.resolve({session:this.recordingList[entryId], started:true});
        }
    }
    ...
    startRecording(entryObj, mp4FilesQueue = null) {
        return Q.resolve().then(()=> {
            ...
            return this.getRecordingEntrySession(entryObj, mp4FilesQueue)
                .then((recordingObj)=> {
                    return recordingObj.session.verifyFlavorSetMatchPlaylistFlavors(entryObj)
                        .then((flavorsMatch) => {
                            if (!flavorsMatch) {
                                return recordingObj.session.splitRecordedEntry(entryObj)
                                    .then(() => {
                                        logger.info(`[${entryObj.entryId}-${entryObj.recordedEntryId}] after split, [${recordingObj.session.recordedEntryId}] replaced by [${entryObj.recordedEntryId}]`);
                        ...
                        .then((recordingObj) => {
                            if (!recordingObj.started) {
                                return recordingObj.session.startRecording()
                                    .then(()=> {
                                        // Once start process is complete, release lock on files queue
                                        recordingObj.session.mp4FilesQueueLock = false;
