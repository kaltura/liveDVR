...
// todo Assumptions:
    ...
    getRecordingEntrySession(entryObj, entryServerNodeId, mp4FilesQueue) {
        ...
        logger.debug(`${header}step:(3)=>[getRecordedEntrySession]`);
        let entryId = entryObj.entryId;
        if (mp4FilesQueue != null && mp4FilesQueue.length > 0) {
            logger.debug(`${header}[getRecordedEntrySession] getRecordingEntrySession called with mp4 files [${JSON.stringify(mp4FilesQueue)}] queue size: [${mp4FilesQueue.length}]`);
        }
        ...
        else if (this.recordingList[entryId].markedForDeletion) {
            let recordingEntrySession = new RecordingEntrySession(entryObj, entryServerNodeId, mp4FilesQueue);
    ...
    startRecording(entryObj, entryServerNodeId, stopAlreadyStartedSession = false) {
        let header = entryObj.recordedEntryId ? `[${entryObj.entryId}-${entryObj.recordedEntryId}]` : `[${entryObj.entryId}-${unknownEntry}]`;
        logger.debug(`${header}[step:(1)]==>[startRecording] get recordedEntryId`);

        return entryObj.syncPromises.exec(()=> {
            ...
            .then((recordedEntryId)=> {
                ...
                header = `[${entryObj.entryId}-${recordedEntryId}]`;
                if (!entryObj.recordedEntryId) {
                    logger.info(`[${header}][startRecording] entryObject doesn't contain recordedEntryId property! updating with createRecordedEntry result [${recordedEntryId}]`);
                    entryObj.recordedEntryId = recordedEntryId;
                }
                let startRecordingTime = new Date();

                if (entryObj.recordedEntryId != recordedEntryId) {
                    logger.info(`[${header}][startRecording] recordedEntryId changed EntryObject:[${entryObj.recordedEntryId}] createRecordedEntry:[${recordedEntryId}]`);
                    entryObj.recordedEntryId = recordedEntryId;
                    stopAlreadyStartedSession = true;
                }
                            ...
                            .then((sessionObj) => {
                                if (!sessionObj.sessionStarted) {
                                    return sessionObj.session.startRecording()
                                        .then(()=> {
                                            return sessionObj.session.updateRecordingStatus(kalturaTypes.KalturaEntryServerNodeRecordingStatus.ON_GOING)
                                                .catch((err)=> {
                                                    logger.error(`${header}[startRecording] Error updating RecordingStatus. Error: ${ErrorUtils.error2string(err)}`);
                                                    return Q.resolve();
                                                })
                                        })
                                        .then(()=> {
                                    ...
                                    if (sessionObj.session.mp4FilesQueueLock) {
                                        logger.warn(`[startRecording] session was not restarted. Check reason why mp4 files queue was locked. Releasing lock.`);
                                        sessionObj.session.mp4FilesQueueLock = false;
                                    }
                                }
                            });
                ...
                logger.error(`${header}[startRecording] Error while trying to start recording. Error: ${ErrorUtils.error2string(err)}, delete object`);
                if (this.recordingList[entryObj.entryId]) {
                    if (this.recordingList[entryObj.entryId].mp4FilesQueueLock) {
                        logger.warn(`[startRecording] new session was not started due to error. Releasing lock.`);
                        this.recordingList[entryObj.entryId].mp4FilesQueueLock = false;
                    }
                    if (!stopAlreadyStartedSession){
    ...
    stopLive(entryId) {
        ...
        let header = recordingEntrySession.recordingSessionId;
    ...
    splitRecording(entryId, reason) {
        ...
        logger.debug(`[${recordingSession.recordingSessionId}][step:(6)]==>[splitRecording]`);
        if (!recordingSession.mp4FilesQueueLock) {
            ...
            // mark session for deletion. Delete session only when creating new RecordedEntrySession
            recordingSession.markedForDeletion = true;
            let stopAlreadyStartedSession = true;
            logger.info(`[${recordingSession.recordingSessionId}][splitRecording] Recording entry ${recordingEntryId2Replace} has passed the limit of recording duration - ${this.recordingMaxDurationInMs / 1000} seconds. Split session`);
    ...
    replaceUnknownRecordedEntry(recordingEntrySession) {
        ...
        return backendClient.getEntryInfo(entryId)
            ...
            .catch((err) => {
                logger.error(`[${header}] Failed to change recorded entry! Error: ${ErrorUtils.error2string(err)}`);
            })
            .finally(() => {
                logger.info(`[${header}] UNKNOWN entry replacement finished`);
    ...
    stopSessionIfNeeded(entryObj, stopAlreadyStartedSession) {
        ...
        if (recordingEntrySession.entryObject.recordedEntryId != entryObj.recordedEntryId || stopAlreadyStartedSession) {
            ...
            if (recordingEntrySession.entryObject.recordedEntryId != entryObj.recordedEntryId) {
                logger.info(`${header}[step:(2)]==>[stopSessionIfNeeded] New recorded entryId [${recordedEntryId}] has been generated. Delete existing session and start new one`);
            } else {
                logger.info(`${header}[step:(2)]==>[stopSessionIfNeeded] stop session was requested, either max duration reached or found flavors mismatch`);
            }
            ...
            .then((sessionExist)=> {
                if (sessionExist) {
                    return sessionObj.session.verifyFlavorSetMatchPlaylistFlavors(entryObj)
                        .then((flavorsMatch) => {
                            if (!flavorsMatch) {
                               // lock the mp4 files queue
                                sessionObj.session.mp4FilesQueueLock = true;
                                // delete only when creating a new RecordedEntrySession
                                sessionObj.session.markedForDeletion = true;
    ...
    endRecordingSession(entryId, recordingEntrySession) {
        ...
        return knownRecordedEntryPromise
            .then(()=> {
              ...
              .then(()=> {
                return recordingEntrySession.updateRecordingStatus(kalturaTypes.KalturaEntryServerNodeRecordingStatus.STOPPED);
              })
              .then(()=> {
            ...
            .finally(()=> {
                if (!recordingEntrySession.markedForDeletion) {
                    delete this.recordingList[entryId];
                }
            })
    }
