...
// todo Assumptions:
    ...
    handleRecordingEntries() {
        ...
        function isRecordingFinish(recordingSession) {
            ...
            if (recordingSession.entryObject.explicitLive) {
                if (recordingSession.entryObject.recordingStatus == 1)//@@NA replace recording status with enum from updated client.
                {
                    logger.info("[%s-%s] recording is paused", recordingSession.entryId, recordingSession.recordedEntryId);
                    return false;
                }
                else if (recordingSession.entryObject.recordingStatus == 0)//@@NA replace recording status with enum from updated client.
                {
