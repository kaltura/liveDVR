    ...
    splitRecording(entryId, reason) {
        ...
        if (!recordingSession.mp4FilesQueueLock) {
            ...
            return recordingSession.splitRecordedEntry(recordingSession.entryObject, reason)
                ...
                .then(()=> {
                    logger.info(`[${entryId}-${entryObject.recordedEntryId}] Split process completed successfully. [${recordingSession.recordedEntryId}] replaced ${recordingEntryId2Replace} by ${entryObject.recordedEntryId}`);
                })
                .catch((err)=> {
