...
// todo Assumptions:
    ...
    stopRecording(entryId, recordedEntryId, recordingSessionPath){
        ...
        function changeRecordedEntry() {
            logger.debug("[%s][stopRecording][changeRecordedEntry][1 of 4] About to call get entry",  entryId)
            return backendClient.getEntryInfo(entryId)
                .then((entryObj)=>{
                    if ('recordedEntryId' in entryObj){
                        let recordedEntryId = entryObj.recordedEntryId
                        logger.debug("[%s][stopRecording] Found recordedEntryId : %s",  entryId, recordedEntryId)
                        return Q.resolve(recordedEntryId)
                    }
                    else{
                        logger.error("[%s[stopRecording] recordedEntryId not exist on entry object : %s",  entryId, recordedEntryId)
                        return Q.reject()
                    }
                })
                .then((recordedEntryId)=> {
                    logger.debug("[%s-%s][stopRecording][changeRecordedEntry][2 of 4] About to call changeRecordedEntry on recordingEntrySession", entryId, recordedEntryId)
                    return recordingEntrySession.changeRecordedEntry(recordedEntryId);
                }).then(()=>{
                    logger.debug("[%s-%s][stopRecording][changeRecordedEntry][3 of 4] About to change directory name from [%s] to [%s]",  entryId, recordedEntryId, recordingSessionPath,  recordingEntrySession.recordingSessionPath)
                    return qio.rename(recordingSessionPath, recordingEntrySession.recordingSessionPath)
                })
                .then(()=>{
                    recordingSessionPath = recordingEntrySession.recordingSessionPath //# todo return promise
                    recordedEntryId = recordingEntrySession.recordedEntryId
                    logger.debug("[%s-%s][stopRecording][changeRecordedEntry][4 of 4] about to export it to live directory",  entryId, recordedEntryId)
                    return recordingEntrySession.accessibleRecording()
                })
                .then(()=>{
                    logger.info("[%s-%s][stopRecording] changeRecordedEntry finished successfully",  entryId, recordedEntryId)
                })
            ...
            return qio.write(stampFilePath, recordingDuration)
                .then(()=> {
                  if (recordedEntryId === unknownEntry){
                      logger.debug("[%s][stopRecording]: recorded entry is unknown, calling changeRecordedEntry", entryId)
                      return changeRecordedEntry()
                  }  
                })
                .then(()=>{
                    ...
                    return qio.symbolicLink(destFilePath, recordingSessionPath, 'directory')
                        .then(()=>{
                            logger.info("[%s-%s][stopRecording] Successfully create soft link from %s, into %s, recording chunks in this session: [%s]", entryId, recordedEntryId, recordingSessionPath, destFilePath, totalAddedChunks);
                        })
                })
                .then(()=> {
                    delete this.recordingList[entryId];
