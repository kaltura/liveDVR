...
// todo Assumptions:
    ...
    getRecordingEntrySession(entryObj, mp4FilesQueue = null) {
        ...
        if (!this.recordingList[entryId]) {
            ...
            return Q.resolve(this.recordingList[entryId]);
        }
    }
            ...
            return this.getRecordingEntrySession(entryObj, mp4FilesQueue)
                .then((recordingEntrySession)=> {
                    return this.verifyStreamMatchPreviousFlavorSet(entryObj)
                        ...
                        .then((recordingEntrySession) => {
                            return recordingEntrySession.startRecording()
                                .then(()=> {
    ...
    splitRecording(entryId) {
        ...
        if (!recordingSession.mp4FilesQueueLock) {
            ...
            return recordingSession.splitRecordedEntry()
                .then(()=> {
        ...
        return qio.exists(recordingSessionPath)
            .then((sessionExist)=> {
                if (!sessionExist) {
                    ...
                    let pattern = /^\d+$/;
                    return fsUtils.isSameFlavorSet(recordingSessionPath, newFlavorNames, [entryObj.recordedEntryId], pattern);
                }
            })
            .catch((err) => {
