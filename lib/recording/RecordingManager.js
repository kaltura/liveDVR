...
// todo Assumptions:
    ...
    handleNewLiveClippingTask(task)
    {
        ...
        return qio.makeTree(newPath).then(() => {
            ...
            let clipFrom = PlaylistGenerator.getAbsoluteTimeFromRelative(playList, task.clipAttributes.offset);
            playList.clipTo = PlaylistGenerator.getAbsoluteTimeFromRelative(playList, task.clipAttributes.offset + duration);
            playList.clipFrom = clipFrom; // setting clipFrom only after calculate clipTo
            ...
            logger.debug("Saving new playlist with clip Attributes to: " + newPlayListPath + " clipFrom: " + playList.clipFrom + " clipTo:" + playList.clipTo);
