...
const maxAllowedMp4FilesQueueSize = 100;
...
// todo Assumptions:
    ...
    addNewChunks(Mp4Files, entryId, flavorId) {
        ...
        if (!recordingEntrySession.mp4FilesQueueLock &&
            ...
            logger.info(`[${entryId}] recording reached max duration [${mathUtils.durationToString(durationMSec, 2, mathUtils.HOUR_IN_MSEC)}] hrs`);
            addChunksPromise = recordingEntrySession.isUnlimitedRecordingDurationEnabled()
                .then((isEnabled)=> {
                    if (isEnabled) {
                        return this.splitRecording(entryId, 'recording reached max duration');
                    }
                    else {
                        stopRecording = true;
            ...
            addChunksPromise = Q.resolve();
        }

        return addChunksPromise
            .then(()=> {
                recordingEntrySession = this.recordingList[entryId];
                if (stopRecording) {
                    logger.info(`[${entryId}] stopped recording`);
                }
                else if (recordingEntrySession)
                {
                    if ((recordingEntrySession.mp4FilesQueueLock && recordingEntrySession.mp4FilesQueue.length < maxAllowedMp4FilesQueueSize)
                        || !recordingEntrySession.mp4FilesQueueLock) {
                    ...
                    logger.warn(`[${entryId}] failed to add chunks. RecordedEntrySession doesn't exist in RecordingList`);
                }
            })
            .catch((err)=> {
