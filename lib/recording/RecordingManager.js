        ...
        return Q.resolve().then(()=> {
            ...
            return getRecordingEntrySession.call(this, entryObj)
                .then((recordingEntrySession)=> {
                    ...
                    return this.verifyStreamMatchPreviousFlavorSet(recordingEntrySession)
                        .then((flavorsMatch) => {
                            if (!flavorsMatch) {
                    ...
                    return qio.listTree(recordingEntrySession.recordingSessionPath,
                        ...
                        .then((flavorsDirs)=> {
                            let flavorNames = _.map(flavorsDirs, function(flavorDir) {
                                return flavorDir.split("/").pop();
                            });
                            // check if flavors match current provisioned flavors
                            logger.debug(`flavorsDirs=${flavorNames}`);
                            if (flavorNames.length != recordingEntrySession.flavors.length) {
                            ...
                            let flavorsDiff = _.difference(flavorNames, recordingEntrySession.flavors);
