...
// todo Assumptions:
    ...
    resetRecordingEntry(entryObj) {

       entryObj.recordedEntryId = null;
        // ??? required
        delete this.recordingList[entryObj.entryId];
        var oldRecordedEntryId = entryObj.recordedEntryId;

        return backendClient.resetRecordingEntry(entryObj, KalturaLiveStatus.PLAYABLE)
            .then(() => {
                logger.debug("[%s] successfully reset recordedEntryId : new [%s] old [%s]", entryObj.entryId, entryObj.recordedEntryId, oldRecordedEntryId);
                return Q.resolve(entryObj.recordedEntryId);
            })
            .reject((err) => {
                logger.error("[%s-%s] failed to reset recordedEntryId. Error: %s", entryObj.entryId, oldRecordedEntryId, ErrorUtils.error2string(err));
                // resolve with unknownEntry or reject with error???
                return Q.reject(err);
            });
    }

    startRecording(entryObj, flavorsObjArray, transcodingProfileChanged){
        ...
        return Q.resolve().then(()=> {
            ...
            if (transcodingProfileChanged) {
                logger.info("[%s-%s] provision profile changed. recordedEntryId will be reset", entryObj.entryId, entryObj.recordedEntryId);
            }

            return (!transcodingProfileChanged ? Q.resolve() : this.resetRecordingEntry(entryObj))
                .then(() => {
    ...
    addNewChunks (Mp4Files, entryId, flavorId){
        ...
        if (!recordingEntrySession) {
            logger.info("[%s] Entry  not exist on recording list: %s - maybe entry was passed the limit of max duration", entryId, chunksNameChained);
            return
        }
