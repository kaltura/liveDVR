...
// todo Assumptions:
        ...
        return Q.resolve().then(()=> {
            ...
            return getRecordingEntrySession.call(this, entryObj)
                .then((recordingEntrySession)=> {
                    ...
                    return this.verifyStreamMatchPreviousFlavorSet(entryObj)
                        .then((flavorsMatch) => {
                            if (!flavorsMatch) {
                                return recordingEntrySession.splitRecordedEntry(entryObj)
                                    .then(() => {
                                        logger.info(`[${entryObj.entryId}-${entryObj.recordedEntryId}] after split, [${recordingEntrySession.recordedEntryId}] replaced by [${entryObj.recordedEntryId}]`);
                                        return getRecordingEntrySession.call(this, entryObj);
                                    })
                            } else {
    ...
    splitRecording(entryId) {
        ...
        if (!recordingSession.mp4FilesQueueLock) {
            ...
            return this.splitRecordedEntry(recordingSession.entryObject)
                .then(()=> {
                    return recordingSession.stopRecording(recordingSession.entryId)
                })
                .then(()=> {
        ...
        if (this.recordingList[entryId]) {
            ...
            return recordingEntrySession.serializedActionsPromise
                .finally(() => { // check first that all chunks are completed
                    ...
                    let recordingSessionDuration = recordingEntrySession.startDuration ? recordingEntrySession.getDurationMSec() - recordingEntrySession.startDuration : recordingEntrySession.startDuration;
                    if (isNaN(recordingSessionDuration) || recordingSessionDuration < this.recordingMinDurationInMS) {
    ...
    verifyStreamMatchPreviousFlavorSet(entryObj) {
        let recordingSessionPath = persistenceFormat.getRecordingSessionPath(entryObj.entryId, entryObj.recordStatus, entryObj.recordedEntryId);
        ...
        return qio.exists(recordingSessionPath)
            .then((sessionExist)=> {
                if (!sessionExist) {
                    ...
                    return qio.listTree(recordingSessionPath,
                        (path, stat) => {
                            ...
                            //let res = /^\d+$/.test(dirName);
                            if (dirName == entryObj.recordedEntryId) {
                                return false;
                            }
                            if (!isNaN(dirName)) {
                                return true;
                        ...
                        .then((flavorsDirs)=> {
                            ...
                            let newFlavorNames = _.map(entryObj.flavorsObjArray, function(newFlavorDir) {
                                return newFlavorDir.name;
                            });
                            // check if flavors match current provisioned flavors
                            if (flavorNames.length != newFlavorNames.length) {
                                logger.debug(`found that provisioned stream has different flavors than previous recording. Previous: [${flavorNames}] current: [${newFlavorNames}]`);
                            ...
                            let flavorsDiff = _.difference(flavorNames, newFlavorNames);
                            if (flavorsDiff.length > 0) {
                                logger.debug(`found that provisioned stream has different flavors than configured (though same number of flavors). Previous: [${flavorNames}] current: [${newFlavorNames}]`);
                                return Q.resolve(false);
                            }
                            return Q.resolve(true);
                        })
            ...
            .catch((err) => {
                logger.error(`failed to verify that current flavors match previous recording. Error: ${err}`);
                // since failed to verify match choosing fallback which is assuming no change in flavor set
                return Q.resolve(true);
            })
    }
}
