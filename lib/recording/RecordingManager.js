...
// todo Assumptions:
    ...
    stopLive(entryObject){
        ...
        if (this.isRecordedEntryUnknown(recordingEntrySession.recordedEntryId)){
            this.changeRecordedEntry(recordingEntrySession);
        }
    ...
    handleRecordingEntries(){
        ...
        function isRecordingFinish(element) {
            ...
            if (element.playlistGenerator.MaxClipCountReached()){
                logger.warn("[%s-%s] recording  has passed num of clips %d - stop recording", element.entryId, element.recordedEntryId);
                return true
            }
            return false
        }
        ...
        try {
            ...
            _.each(this.recordingList,  (element, entryId) =>{
                ...
                if (this.isRecordedEntryUnknown(element.recordedEntryId)){
                    this.changeRecordedEntry(element)
                }
    ...
    changeRecordedEntry(recordingEntrySession) {
        try {

            logger.debug("[%s][changeRecordedEntry][1 of 4] About to call get entry");
            ...
            logger.error("ron Failed to changeRecordedEntry: %s", ErrorUtils.error2string(err))
        }
    }
