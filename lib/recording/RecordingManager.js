...
// todo Assumptions:
    ...
    startRecording(entryObj, flavorsObjArray){
        return Q.resolve().then(()=> {
            ...
            function getRecordingEntrySession(entryObj) {
                ...
                if (!this.recordingList[entryId]) {
                    ...
                    return this.stopRecording(entryObj.entryId)
                        ...
                        .catch((error)=> {
                            logger.error(`Failed to stop existed recording session and create a one. Error: ${ErrorUtils.error2string(error)}`);
            ...
            logger.error(`[${entryObj.entryId}-${entryObj.recordedEntryId}] Error while try to start recording : ${ErrorUtils.error2string(err)}, delete object`);
    ...
    addNewChunks (Mp4Files, entryId, flavorId) {
        ...
        if (recordingEntrySession.getDuration() > this.recordingMaxDurationInMs) {
            logger.warn(`[${entryId}] Recording entry ${this.recordingList[entryId].recordedEntryId} has passed the limit of recording duration - ${this.recordingMaxDurationInMs/1000} seconds`);
        }
        ...
        if (this.recordingList[entryId]) {
            ...
            return recordingEntrySession.serializedActionsPromise.finally(() => { // check first that all chunks are completed
                ...
                if (!recordingSessionDuration > 0) {
                    logger.warn("[%s-%s][stopRecording] Entry duration hasn't changed, do nothing. Start [%s] Current [%s], totalAddedChunks [%s]", entryId,
                        recordingEntrySession.recordedEntryId, recordingEntrySession.getDuration(), recordingEntrySession.startDuration, totalAddedChunks);
                ...
                return qio.write(stampFilePath, recordingDuration)
                    .then(() => {
                        ...
                        if (recordingEntrySession.serverType !== kalturaTypes.KalturaEntryServerNodeType.LIVE_PRIMARY) {
                            logger.info("[%s-%s][stopRecording] entry is backup, skip creating soft link", entryId, recordingEntrySession.recordedEntryId);
                            return Q.resolve()
                        }
                        ...
                        return qio.symbolicLink(destFilePath, recordingEntrySession.recordingSessionPath, 'directory')
                            .then(() => {
                                logger.info("[%s-%s][stopRecording] Successfully create soft link from %s to %s; Recording chunks in this session: [%s] recordingSessionDuration [%s]", entryId, recordingEntrySession.recordedEntryId,
                                    recordingEntrySession.recordingSessionPath, destFilePath, totalAddedChunks, recordingSessionDuration);
                            })
                    })
                    .then(() => {
                        delete this.recordingList[entryId];
