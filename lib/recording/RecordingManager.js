...
// todo Assumptions:
    ...
    static updateClipPlaylist(playlist, offset, duration)
    {
        let totalPlaylistDuration = playlist.durations.reduce((a, b) => a + b, 0);
        if (totalPlaylistDuration < ((offset + duration)))
        {
            logger.error(`Error in validation of updateClipPlaylist. total=${totalPlaylistDuration},offset=${offset},duration=${duration}`);
            return null; //TODO check if update entryServerNode status as ERROR and raise error
        }

        let i = 0;
        while (offset > playlist.durations[i])
            offset -= playlist.durations[i++]; //search for the first clip
        
        playlist.clipFrom = playlist.clipTimes[i] + offset;
        let clipTimeAvailable = playlist.durations[i] - offset; // setting remained time in current (i) clip
        while (duration > clipTimeAvailable) //searching the last clip for setting the clipTo attribute
        {
            duration -= clipTimeAvailable;
            clipTimeAvailable = playlist.durations[i+1];
            i++;
            offset = 0; // so we won't add that to the clipTo because move to new clip
        }
        playlist.clipTo = playlist.clipTimes[i] + duration + offset;
        return playlist;
    }
    ...
    handleNewLiveClippingTask(task)
    {
        ...
        return qio.makeTree(newPath).then(() => {
            ...
            let playList = RecordingManager.updateClipPlaylist(JSON.parse(data), task.clipAttributes.offset, duration);
