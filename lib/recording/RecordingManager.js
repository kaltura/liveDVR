...
// todo Assumptions:
    ...
    getRecordingEntrySession(entryObj) {
        let entryId = entryObj.entryId;
        if (this.chunksBufferingList[entryObj.entryId] != null && this.chunksBufferingList[entryObj.entryId].length > 0) {
            logger.debug(`[${entryObj.entryId}-${entryObj.recordedEntryId}] getRecordingEntrySession called with mp4 files queue size [${this.chunksBufferingList[entryObj.entryId].length}]`);
        }
        ...
        if (!this.recordingList[entryId]) {
            ...
            if (!this.chunksBufferingList[entryObj.entryId]) {
                this.chunksBufferingList[entryObj.entryId] = [];
            }
    ...
    startRecording(entryObj) {
        return Q.resolve().then(()=> {
            ...
            return this.stopSessionIfNeeded(entryObj)
                .then(()=> {
                    return this.getRecordingEntrySession(entryObj)
                })
                .then((sessionObj)=> {
            ...
            logger.error(`[${entryObj.entryId}-${entryObj.recordedEntryId}] failed to start new recording session. Error: [${ErrorUtils.error2string(err)}], deleting session`);
            if (this.recordingList[entryObj.entryId]) {
                delete this.recordingList[entryObj.entryId];
            }
            if (this.chunksBufferingList[entryObj.entryId]) {
                delete this.chunksBufferingList[entryObj.entryId];
            }
    ...
    splitRecording(entryId, reason) {
        ...
        return recordingSession.isUnlimitedRecordingDurationEnabled()
            .then((isEnabled)=> {
                if (!isEnabled) {
                    return this.stopRecording(entryId, true);
                }
                else {
    ...
    addNewChunks(Mp4Files, entryId, flavorId) {
        ...
        return this.handleMaxDuration(entryId)
            .then(()=> {
                this.bufferChunks(Mp4Files, entryId, flavorId);
            })
            .then(()=> {
                let recordingEntrySession = this.recordingList[entryId];
                if (!recordingEntrySession) {
                ...
                if (this.chunksBufferingList[entryId].length > maxAllowedMp4FilesQueueSize) {
                    logger.fatal(`[${entryId}] chunks buffering queue reached [${maxAllowedMp4FilesQueueSize}] size. Dropping chunks!!!! CALL DEVELOPER`);
                }
                else {
                    // update lastUpdateTime
                    recordingEntrySession.lastUpdateTime = new Date();
                    recordingEntrySession.updateRecordingFiles(this.chunksBufferingList[entryId], flavorId);
                }
            })
            .catch((err)=> {
        ...
        try {
            ...
            _.each(this.recordingList, (element, entryId) => {
                ...
                if (isRecordingFinish(element)) {
                    this.stopRecording(entryId, true);
                }
            });
    ...
    replaceUnknownRecordedEntry(recordingEntrySession) {
        ...
        return backendClient.getEntryInfo(entryId)
            ...
            .finally(() => {
                logger.info(`[${entryId}-${recordedEntryId}] [replaceUnknownRecordedEntry]  UNKNOWN entry replacement finished`);
            })
    }

    stopRecording(entryId, doCleanup = false) {
        if (this.recordingList[entryId]) {
            let recordingEntrySession = this.recordingList[entryId];
            ...
            return recordingEntrySession.serializedActionsPromise
                .finally(() => { // check first that all chunks are completed
                    ...
                    if (doCleanup && this.chunksBufferingList[entryId]) {
                        delete this.chunksBufferingList[entryId];
                    }
                    ...
                    if (recordingSessionDuration < this.recordingMinDurationInMS) {
                        logger.warn(`[${entryId}-${recordingEntrySession.recordedEntryId}][stopRecording] Recording duration is less than minimum required, do nothing. Current: [${recordingEntrySession.getDurationMSec()}], Start: [${recordingEntrySession.startDuration}], totalAddedChunks: [${totalAddedChunks}]`);
                        delete this.recordingList[entryId];
                ...
                .catch((err) => {
                    ...
                    if (this.chunksBufferingList[entryId]) {
                        delete this.chunksBufferingList[entryId];
                    }
                    return Q.resolve();
                });
    ...
    handleMaxDuration(entryId) {
        ...
        if (recordingEntrySession.maxDurationReached() && !recordingEntrySession.maxDurationFlag) {
            recordingEntrySession.maxDurationFlag = true;
    ...
    bufferChunks(mp4Files, entryId, flavorId) {
        let mp4FilesClone = _.clone(mp4Files);
        if (this.chunksBufferingList[entryId]) {
            this.chunksBufferingList[entryId].push({newMp4Files: mp4FilesClone, flavorId: flavorId});
        }
        else {
            let chunksNameChained = _.reduce(mp4Files, function (str, obj) {
                return str + obj.chunkName + " ";
            ...
            logger.error(`[${entryId}] couldn't find entry in chunksBefferingList. Dumping chunks [${chunksNameChained}]`);
