...
// todo Assumptions:
        ...
        return qio.exists(recordingSessionPath).then((sessionExist)=> {
            ...
            return startSessionPromise
                .then(()=>{
                    ...
                    logger.info("[%s-%s] Start recording is completed, time : %s", entryObj.entryId, entryObj.recordedEntryId, startRecordingTimeDuration)
                })
        }).catch((err)=>{
    ...
    updateServer(entryObject){
        ...
        recordingEntrySession.updateEntry(true)
}
