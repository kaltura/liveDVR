...
// todo Assumptions:
    ...
    splitRecording(entryId, reason) {
        ...
        return recordingSession.isUnlimitedRecordingDurationEnabled()
            .then((isEnabled)=> {
                if (!isEnabled) {
                ...
                else if (!recordingSession.mp4FilesQueueLock) {
                    ...
                    logger.warn(`[${entryId}-${recordingSession.recordedEntryId}] splitting in progress, skipping split`);
                    return Q.resolve();
                }
            })
    }
    ...
    addNewChunks(Mp4Files, entryId, flavorId) {
        ...
        return this.handleMaxDuration(entryId, recordingEntrySession)
            .then(()=> {
    ...
    handleMaxDuration(entryId, recordingEntrySession) {
        let handleMaxDurationPromise = null;
        let header = `[${entryId}-${recordingEntrySession.entryObject.recordedEntryId}]`;
        if (recordingEntrySession.maxDurationReached()) {
            let durationMSec =  recordingEntrySession.getDurationMSec();
            logger.info(`${header} recording reached max duration [${mathUtils.durationToString(durationMSec, 2, mathUtils.HOUR_IN_MSEC)}] hrs`);
