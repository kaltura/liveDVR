...
// todo Assumptions:
    ...
    startRecording(entryObj, entryServerNodeId, mp4FilesQueue = null){
        return backendClient.createRecordedEntry(entryObj)
            .then((recordedEntryId)=> {
                entryObj.recordedEntryId = recordedEntryId;
                ...
                function getRecordingEntrySession(entryObj, entryServerNodeId) {
                    ...
                    if (!this.recordingList[entryId]) {
                        ...
                        let recordingEntrySession = new RecordingEntrySession(entryObj, entryServerNodeId);
                        ...
                        logger.info(`[${entryObj.entryId}-${entryObj.recordedEntryId}] New recorded entryId has been generated. Delete existing session and start new one`);
                ...
                const startExistSession = (recordingEntrySession)=> {
                    ...
                    return recordingEntrySession.accessibleRecording()
                        ...
                        .then((fileDoneExist)=> {
                            ...
                            if (fileDoneExist) {
                                logger.debug(`[${entryObj.entryId}-${entryObj.recordedEntryId}] done file exists, removing it`);
                                return qio.remove(doneFilePath)
                            }
                            else {
                                logger.warn(`[${entryObj.entryId}-${entryObj.recordedEntryId}] done file doesn't exist, check for restoration`);
                ...
                return getRecordingEntrySession.call(this, entryObj, entryServerNodeId)
    ...
    stopLive(entryId){
        ...
        if (recordingEntrySession == null) {
            logger.debug(`[${entryId}[stopLive] RecordedEntrySession not found`);
            return Q.resolve();
        }
        logger.debug(`[${entryId}-${recordingEntrySession.entryObject.recordedEntryId}][stopLive] live stream stopped. Recording duration will be updated`);
        if (!recordingEntrySession) {
            logger.warn(`[${entryId}-${recordingEntrySession.entryObject.recordedEntryId}] [stopLive] can't updateServer recording, entry does not exist on recording list`);
            return Q.resolve();
        }
            ...
            .then(()=> {
                if (this.isRecordedEntryUnknown(recordingEntrySession.entryObject.recordedEntryId)) {
                    this.replaceUnknownRecordedEntry(recordingEntrySession)
                        .catch((err)=> {
                            logger.error(`[${entryId}-${recordingEntrySession.entryObject.recordedEntryId}] [stopLive] failed to replace recorded entry (content root dir not updated). Error: ${ErrorUtils.error2string(err)}`);
                        })
                        .then(()=> {
                            return Q.resolve();
                        });
    ...
    splitRecording(entryId) {
        ...
        if (!recordingSession.mp4FilesQueueLock) {
            ...
            logger.info(`[${entryId}] Recording entry ${this.recordingList[entryId].entryObject.recordedEntryId} has passed the limit of recording duration - ${this.recordingMaxDurationInMs/1000} seconds. Split session`);
            return recordingSession.handleReachedMaxRecordingDuration()
                .then(()=> {
                    // Pass startRecording the queue of previous session in case there are files waiting to be written
                    return this.startRecording(recordingSession.entryObject, recordingSession.entryServerNodeId, recordingSession.mp4FilesQueue);
                })
                .then(()=> {
    ...
    addNewChunks (Mp4Files, entryId, flavorId) {
        ...
        if (recordingEntrySession.splitRecordingRequired) {
            let recordingSession = this.recordingList[entryId];
            this.startRecording(recordingSession.replacementEntryObj, recordingSession.entryServerNodeId, recordingSession.mp4FilesQueue);
        }
    ...
    handleRecordingEntries(){
        ...
        function isRecordingFinish(recordingSession) {
            if (now - recordingSession.lastUpdateTime > recordingSession.recordingSessionDuration){
                logger.info("[%s-%s] recording  didn't get new chunks for %d - stop recording", recordingSession.entryId, recordingSession.entryObject.recordedEntryId, recordingSession.recordingSessionDuration);
                return true
            }
            if (recordingSession.playlistGenerator.MaxClipCountReached()){
                logger.warn("[%s-%s] recording  has passed num of clips - stop recording", recordingSession.entryId, recordingSession.entryObject.recordedEntryId);
                return true
            }
            return false
        }
        ...
        return backendClient.getEntryInfo(entryId)
            .then((entryObj) => {
                if (entryObj.recordedEntryId) {
                    ...
                    let newRecordingDirectory = recordingEntrySession.getRecordingSessionPath(recordedEntryId);
            ...
            return recordingEntrySession.updateEntryDuration(true)
                ...
                .finally(() => { // check first that all chunks are completed
                    ...
                    if (totalRecordingDurationMsec < this.recordingMinDurationInMS) {
                        logger.warn(`[${entryId}-${recordingEntrySession.entryObject.recordedEntryId}][stopRecording] Recording duration is less than minimum required, do nothing. Total recording duration: [${totalRecordingDurationMsec}] msec, totalAddedChunks: [${totalAddedChunks}]`);
                    ...
                    return qio.write(stampFilePath, totalRecordingDurationMsec.toString())
                         .then(() => {
                            if (this.isRecordedEntryUnknown(recordingEntrySession.entryObject.recordedEntryId)) {
                                logger.error(`[${entryId}-${recordingEntrySession.entryObject.recordedEntryId}][stopRecording]: recorded entry is unknown, skip creating soft link. This is a bug, call developer`);
                                return Q.resolve()
                            }
                            let destFilePath = path.join(this.completedRecordingFolderPath, entryId + '_' + recordingEntrySession.entryObject.recordedEntryId + '_' + totalRecordingDurationMsec);
                            return qio.symbolicLink(destFilePath, recordingEntrySession.recordingSessionPath, 'directory')
                                .then(() => {
                                    logger.info(`[${entryId}-${recordingEntrySession.entryObject.recordedEntryId}][stopRecording] Successfully created soft link from ${recordingEntrySession.recordingSessionPath} to ${destFilePath}. Total recording duration [${totalRecordingDurationMsec}] msec, total chunks in this session: [${totalAddedChunks}] `);
                                })
                        })
                        .then(() => {
                            delete this.recordingList[entryId];
                ...
                .catch((err) => {
                    logger.error(`[${entryId}-${recordingEntrySession.entryObject.recordedEntryId}][stopRecording] Failure: ${ErrorUtils.error2string(err)}`);
