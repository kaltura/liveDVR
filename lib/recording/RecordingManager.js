...
// todo Assumptions:
    ...
    getRecordingEntrySession(entryObj, mp4FilesQueue = null) {
        ...
        if (!this.recordingList[entryId]) {
            ...
            return Q.resolve({session:recordingEntrySession, newSession:true});
        }
            ...
            return Q.resolve({session:this.recordingList[entryId], newSession:false});
        }
    }
                        ...
                        .then((flavorsMatch) => {
                            if (!flavorsMatch) {
                                return sessionObj.session.splitRecordedEntry(entryObj)
                                    .then(() => {
                                        logger.info(`[${entryObj.entryId}-${entryObj.recordedEntryId}] after split, [${sessionObj.session.recordedEntryId}] replaced by [${entryObj.recordedEntryId}]`);
                                        if (!sessionObj.newSession) {
                                            return this.stopSessionIfNeeded(entryObj)
                                        }
                                    })
                                    .then((mp4FilesQueue)=> {
        ...
        if (!recordingSession.mp4FilesQueueLock) {
            ...
            return recordingSession.splitRecordedEntry(recordingSession.entryObject)
                .then(()=> {
                    ...
                    return this.startRecording(recordingSession.entryObject);
                })
                .then(()=> {
