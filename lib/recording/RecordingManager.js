...
// todo Assumptions:
    ...
    resetRecording(entryId) {
        let recordingSession = this.recordingList[entryId];
        return recordingSession.reportServerMaxRecordingDuration()
            .then((newRecordedEntryId)=> {
                logger.info(`[${entryId}] Recording session has been reset. New recordedEntryId: [${newRecordedEntryId}]; restart recording`);
                let entryObj = recordingSession.entryObj;
                entryObj.recordedEntryId = newRecordedEntryId;
                return this.startRecording(entryObj);
            })
            .catch((err)=> {
                logger.error(`[${entryId}] Error resetting recording for entry. Error: ${ErrorUtils.error2string(err)}`);
            });
    }
    ...
    addNewChunks (Mp4Files, entryId, flavorId) {
        ...
        if (recordingEntrySession.getDuration() > this.recordingMaxDurationInMs) {
            logger.warn(`[${entryId}] Recording entry ${this.recordingList[entryId].recordedEntryId} has passed the limit of recording duration - ${this.recordingMaxDurationInMs/1000} seconds. Reset recordedEntryId`);
            return this.resetRecording(entryId);
        }
    ...
    handleRecordingEntries(){
        ...
        function isRecordingFinish(recordingSession) {
            if (now - recordingSession.lastTimeUpdateChunk > recordingSession.recordingSessionDuration){
            ...
            return recordingEntrySession.serializedActionsPromise
                .finally(() => { // check first that all chunks are completed
                    ...
                    if (recordingSessionDuration < this.recordingMinDurationInMS) {
                        logger.warn(`[${entryId}-${recordingEntrySession.recordedEntryId}][stopRecording] Recording duration is less than minimum required, do nothing. Start: [${recordingEntrySession.getDuration()}], Current: [${recordingEntrySession.startDuration}], totalAddedChunks: [${totalAddedChunks}]`);
