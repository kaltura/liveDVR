...
// todo Assumptions:
    ...
    setAudioLanguage(entryId, flavor, language) {
        let recordingEntrySession =  this.recordingList[entryId];

        if (!recordingEntrySession) {
            logger.warn("[%s] Entry is not exist on recording list: language will not be updated", entryId);
            return;
        }

        recordingEntrySession.setAudioLanguage(flavor, language)
    }
