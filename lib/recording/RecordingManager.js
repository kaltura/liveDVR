...
// todo Assumptions:
    ...
    getRecordingEntrySession(entryObj, entryServerNodeId, mp4FilesQueue) {
        ...
        if (mp4FilesQueue != null && mp4FilesQueue.length > 0) {
            logger.debug(`[${entryObj.entryId}-${entryObj.recordedEntryId}] getRecordingEntrySession called with mp4 files queue size [${mp4FilesQueue.toString()}]`);
        }
    ...
    startRecording(entryObj, entryServerNodeId) {
    ...
    splitRecording(entryId, reason) {
        ...
        if (!recordingSession.mp4FilesQueueLock) {
            ...
            let recordingEntryId2Replace = entryObject.recordedEntryId;
            ...
            return recordingSession.splitRecordedEntry(recordingSession.entryObject, reason)
                .then(()=> {
                    ...
                    return this.startRecording(entryObject, recordingSession.entryServerNodeId);
                })
                .then(()=> {
                    logger.info(`[${entryId}-${entryObject.recordedEntryId}] Split process completed successfully. [${entryObject.recordedEntryId}] replaced ${recordingEntryId2Replace} by ${entryObject.recordedEntryId}`);
                })
                .catch((err)=> {
    ...
    addNewChunks(Mp4Files, entryId, flavorId) {
        ...
        if (recordingEntrySession.splitRecordingRequired) {
            let recordingSession = this.recordingList[entryId];
            this.startRecording(recordingSession.replacementEntryObj, recordingSession.entryServerNodeId);
        }
            ...
            return recordingEntrySession.updateEntryDuration(true)
                ...
                .finally(() => { // check first that all chunks are completed
                    ...
                    if (totalRecordingDurationMsec < this.recordingMinDurationInMS) {
                        logger.warn(`[${entryId}-${recordingEntrySession.recordedEntryId}][stopRecording] Recording duration is less than minimum required, do nothing. Current duration: [${totalRecordingDurationMsec}] msec, start duration: [${recordingEntrySession.startDuration}] msec, totalAddedChunks: [${totalAddedChunks}]`);
                        ...
                        .then((flavorsMatch) => {
                            if (!flavorsMatch) {
                                return sessionObj.session.splitRecordedEntry(entryObj, 'found mismatch in flavors set from previous session')
                                    .then(()=> {
                                        logger.info(`after split, [${sessionObj.session.entryObject.recordedEntryId}] replaced by [${entryObj.recordedEntryId}]`);
                                        if (sessionObj.sessionStarted) {
                                            return this.stopSessionIfNeeded(entryObj)
                                        }
                                        else {
