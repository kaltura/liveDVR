...
// todo Assumptions:
    ...
    getRecordingEntrySession(entryObj, mp4FilesQueue = null) {
        ...
        if (mp4FilesQueue != null && mp4FilesQueue.length > 0) {
            let chunksNameChained = _.reduce(mp4FilesQueue, function (str, obj) {
                return str + obj.chunkName + " ";
            }, "");
            logger.debug(`[${entryObj.entryId}-${entryObj.recordedEntryId}] getRecordingEntrySession called with mp4 files queue size [${mp4FilesQueue.length}][${chunksNameChained}]`);
        }
