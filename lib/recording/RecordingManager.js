...
// todo Assumptions:
    ...
    constructor() {
        ...
        TaskManager.register(kalturaTypes.KalturaEntryServerNodeType.LIVE_CLIPPING_TASK, this.handleNewLiveClippingTask);
    ...
    handleNewLiveClippingTask(task)
    {
        ...
        qio.makeTree(newPath).then(() => {
            return qio.read(playListPath);
        }).then(data => {
            let playList = JSON.parse(data);
            playList.clipFrom = playList.firstClipTime + task.clipAttributes.offset;
            playList.clipTo = playList.clipFrom + task.clipAttributes.duration;
            ...
            //create sl to incoming to live recorder
            let destFilePath = path.join(this.completedRecordingFolderPath, task.entryId + '_' + task.clippedEntryId);
