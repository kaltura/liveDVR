...
// todo Assumptions:
    ...
    getRecordingEntrySession(entryObj, mp4FilesQueue = null) {
        ...
        if (!this.recordingList[entryId]) {
            ...
            logger.info(`[${entryObj.entryId}-${entryObj.recordedEntryId}] Found previous recording session with different recordedEntryID, with recordedEntryId=[${this.recordingList[entryId].recordedEntryId}]`);
            let recordingEntrySession = new RecordingEntrySession(entryObj, mp4FilesQueue);
    ...
    startRecording(entryObj) {
        return Q.resolve().then(()=> {
            ...
            return this.stopSessionIfNeeded(entryObj)
                ...
                .then((sessionObj)=> {
                    return qio.exists(sessionObj.session.recordingSessionPath)
                        .then((sessionExist)=> {
                            if (sessionExist) {
    ...
    handleFlavorsMismatch(entryObj, sessionObj) {
        return sessionObj.session.verifyFlavorSetMatchPlaylistFlavors(entryObj)
            .then((flavorsMatch) => {
                if (!flavorsMatch) {
                    return sessionObj.session.splitRecordedEntry(entryObj)
                        .then(()=> {
