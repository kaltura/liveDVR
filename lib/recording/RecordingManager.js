...
// todo Assumptions:
            ...
            return getRecordingEntrySession.call(this, entryObj)
                .then((recordingEntrySession)=> {
                    ...
                    return this.verifyStreamMatchPreviousFlavorSet(entryObj)
                        .then((flavorsMatch) => {
                            if (!flavorsMatch) {
                                return this.splitRecordedEntry(entryObj)
                                    .then(() => {
                                        return getRecordingEntrySession.call(this, entryObj);
                                    })
                            }
    ...
    splitRecording(entryId) {
        ...
        if (!recordingSession.mp4FilesQueueLock) {
            ...
            return this.splitRecordedEntry(recordingSession.entryObject)
                .then(()=> {
                    return this.stopRecording(recordingSession.entryId)
                })
                .then(()=> {
    ...
    splitRecordedEntry(entryObject) {

        if (entryObject.serverType == kalturaTypes.KalturaEntryServerNodeType.LIVE_PRIMARY) {
            return this.sync.exec(() => {
                logger.info('Primary serverNode -> Reset recording information in server and complete split process');
                return backendClient.resetRecordingEntry(entryObject);
            })
                .then(newRecordedEntryId => {
                    logger.info(`Successfully created new recordedEntryId: [${newRecordedEntryId}]; restart session!`);
                    entryObject.recordedEntryId = newRecordedEntryId;
                });
        }
        else {
            logger.info(`Backup serverNode -> Set recordedEntryId to [${unknownEntry}] and complete split process`);
            entryObject.recordedEntryId = unknownEntry;
            return Q.resolve();
        }
    }

    verifyStreamMatchPreviousFlavorSet(recordingEntrySession) {
        let recordingSessionPath = persistenceFormat.getRecordingSessionPath(recordingEntrySession.entryId, recordingEntrySession.recordStatus, recordingEntrySession.recordedEntryId);

        return qio.exists(recordingEntrySession.recordingSessionPath)
            .then((sessionExist)=> {
                if (!sessionExist) {
                    return Q.resolve(true);
                }
                else {
                    return qio.listTree(recordingEntrySession.recordingSessionPath,
                        (path, stat) => {
                            ...
                            if (dirName == recordingEntrySession.recordedEntryId) {
                                return false;
                            }
                            if (!isNaN(dirName)) {
                                return true;
                        ...
                        .then((flavorsDirs)=> {
                            ...
                            logger.debug(`flavorsDirs=${flavorsDirs}`);
                            if (flavorsDirs.length != recordingEntrySession.flavors.length) {
                                logger.debug(`found that provisioned`);
                            ...
                            let flavorsDiff = _.difference(flavorsDirs, recordingEntrySession.flavors);
                            if (flavorsDiff.length > 0) {
                                logger.debug(`found that currently provisioned flavors are different than previous. Diff: ${flavorsDiff}`);
                                return Q.resolve(false);
                            }
                            return Q.resolve(true);
                        })
