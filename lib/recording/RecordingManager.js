...
function chunksNameChained(mp4Files) {
    let chainedFilesStr =_.reduce(mp4Files, function (str, obj) {
        return str + obj.chunkName + " ";
    }, "");
    return chainedFilesStr;
}
...
// todo Assumptions:
    ...
    getRecordingEntrySession(entryObj, mp4FilesQueue = null) {
        ...
        if (mp4FilesQueue != null && mp4FilesQueue.length > 0) {
            logger.debug(`[${entryObj.entryId}-${entryObj.recordedEntryId}] getRecordingEntrySession called with mp4 files queue size [${mp4FilesQueue.length}][${chunksNameChained(mp4FilesQueue)}]`);
        }
        ...
        if (!this.recordingList[entryId]) {
            ...
            logger.debug(`[${entryObj.entryId}-${entryObj.recordedEntryId}] [getRecordingEntrySession] RecordingEntrySession is marked for deletion. Queue size [${recordingSession.mp4FilesQueue.length}][${chunksNameChained(recordingSession.mp4FilesQueue)}]`);
            let mp4FilesClone = _.clone(recordingSession.mp4FilesQueue);
            let newRecordingSession = new RecordingEntrySession(entryObj, mp4FilesClone);
            delete this.recordingList[entryId];
            this.recordingList[entryId] = newRecordingSession;
            return Q.resolve({session:newRecordingSession, sessionStarted:false});
        }
    ...
    splitRecording(entryId, reason) {
        ...
        if (!recordingSession.mp4FilesQueueLock) {
            ...
            logger.info(`[splitRecording] set lock mp4FilesQueueLock`);
            recordingSession.mp4FilesQueueLock = true;
            // mark session for deletion. Delete session only when creating new RecordingEntrySession
            ...
            return recordingSession.splitRecordedEntry(recordingSession.entryObject, reason)
                ...
                .catch((err)=> {
                    ...
                    logger.info(`[splitRecording] release lock mp4FilesQueueLock`);
                    recordingSession.mp4FilesQueueLock = false;
                });
        }
        else  {
    ...
    addNewChunks(Mp4Files, entryId, flavorId) {
        ...
        logger.info(`[${entryId}] Receiving new chunks: ${chunksNameChained(Mp4Files)}`);
        ...
        if (!recordingEntrySession.mp4FilesQueueLock &&
            recordingEntrySession.maxDurationReached() && !recordingEntrySession.disableRecording) {
            logger.info(`recording reached max duration [${mathUtils.durationToString(durationMSec, 2, mathUtils.HOUR_IN_MSEC)}] hrs`);
            recordingEntrySession.isUnlimitedRecordingDurationEnabled()
                .then((isEnabled)=> {
                    if (isEnabled) {
    ...
    replaceUnknownRecordedEntry(recordingEntrySession) {
        ...
        return backendClient.getEntryInfo(entryId)
            ...
            .finally(() => {
                ...
                logger.info(`[replaceUnknownRecordedEntry] release lock mp4FilesQueueLock`);
        ...
        if (this.recordingList[entryId]) {
            ...
            return recordingEntrySession.serializedActionsPromise
                ...
                .catch((err) => {
                    ...
                    if (this.recordingList[entryId] && !this.recordingList[entryId].markedForDeletion) {
                        delete this.recordingList[entryId];
                    }
                    return Q.resolve();
                });
            ...
            .then((sessionExist)=> {
                if (sessionExist) {
                    return sessionObj.session.verifyFlavorSetMatchPlaylistFlavors(entryObj)
                        .then((flavorsMatch) => {
                            if (!flavorsMatch) {
                                logger.info(`[handleFlavorsMismatch] set lock mp4FilesQueueLock`);
                                sessionObj.session.mp4FilesQueueLock = true;
                                // delete old RecordingEntrySession only when creating a new RecordingEntrySession
