...
// todo Assumptions:
    ...
    stopLive(entryId){
        ...
        logger.debug(`[${entryId}-${recordingEntrySession.recordedEntryId}][stopLive] live stream stopped. Recording duration will be updated`);
        if (!recordingEntrySession) {
            logger.warn(`[${entryId}-${recordingEntrySession.recordedEntryId}] [stopLive] can't updateServer recording, entry does not exist on recording list`);
            return Q.resolve();
        }
        return recordingEntrySession.updateEntryDuration(true)
            .catch((err)=> {
                logger.error(`[${entryId}-${recordingEntrySession.recordedEntryId}] [stopLive] failed to update duration. Error: ${ErrorUtils.error2string(err)}`);
            })
            .then(()=> {
                if (this.isRecordedEntryUnknown(recordingEntrySession.recordedEntryId)) {
                    this.replaceUnknownRecordedEntry(recordingEntrySession)
                        .catch((err)=> {
                            logger.error(`[${entryId}-${recordingEntrySession.recordedEntryId}] [stopLive] failed to replace recorded entry (content root dir not updated). Error: ${ErrorUtils.error2string(err)}`);
                        })
                        .then(()=> {
                            return Q.resolve();
                        });
    ...
    addNewChunks (Mp4Files, entryId, flavorId) {
        ...
        else if (recordingEntrySession.getDurationMSec() > this.recordingMaxDurationInMs) {
            ...
            return recordingEntrySession.updateEntryDuration(true)
                ...
                .finally(() => { // check first that all chunks are completed
                    ...
                    if (totalRecordingDurationMsec < this.recordingMinDurationInMS) {
                        logger.warn(`[${entryId}-${recordingEntrySession.recordedEntryId}][stopRecording] Recording duration is less than minimum required, do nothing. Total recording duration: [${totalRecordingDurationMsec}] msec, totalAddedChunks: [${totalAddedChunks}]`);
                    ...
                    return qio.write(stampFilePath, totalRecordingDurationMsec.toString())
                         .then(() => {
                            if (this.isRecordedEntryUnknown(recordingEntrySession.recordedEntryId)) {
                                logger.error(`[${entryId}-${recordingEntrySession.recordedEntryId}][stopRecording]: recorded entry is unknown, skip creating soft link. This is a bug, call developer`);
                                return Q.resolve()
                            }
                            let destFilePath = path.join(this.completedRecordingFolderPath, entryId + '_' + recordingEntrySession.recordedEntryId + '_' + totalRecordingDurationMsec);
                            return qio.symbolicLink(destFilePath, recordingEntrySession.recordingSessionPath, 'directory')
                                .then(() => {
                                    logger.info(`[${entryId}-${recordingEntrySession.recordedEntryId}][stopRecording] Successfully created soft link from ${recordingEntrySession.recordingSessionPath} to ${destFilePath}. Total recording duration [${totalRecordingDurationMsec}] msec, total chunks in this session: [${totalAddedChunks}] `);
                                })
                        })
                        .then(() => {
                            delete this.recordingList[entryId];
