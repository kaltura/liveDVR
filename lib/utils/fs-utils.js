...
function cleanFolder(targetPath, newName) {
    ...
    return qio.isDirectory(targetPath).then(function (res) {
        if (res) {
            ...
            let oldContentDirHashed =  path.join(oldContentFolderPath, persistenceFormat.getEntryHashIndex(newName));
            return qio.makeTree(oldContentDirHashed)
                .then(() => {
                    let renamedFolderName = path.join(oldContentDirHashed, newName + '_' + (new Date()).getTime().toString());
