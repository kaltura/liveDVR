...
function cleanFolder(targetPath, newName) {
    ...
    return qio.isDirectory(targetPath).then(function (res) {
        if (res) {
            ...
            var renamedFolderName = path.join(oldContentFolderPath, newName + '_' + (new Date()).getTime().toString());
...
function createHierarchyPath(destPath) {
    let currDay = new Date().getDate().toString();
    let currHour = new Date().getHours().toString();
    let fileFullPath = path.join(destPath, currDay, currHour);
...
module.exports = {
    ...
    createHierarchyPath : createHierarchyPath
};