...
ConcatSource.prototype.popClip = function() {
    ...
    logger.info('MixFilterClip.popClip. ', util.inspect(that.getItem(0)));

    that.expires.shift();
    that.state.totalDuration -= that.durations[0];
    that.durations.shift();
    that.scheduleClipRemoval(that.paths[0],Date.now() + palylistConfig.waitTimeBeforeRemoval);
    that.paths.shift();
    that.firstDTS.shift();
...
ConcatSource.prototype.concatClip = function (fileInfo,track,expires) {
    ...
    logger.info('ConcatSource.concatClip(%d,%s): appended clip=%s dts=%d enc_dts=%d dur=%d sig=%s',
...
ConcatSource.prototype.validate = function (opts) {
    ...
    var nonexisting =[];
    that.paths.forEach( function(path,index){
    ...
    if( nonexisting.length) {
        ...
        if(opts && opts.recover) {
            nonexisting.forEach(function (index) {
    ...
    if( !that.durations.every( function(d){
            return d >= 0;
        }) ) {
    ...
    var td = that.state.totalDuration,
        calcD = that.durations.reduce( sum,0);
    if( calcD  !== td ){
...
ConcatSource.prototype.collapseGap = function (from,to) {
    ...
    that.firstDTS.forEach(function(firstDTS,index){
...
MixFilterClip.prototype.checkExpires = function (expires,maxChunkCount){
    ...
    this.sources.forEach( function (s) {
        ...
        p = p.filter( function(file) {
            return that.sources.every(function (s1) {
...
MixFilterClip.prototype.getDTSRange = function(){
    ...
    that.sources.forEach(  function(s) {
...
Playlist.prototype.validate = function validate(opts) {
    ...
    if( !that.sequences.every( function(seq){
...
Playlist.prototype.recalculateOffsetsAndDuration = function recalculateOffsetsAndDuration (index){
    ...
    // determine dts range
    that.sequences.forEach(function(seq){
...
Playlist.prototype.serializeTo = function (stream){
    var that = this;

    var jsonStr = JSON.stringify(that);

    logger.debug("persistPlayList " + jsonStr);
