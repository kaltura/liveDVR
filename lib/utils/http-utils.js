...
var preserveOriginalHLS = configuration.get('preserveOriginalHLS').enable;


var HttpDownloader = function(loggerInfo ,compareFn, preserveHlsPath) {
    
    var logger = loggerModule.getLogger("Http-utils", loggerInfo);
    var obj = {
        preserveHlsPath: preserveHlsPath
    };

    obj.downloadFile = function(url, pathDestination, hlsPreservePath, timeout) {
        ...
        return networkClient
            ...
            .then((content) => {
                if (hlsPreservePath) {
                    //let chunklistPath = `${obj.preserveHlsPath}/${pathDestination.substr(pathDestination.lastIndexOf('/')+1)}`;
                    return qfs.write(hlsPreservePath, content)
                        .then(function () {
                            var savedTime = new Date();
                            var fileSaveTime = savedTime - downloadedTime;
                            logger.debug("Preserving copy of file %s took %d ms", hlsPreservePath, fileSaveTime);
                            return content;
                        })
                        .catch( err => {
                            logger.error(`Error preserving playlist to ${hlsPreservePath}. Error: ${ErrorUtils.error2string(err)}`);
                        })
                } else {
                    return content;
                }
    ...
    obj.downloadConvert = function(url, pathDestination, timeout, tsPath ) {
        var stream = new MP4WriteStream(pathDestination, tsPath);
