...
var HttpDownloader = function(loggerInfo ,compareFn) {
    ...
    obj.downloadConvert = function(url, pathDestination, timeout ) {
        let tsFilePath = preserveOriginalHLS ? `${pFormat.getBasePathFromFull(pathDestination)}/${url.substring(url.lastIndexOf('/')+1)}`: undefined;
        var stream = new MP4WriteStream(pathDestination, loggerInfo, tsFilePath);
