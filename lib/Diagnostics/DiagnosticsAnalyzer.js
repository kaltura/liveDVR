...
const decimalDigitsNum = config.get('diagnostics').decimalDigitsForFrameRateCalculation;
const frameRateDifferentialPercentAllowed = config.get('diagnostics').frameRateDifferentialPercentAllowed
...
class DiagnosticsAnalyzer extends EventEmitter{
    ...
    _checkInvalidKeyFrameInterval(flavorsArray) {
        ...
        let invalidKeyFrameFlavor = _.find(flavorsArray, f => {
            return Math.abs(keyFrameIntervalExpected - f.mediaInfo.keyFramesDistance) > keyFrameIntervalDiffAllowed
        });

        if (invalidKeyFrameFlavor) {
            let alertObj = new diagnosticsAlerts.InvalidKeyFrameIntervalAlert(invalidKeyFrameFlavor.name, invalidKeyFrameFlavor.mediaInfo.lastChunkName, keyFrameIntervalExpected / 1000, Math.round(invalidKeyFrameFlavor.mediaInfo.keyFramesDistance) / 1000);
            this._logger.info(alertObj.msg);
            this.alertsToSend.push(alertObj);
        }
        else {
    ...
    _checkFramesPerSecondRate(flavorsArray) {
        this._logger.debug(`Starting high frame rate check`);
        // Go over all flavors and return the first one that its FPS rate is too high
        let highFpsFlavor = _.find(flavorsArray, f => {
            return _.last(f.mediaInfo.actualFramerateArray) > maxFramesPerSeconds;
        });

        if (highFpsFlavor) {
            let alertObj = new diagnosticsAlerts.HighFpsRateAlert(highFpsFlavor.name, highFpsFlavor.mediaInfo.lastChunkName, _.last(highFpsFlavor.mediaInfo.actualFramerateArray));
            this._logger.info(alertObj.msg);
            this.alertsToSend.push(alertObj);
        }
        else {
    ...
    _checkInconsistentFrameRate(flavorsArray) {
        this._logger.debug(`Starting inconsistent frame rate check`);
        let inconsistentFlavor = _.find(flavorsArray, f => {
            // Make sure flavor exists in average map
            let flavor = f.name;
            if (!this._frameRateAvgMap.has(flavor)) {
                this._frameRateAvgMap.set(flavor, 0);
            }

            let lastFrameRateSample = _.last(f.mediaInfo.actualFramerateArray);
            // Return true if the last frame rate sample is different more than 20%
            let inconsistent = this._frameRateAvgMap.get(flavor) &&
                               (Math.abs(this._frameRateAvgMap.get(flavor) - lastFrameRateSample) > (lastFrameRateSample * frameRateDifferentialPercentAllowed));

            // Calculate new average
            let mean = +(_.reduce(f.mediaInfo.actualFramerateArray, (memo, num) => { return memo + num; }, 0) / f.mediaInfo.actualFramerateArray.length).toFixed(decimalDigitsNum);
            this._frameRateAvgMap.set(flavor, mean) ;

            return inconsistent;
        });

        if (inconsistentFlavor) {
            let alertObj = new diagnosticsAlerts.InconsistentFrameRateAlert(inconsistentFlavor.name,
                                                                            inconsistentFlavor.mediaInfo.lastChunkName,
                                                                            this._frameRateAvgMap.get(inconsistentFlavor.name),
                                                                            _.last(inconsistentFlavor.mediaInfo.actualFramerateArray));
            this._logger.info(alertObj.msg);
            this.alertsToSend.push(alertObj);
        }
        else {
            this.alertsToRemove.push(DiagnosticsErrorCodes.InconsistentFrameRateAlert + '_1');
        }
    }

    _checkConfiguredFrameRate(flavorsArray) {
        this._logger.debug(`Starting configured vs actual frame rate check`);
        let mismatchedFpsFlavored = _.find(flavorsArray, f => {
            return f.mediaInfo.actualFramerateArray.length &&
                (Math.abs(_.last(f.mediaInfo.actualFramerateArray) - f.mediaInfo.frameRate) > (f.mediaInfo.frameRate * frameRateDifferentialPercentAllowed));
        });

        if (mismatchedFpsFlavored) {
            let alertObj = new diagnosticsAlerts.FrameRateDoesNotMatchAlert(mismatchedFpsFlavored.name,
                                                                            mismatchedFpsFlavored.mediaInfo.lastChunkName,
                                                                            mismatchedFpsFlavored.mediaInfo.frameRate,
                                                                            _.last(mismatchedFpsFlavored.mediaInfo.actualFramerateArray));
            this._logger.info(alertObj.msg);
            this.alertsToSend.push(alertObj);
        }
        else {
            this.alertsToRemove.push(DiagnosticsErrorCodes.FrameRateDoesNotMatchAlert + '_1');
        }
    }

    _checkBackupStreamOnlyAlert(entryObject) {
        let alertObj;
        if (entryObject.recordStatus !== kalturaTypes.KalturaRecordStatus.DISABLED) {
        ...
        if (!entryObject.isRedundancy && (entryObject.serverType === kalturaTypes.KalturaEntryServerNodeType.LIVE_BACKUP)) {
            this._logger.info(alertObj.msg);
            this.alertsToSend.push(alertObj);
        }
        else {
    ...
    runFullDiagnosticsAnalysis(diagnosticsObj, entryObject) {
        try {
            ...
            this._checkBackupStreamOnlyAlert(entryObject);
            ...
            if (diagnosticsObj.flavors && diagnosticsObj.flavors.length) {
                ...
                this._checkInconsistentFrameRate(diagnosticsObj.flavors);
                this._checkConfiguredFrameRate(diagnosticsObj.flavors);
            }
