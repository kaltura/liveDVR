...
class DiagnosticsAnalyzer extends EventEmitter{
    constructor(entryId, alertsWindowSize) {
        ...
        this._frameRateMedianMap = new Map();
        this._frameRateStandardDeviationMap = new Map();
    ...
    _checkFramesPerSecondRate(flavorsArray) {
        ...
        let highFpsFlavor = _.find(flavorsArray, f => {
            return _.last(f.mediaInfo.framerateArray) > maxFramesPerSeconds;
        });
    ...
    _checkInconsistentFrameRate(flavorsArray) {
        let calculateStandardDeviation = (samples, median) => {
            let numerator = _.map(samples, s => {
                return Math.pow(s - median, 2);
            });

            return Math.sqrt(_.reduce(numerator) / (samples.length - 1));
        };

        let inconsistentFlavor = _.find(flavorsArray, f => {
            // Make sure flavor exists in both median and SD maps
            let flavor = f.name;
            if (!this._frameRateMedianMap.has(flavor) || !this._frameRateStandardDeviationMap.has(flavor)) {
                this._frameRateMedianMap.set(flavor, 0);
                this._frameRateStandardDeviationMap.set(flavor, 0);
            }

            let lastFrameRateSample = _.last(f.mediaInfo.framerateArray);
            let x = this._frameRateStandardDeviationMap.get(flavor);
            let y = this._frameRateMedianMap.get(flavor);
            let inconsistent = x && (Math.abs(y - lastFrameRateSample) > x);

            // Calculate new median and standard deviation
            let median = _.reduce(f.mediaInfo.framerateArray) / f.mediaInfo.framerateArray.length;
            this._frameRateMedianMap.set(flavor, median);
            this._frameRateStandardDeviationMap.set(flavor, calculateStandardDeviation(f.mediaInfo.framerateArray, median));
        ...
        if (inconsistentFlavor) {
            let alertObj = new diagnosticsAlerts.InconsistentFrameRateAlert(inconsistentFlavor.name,
                                                                            ...
                                                                            this._frameRateMedianMap.get(inconsistentFlavor.name),
                                                                            _.last(inconsistentFlavor.mediaInfo.framerateArray))
            this._logger.info(alertObj.msg);
            this.alertsToSend.push(alertObj);
        }
        else {
