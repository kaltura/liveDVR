...
class EntryDiagnostics extends EventEmitter{
    ...
    _checkInvalidKeyFrameInterval(flavorsArray) {
        ...
        if (invalidKeyFrameFlavor) {
            let alertObj = new diagnosticsAlerts.InvalidKeyFrameInterval(invalidKeyFrameFlavor.name, invalidKeyFrameFlavor.mediaInfo.lastChunkName, keyFrameIntervalExpected / 1000, Math.round(invalidKeyFrameFlavor.mediaInfo.keyFrameDistance) / 1000);
            this._logger.info(alertObj.msg);
            this.alertsToSend.push(alertObj);
        }
        else {
            this.alertsToRemove.push(DiagnosticsErrorCodes.InvalidKeyFrameInterval + '_1');
        }
    }
    ...
    _checkFramesPerSecondRate(flavorsArray) {
        ...
        if (highFpsFlavor) {
            let alertObj = new diagnosticsAlerts.HighFpsRate(highFpsFlavor.name, highFpsFlavor.mediaInfo.lastChunkName, highFpsFlavor.mediaInfo.framerate);
            this._logger.info(alertObj.msg);
            this.alertsToSend.push(alertObj);
        }
        else {
            this.alertsToRemove.push(DiagnosticsErrorCodes.HighFpsRate + '_1');
        }
    }

    _checkStreamTypeAlert(serverType, redundancy, input) {
        let alertObj = new diagnosticsAlerts.BackupOnlyStream(this._entryId, input);
    ...
    runFullDiagnosticsAnalysis(diagObj, serverType, redundancy) {
        try {
            ...
            if (diagObj.flavors && diagObj.flavors.length) {
                this._checkInvalidKeyFrameInterval(diagObj.flavors);
                this._checkFramesPerSecondRate(diagObj.flavors);
            }
