            ...
            _.each(diagObj.inputs, (input, index)=> {
                ...
                if (input.Encoder) {
                    ...
                    if (input.IOPerformance) {
                        this._checkIncomingBitrate(input.IOPerformance.bitrate, this._getConfiguredDatarate(diagObj.inputs[index]), indexNum);
                    }
                    if (input.Encoder.syncPTSData) {
                        this._checkPtsSynchronization(diagObj.currentTime, input.Encoder.syncPTSData, indexNum);
                    }
                }
            });
