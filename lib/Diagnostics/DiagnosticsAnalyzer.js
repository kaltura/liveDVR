...
class EntryDiagnostics extends EventEmitter{
    constructor(entryId, alertsWindowSize) {
        ...
        this.alertsWindowSizeInMSec = alertsWindowSize;
    ...
    checkAudioVideoSignals(audioDataRate, videoDataRate, input) {
        ...
        if (currTime - this.lastAudioVideoCheck.get(input) > this.alertsWindowSizeInMSec) {
    ...
    runFullDiagnosticsAnalysis(diagObj) {
        try {
            ...
            _.each(diagObj.inputs, (input, index)=> {
                if (input.Properties) {
                    this.hasEntryRestarted(input.Properties.timeRunningSeconds, index);
                }
                if (input.Encoder) {
            ...
            if (this.alertsToSend.length) {
                this.emit('reportBeacon', this.alertsToSend);
            }
            if (this.alertsToRemove.length) {
                this.emit('removeBeacon', this.alertsToRemove);
            }
        }
        catch (error) {
