...
class PtsDrift extends EntryAlert {
    constructor(entryId, ptsInfo, input) {
        super(entryId, input);
        this.args.video = this.buildPtsAlertJson(ptsInfo, 'video');
        this.args.audio = this.buildPtsAlertJson(ptsInfo, 'audio');
    }

    buildPtsAlertJson (ptsInfo, type = "") {
        ...
        switch (type) {
            case 'video':
                return {
                    ...
                    first: getData(ptsInfo.firstPts, 0),
                    last: getData(ptsInfo.lastPts, 0)
                };
            case 'audio':
                return {
                    drift: ptsInfo.audioAnalysis.drift,
                    first: getData(ptsInfo.firstPts, 1),
                    last: getData(ptsInfo.lastPts, 1)
                };
            default:
                return {};
        }
