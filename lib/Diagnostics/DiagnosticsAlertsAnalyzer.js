...
class EntryDiagnostics {
    ...
    calculatePtsDrift(first, last, type = '') {
        ...
        let calcPtsRate = (index)=> {
           absPtsFirst = first.ptsInfo[index][0] - first.ptsInfo[index][1] + first.ptsInfo[index][2];
           absPtsLast = last.ptsInfo[index][0] - last.ptsInfo[index][1] + last.ptsInfo[index][2];
        ...
        return {
            ...
            isAlert : Math.abs(ptsDrift - 1) > (ptsPercentageDifference / 100)
        };
    }

    buildPtsAlertJson (first, last, videoDrift, audioDrift) {
        let getData = (data, index) => {
            return {
                refClock : data.ptsInfo[index][0],
                refPts : data.ptsInfo[index][1],
                lastPts : data.ptsInfo[index][2]
            }
        };

        return {
            video: {
                drift: videoDrift.drift,
                first: getData(first, 0),
                last: getData(last, 0)
            },
            audio: {
                drift: audioDrift.drift,
                first: getData(first, 1),
                last: getData(last, 1)
            }
        };
    }
    ...
    checkPtsSynchronization(clock, ptsInfo, input) {
        ...
        let firstPts = this.ptsDataHistory[0];
        let lastPts = this.ptsDataHistory[this.ptsDataHistory.length - 1];
        ...
        let alertArgs = this.buildPtsAlertJson(firstPts, lastPts, videoPtsDrift, audioPtsDrift);
