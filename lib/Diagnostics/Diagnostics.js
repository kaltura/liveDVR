...
class Diagnostics extends EventEmitter {
    constructor(entryObject) {
        ...
        this.beaconPrefix = entryObject.serverType;
        this.isRedundancy = entryObject.isRedundancy;
    ...
    _initAlertsAnalyzer(entryId) {
        this._diagnosticsAnalyzer = new DiagnosticsAnalyzer(entryId, this._alertWindowSizeInMSec);
        // Enlist on event
    ...
    updateBeacons(beaconsModificationObj) {
        ...
        let isRemoved = false;
        this._logger.debug(`Started [updateBeacons] method`);
        ...
        if (isReported || isRemoved) {
            this._logger.debug(`Alerts in map have been modified. Updating streamHealth`);
            return this._updateStreamHealth.call(this)
                .then(() => {
                    this._logger.debug('Beacons updated and sent to server. Erasing all \"Timed\" alerts');
                    this.reportedAlerts.forEach((value, key) => {
                        if (value.isTimed) {
                            this._logger.debug(`Alert ${value.name} was sent to server and \"isTimed\". Erasing from map`);
                            this.reportedAlerts.delete(key);
                        }
                    });
                })
        }
    ...
    _reportBeacon(alertObj) {
        ...
        alertObj.forEach(a => {
            ...
            if (!this.reportedAlerts.has(a.hashKey)) {
                this._logger.debug(`[reportBeacon] Alert [${a.name}] inserted to map ; Severity: [${a.severity}]`);
                insertResult = true;
            }
            else {
    ...
    _updateStreamHealth() {
        ...
        .then((result)=> {
            return this._syncPromise.exec(()=> {
                this._logger.debug(`Sending [${this.beaconPrefix}_healthData] beacon. Stream max severity: [${result}]`);
                let data = {
                    maxSeverity: result,
                    alerts: [...this.reportedAlerts.values()]
                };
                return BackendClient.sendBeacon(this.entryId, this.partnerId, data, this.beaconPrefix + '_healthData');
            });
        });
    }
    ...
    _runDiagnosticsReport() {
        ...
        return Q.fcall(() => {
            ...
            if (this.staticMetaData.modified) {
                this._logger.info(`Entry static information has been modified. Reporting [${this.beaconPrefix}_staticData] beacon`);
                return BackendClient.sendBeacon(this.entryId, this.partnerId, this.staticMetaData.data, this.beaconPrefix + '_staticData')
                    .then(()=> {
                        this.staticMetaData.modified = false;
                    });
            }
        ...
        .then(() => {
            ...
            if (this.dynamicMetaData.flavors.length) {
                this._logger.info(`Sending [${this.beaconPrefix}_dynamicData] beacon`);
                return BackendClient.sendBeacon(this.entryId, this.partnerId, this.dynamicMetaData, this.beaconPrefix + '_dynamicData');
            }
        })
        .then(() => {
        ...
        .finally(() => {
            // Only if module should keep running create a new timeout handler
            if (this._shouldRun) {
    ...
    handleDiagnosticsReport(metaData) {
        ...
        if (this.lastDiagnosticsObjTime === metaData.source.currentTime)
            return;
        // beaconPrefix is the string casting of the server type
        this._diagnosticsAnalyzer.runFullDiagnosticsAnalysis(metaData, parseInt(this.beaconPrefix), this.isRedundancy);
