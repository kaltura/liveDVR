...
class Diagnostics extends EventEmitter {
    ...
    _initAlertsAnalyzer(entryId) {
        ...
        // Enlist on events
        this._diagnosticsAnalyzer.on('reportBeacon', this.reportBeacon.bind(this));
        this._diagnosticsAnalyzer.on('removeBeacon', this.removeBeacon.bind(this));
    }
    ...
    reportBeacon(alertObj) {
        ...
        return this._updateStreamHealth.call(this)
            .then(()=> {
                this._logger.debug('Beacon inserted to map and sent to server');
            })
    }

    removeBeacon(key) {
        let removedAlert = this.reportedAlerts.get(key);
        let deleteResult = this.reportedAlerts.delete(key);
