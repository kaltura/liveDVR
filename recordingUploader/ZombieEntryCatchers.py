...
polling_interval_sec= get_config('cron_job_polling_interval_hours') * 60 * 60


def job(sc):
    logger.info("Start scanning direcotry in %s", recordings_dir)
    now = int(time.time())
    for entry_id in os.listdir(recordings_dir):
        try:
            entry_id_path = os.path.join(recordings_dir, entry_id)
            if not pattern.match(entry_id) or not os.path.isdir(entry_id_path):
                continue
            for recorded_id in os.listdir(entry_id_path):
                recorded_id_path = os.path.join(entry_id_path, recorded_id)
                if not pattern.match(recorded_id) or not os.path.isdir(recorded_id_path):
                    continue
                ...
                logger.debug("[%s] now %s last_modify_time %s, diff: %s, is_expired : %s, done_exist: %s", session_id, now, last_modify_time, now-last_modify_time, is_expired, done_exist)
